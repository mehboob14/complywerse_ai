...
class HandshakeIn(BaseModel):
    ...
    # Collector routing: when set, scans for this connection go through
    # the named collector agent (a Linux box inside the customer's LAN)
    # instead of being executed directly by the backend. Required for
    # cloud-SaaS deployments where the backend can't reach customer LAN
    # IPs. None = legacy direct execution.
    assigned_collector_agent_id: Optional[int] = None
    ...
    # When an operator clicks "Connect" on an EXISTING app-asset page,
    # we pass its asset_id so the backend skips auto-create and links the
    # new connection directly to that asset. The connection scope is also
    # automatically narrowed to just this asset (asset_list mode).
...
def handshake(body: HandshakeIn, db: Session = Depends(get_db)):
    ...
    if platform == "windows":
        ...
        console_port = body.db_port or 27017
    elif platform == "ad":
    ...
    if platform in ("mssql", "postgres", "mysql", "mongodb"):
        prefix_map = {"mssql": "mssql", "postgres": "postgres", "mysql": "mysql", "mongodb": "mongodb"}
    ...
    if existing:
        ...
        conn.is_active = True
        # Persist collector routing on update too (operator can move a
        # connection between collectors later).
        if body.assigned_collector_agent_id is not None:
            conn.assigned_collector_agent_id = body.assigned_collector_agent_id
    else:
        conn = IntegrationConnection(
            ...
            assigned_collector_agent_id=body.assigned_collector_agent_id,
        )
        db.add(conn)
    db.flush()

    # ─── Asset linking ────────────────────────────────────────────────
    # When `target_asset_id` is provided the operator clicked "Connect"
    # on an EXISTING app-asset (e.g. PostgreSQL 18 child-asset). We use
    # that asset directly — no hostname lookup, no auto-create.  The
    # scope on the connection is narrowed to just that asset.
    if body.target_asset_id:
        asset = (
            db.query(ITAsset)
            .filter(
                ITAsset.id == body.target_asset_id,
                ITAsset.tenant_id == tenant_id,
            )
            .first()
        )
        if asset:
            # Auto-scope this credential to the one app-asset. The
            # operator can widen it later in the Connections admin page.
            conn.scope_mode = "asset_list"
            conn.scope_value = {"asset_ids": [asset.id]}
    else:
        # Default: find or auto-create an asset by hostname
        asset = (
            db.query(ITAsset)
            .filter(
                ITAsset.tenant_id == tenant_id,
    ...
    from grc.modules.compliance_plugins.services.os_detector import (
        ...
        truncate_os_key_to_second_level,
    )
    os_family, os_version, os_normalized, os_build, os_edition = detect_for_runner_full(
        integration_type, test_creds or {}
    )
    # SAFEGUARD: even though detect_for_runner_full's Linux branch already
    # uses normalise_linux (which truncates), the Windows/Cisco branches
    # build their own keys. Apply a defensive truncation on the way out so
    # ALL wizard-created assets land at level-2 regardless of which
    # platform branch produced the key.
    os_normalized = truncate_os_key_to_second_level(os_normalized) if os_normalized else os_normalized

    if not asset and not body.target_asset_id:
        # Only auto-create an asset for the DEFAULT flow (no explicit target).
        # When the operator connected via an app-asset's "Connect" button
        # (target_asset_id provided), the asset already exists — skip creation.
        asset = ITAsset(
            tenant_id=tenant_id,
            name=label,
            ...
            criticality="medium",
            status="active",
            # The Wizard operator becomes the asset owner. Without this
            # auto-created assets show as "unowned" in the team view and
            # operators can't tell who's responsible for each device.
    ...
    # ─── App inventory at connect time ("rides the connection") ─────────
    # The moment an agentless host is connected we pull the applications
    # running inside it — the SAME 3-layer inventory an agent reports on
    # heartbeat — so the "Applications on this host" panel populates right
    # away, before any CIS scan. This is purely connection-driven; the CIS
    # "Scan now" button is unrelated. Best-effort: an inventory failure must
    # never fail the connect (the OS profile + credential are already saved).
    if asset is not None and integration_type in ("windows_winrm", "linux_ssh"):
        try:
            from grc.modules.compliance_plugins.services import agentless_inventory as _ai
            _ai.probe_and_store(db, asset, conn)
        except Exception as _inv_exc:  # noqa: BLE001
            import logging as _lg
            _lg.getLogger(__name__).warning(
                "connect-time app inventory skipped for %s: %s", body.hostname, _inv_exc)
