...
const getTenantSlugFromHost = (): string | null => {
  ...
  // Skip Replit hosting domains — they are not tenant subdomains
  if (host.endsWith('.replit.dev') || host.endsWith('.replit.app')) return null;
  ...
  (error: AxiosError) => {
    if (error.response?.status === 401) {
      if (typeof window !== 'undefined') {
        ...
        // A hard redirect on /login would cause an infinite reload loop
        // (AuthGuard fires 401 → interceptor reloads /login → repeat).
...
// ─── Policy AI HITL (Task #46) ─────────────────────────────────────────────
export const policyAIApi = {
  // Patch proposals (HITL inbox)
  listProposals: (params?: { document_id?: number; status?: string }) =>
    apiClient.get('/governance/patch-proposals', { params }),
  getProposal: (id: number) =>
    apiClient.get(`/governance/patch-proposals/${id}`),
  decide: (id: number, body: { decision: 'approve' | 'reject' | 'edit'; comment?: string; edited_text?: string }) =>
    apiClient.post(`/governance/patch-proposals/${id}/decision`, body),
  criticalDecision: (id: number, body: { decision: 'allow' | 'deny'; justification: string }) =>
    apiClient.post(`/governance/patch-proposals/${id}/critical-decision`, body),
  sendEmail: (id: number, approver_user_id: number) =>
    apiClient.post(`/governance/patch-proposals/${id}/send-email`, { approver_user_id }),

  // Critical rules
  listCriticalRules: (framework_id?: number) =>
    apiClient.get('/governance/critical-rules', { params: framework_id ? { framework_id } : undefined }),
  updateCriticalRule: (id: number, body: { enabled?: boolean; params?: Record<string, any>; approver_chain?: any[] }) =>
    apiClient.put(`/governance/critical-rules/${id}`, body),

  // Clause coverage / dedup
  scanCoverage: (document_id: number, use_ai: boolean = true) =>
    apiClient.post(`/governance/clause-coverage/scan/${document_id}?use_ai=${use_ai}`),
  listCoverage: (document_id: number, framework_id?: number) =>
    apiClient.get(`/governance/clause-coverage/${document_id}`, { params: framework_id ? { framework_id } : undefined }),
  recordCoverageDecision: (document_id: number, coverage_id: number, body: { user_choice: string; lock?: boolean }) =>
    apiClient.post(`/governance/clause-coverage/${document_id}/decision/${coverage_id}`, body),
  dedupPreflight: (document_id: number, framework_id: number) =>
    apiClient.get(`/governance/clause-coverage/dedup-preflight/${document_id}`, { params: { framework_id } }),
  tenantCoverageReport: (framework_id: number) =>
    apiClient.get(`/governance/clause-coverage/tenant-report/${framework_id}`),
};
...
export const assetsApi = {
  ...
  getChildren: (id: number) => apiClient.get(`/assets/${id}/children`),
  // IP-based group: all assets at the same IP address (inventory-imported linking)
  getIPPeers: (id: number) => apiClient.get(`/assets/${id}/ip-peers`),
  // Criticality weights for the composite score formula (tenant-configurable)
  getCompositeWeights: () => apiClient.get('/assets/composite-weights'),
  updateCompositeWeights: (w: { low: number; medium: number; high: number; critical: number }) =>
    apiClient.put('/assets/composite-weights', w),
  resetCompositeWeights: () => apiClient.delete('/assets/composite-weights'),
  getSoftwareCatalog: () => apiClient.get('/assets/software-catalog'),
  getOsCatalog: () => apiClient.get('/assets/os-catalog'),
  addApplication: (id: number, software_key: string, display_name?: string, criticality?: string) =>
    apiClient.post(`/assets/${id}/add-application`, { software_key, display_name, criticality }),
  ...
  // security-compliance/* endpoints removed — the tab they fed was wired to
  // a static WS2012R2 seed file that ignored the strict OS matcher. Real
  // per-asset rule selection lives on /compliance/plugins/library.
...
export const vulnManagementApi = {
  ...
  dashboard: {
    ...
    getDiscoveryTrend: (months?: number) => 
      apiClient.get('/vuln-management/dashboard/discovery-trend', { params: { months } }),
  },
};
...
export const adminApi = {
  ...
  getAuditLogs: (params?: { limit?: number; offset?: number; action?: string; exclude_action?: string; module?: string; user_id?: number; start_date?: string; end_date?: string; actor_type?: string; search?: string }) =>
    apiClient.get('/admin/audit-logs', { params }),
  getAuditLogFilters: () => apiClient.get<{ actions: string[]; modules: string[]; date_presets: string[]; actor_types: string[] }>('/admin/audit-logs/filters'),
  generateAuditLogAiSummary: (logId: number, force = false) =>
    apiClient.post<{ ai_summary: string | null; cached?: boolean; fallback?: boolean }>(`/admin/audit-logs/${logId}/ai-summary`, {}, { params: { force } }),
};
...
export const compliancePluginsApi = {
  list: (params?: Record<string, unknown>) =>
    apiClient.get('/compliance-plugins', { params }),
  get: (id: number) =>
    apiClient.get(`/compliance-plugins/${id}`),
  benchmarks: () =>
    apiClient.get('/compliance-plugins/benchmarks'),
  seed: () =>
    apiClient.post('/compliance-plugins/seed'),
  ...
  listRuns: (params?: Record<string, unknown>) =>
    apiClient.get('/compliance-plugins/runs', { params }),
  getRun: (runId: number) =>
    apiClient.get(`/compliance-plugins/runs/${runId}`),
  ...
  perUserSummary: () =>
    apiClient.get('/compliance-plugins/per-user-summary'),
  assetsOverview: () =>
    apiClient.get('/compliance-plugins/assets-overview'),
  perAssetCoverage: (assetId: number) =>
    apiClient.get('/compliance-plugins/per-asset-coverage', { params: { asset_id: assetId } }),
  matchPreview: (assetId: number) =>
    apiClient.get('/compliance-plugins/match-preview', { params: { asset_id: assetId } }),
  // Admin tenant-wide overview — one aggregation call powering /admin/overview
  adminOverview: () => apiClient.get('/admin/overview'),
  // Gap B — AI mapping suggester for assets whose os_normalized has no
  // matching grc_benchmark_os_mappings row. Returns {benchmark_name,
  // confidence, reasoning}. Anti-hallucination: AI suggests, operator
  // confirms — this endpoint never writes the mapping.
  ...
  setConnectionScope: (id: number, body: { scope_mode: string; scope_value?: Record<string, unknown> }) =>
    apiClient.put(`/compliance-plugins/connections/${id}/scope`, body),
  previewConnectionScope: (id: number, body: { scope_mode: string; scope_value?: Record<string, unknown> }) =>
    apiClient.post(`/compliance-plugins/connections/${id}/scope-preview`, body),
  reDetectAssetOs: (assetId: number) =>
    apiClient.post(`/compliance-plugins/assets/${assetId}/re-detect-os`, {}),
  libraryTree: () => apiClient.get('/compliance-plugins/library-tree'),
  ...
  reviewBulk: (pluginIds: number[], decision: 'approve' | 'reject') =>
    apiClient.post('/compliance-plugins/review-bulk', {
  ...
  getAssetScope: (pluginId: number) =>
    apiClient.get(`/compliance-plugins/${pluginId}/asset-scope`),
  ...
  reparseIngestJob: (jobId: number) =>
    apiClient.post(`/compliance-plugins/ingest/${jobId}/reparse`),
  deleteIngestJob: (jobId: number) =>
    apiClient.delete(`/compliance-plugins/ingest/${jobId}`),
...
export const riskPostureApi = {
  ...
  previewAsset: (assetId: number, data: {
    is_customer_facing?: boolean;
    is_internet_facing?: boolean;
    regulated_data_type?: string;
    operational_dependency?: string;
  }) => apiClient.post(`/risk-posture/asset/${assetId}/preview`, data),
};
