...
def _run_to_dict(r: CompliancePluginRun, plugin: Optional[CompliancePlugin] = None,
                 ...
                 triggered_by_user: Optional[GRCUser] = None,
                 asset_owner: Optional[GRCUser] = None) -> dict:
    return {
        "id": r.id,
        ...
        # Asset OWNER (who's responsible for the asset) — distinct from the
        # user who triggered this particular run. Lets the operator see
        # "scan triggered by X against asset owned by Y" in one row.
        "asset_owner": _user_label(asset_owner),
...
def match_preview(
    ...
    # result.
    from .services.strict_matcher import (
        applicable_plugins_for_asset_aware, pick_benchmark_for_os,
    )

    # Strict mapping row (OS hosts). None for application assets that match
    # via the library fallback — that's expected; matcher_mapping below then
    # reports null while primary_benchmark still resolves from the library.
    mapping = pick_benchmark_for_os(db, tenant_id, os_normalized or "")
    # Asset-role-aware: OS hosts stay strict (mapping table); application
    # assets (promoted children like postgresql-18) fall back to matching
    # their benchmark straight from the library, so they're scannable and
    # auto-detect newly uploaded app benchmarks.
    stage2_kept, picked_bench = applicable_plugins_for_asset_aware(
        db, tenant_id, os_normalized or "", getattr(asset, "asset_role", None),
    )
...
def library_tree(
    ...
    # SAFETY: filter to rows where os_keys is a JSONB array BEFORE
    # calling jsonb_array_elements_text — otherwise a single row with a
    # scalar value (legacy from before the array convention, or a row
    # mid-write from the bulk ingest) crashes the whole query with
    # "cannot extract elements from a scalar".
    key_counts_rows = db.execute(_sql_text(
        "SELECT key, COUNT(*) AS n "
        "FROM ( "
        "  SELECT os_keys FROM grc_compliance_plugins "
        ...
        ") AS sub, "
        "jsonb_array_elements_text(sub.os_keys) AS key "
        "GROUP BY key"
    ), {"tid": tenant_id}).all()
    ...
    # ── Auto-create registry buckets for any os_key not in grc_os_versions ──
    # Why this exists: the OS-version registry table is operator-maintained
    # and lags behind the 200+ benchmarks we now ingest. Before this fix,
    # any benchmark whose os_key wasn't in the registry (e.g. cassandra-4.0,
    # fortigate-7.4, mongodb-6, apache-httpd-2.4) silently dumped into
    # "Other / unclassified" — burying ~45% of all rules. We now infer the
    # family from the key prefix and create synthetic registry nodes on
    # the fly, so the library tree organises everything correctly without
    # operator intervention.
    def _infer_family_from_key(k: str) -> tuple[str, str]:
        """Return (family, parent_key) for an unknown os_key.

        Match strategy: check exact non-versioned tokens FIRST (so a key
        like "github" or "freebsd" classifies cleanly), then prefix match
        for versioned keys (like "cassandra-4.0").
        """
        kl = k.lower()
        # Exact non-versioned tokens for products that have one release
        # only or where we don't track version in the key.
        EXACT_NETWORK   = {"f5", "paloalto", "juniper", "pfsense", "checkpoint",
                           "fortigate", "hpe-aruba",
                           "cisco-firewall", "cisco-firepower", "cisco-asa",
                           "cisco-nx-os", "cisco-ios-xr"}
        EXACT_DB        = {"postgresql", "mysql", "mariadb", "snowflake", "redis",
                           "elasticsearch", "ibm-db2", "ibm-cics"}
        EXACT_WEB       = {"nginx", "websphere"}
        EXACT_CONTAINER = {"docker", "kubernetes"}
        EXACT_LINUX     = {"freebsd", "aliyun", "alibaba-linux",
                           "ibm-aix", "chromeos", "google-cos",
                           "bottlerocket"}
        EXACT_MAINFRAME = {"ibm-zos"}
        EXACT_CLOUD     = {"github", "gitlab", "bitbucket", "google-workspace",
                           "microsoft-365", "ibm-cloud", "digitalocean",
                           "snowflake", "aws", "azure", "gcp", "alibaba",
                           "oracle-cloud", "aws-account"}
        if kl in EXACT_NETWORK:   return ("network", "network")
        if kl in EXACT_DB:        return ("db", "db")
        if kl in EXACT_WEB:       return ("web", "web")
        if kl in EXACT_CONTAINER: return ("container", "container")
        if kl in EXACT_LINUX:     return ("linux", "linux")
        if kl in EXACT_CLOUD:     return ("cloud", "cloud")
        if kl in EXACT_MAINFRAME: return ("mainframe", "mainframe")

        # Prefix matches for versioned keys (cassandra-4.0, mongodb-6, etc).
        if kl.startswith("cisco-"):
            return ("cisco", "cisco")
        if kl.startswith(("fortigate-", "f5-", "pfsense-", "checkpoint-",
                          "juniper-", "paloalto-")):
            return ("network", "network")
        if kl.startswith(("docker-", "kubernetes-", "k8s-", "openshift-")):
            return ("container", "container")
        if kl.startswith(("vmware-", "esxi-", "vmware-esxi-", "hyper-v")):
            return ("hypervisor", "hypervisor")
        if kl.startswith(("mssql-", "postgres-", "postgresql-", "mysql-",
                          "oracle-db-", "mongodb-", "cassandra-", "redis-",
                          "elasticsearch-", "mariadb-", "ibm-db2-")):
            return ("db", "db")
        if kl.startswith(("apache-httpd", "apache-tomcat", "tomcat-",
                          "nginx-", "iis-", "jboss-", "websphere-")):
            return ("web", "web")
        if kl.startswith("macos-"):
            return ("apple", "apple")
        if kl.startswith(("ubuntu-", "debian-", "rhel-", "centos-",
                          "almalinux-", "rocky-", "oraclelinux-",
                          "amazonlinux-", "sles-", "suse-", "aliyun-",
                          "alibaba-linux-", "fedora-", "freebsd-", "aks-")):
            return ("linux", "linux")
        if kl.startswith(("aws-", "azure-", "gcp-", "alibaba-",
                          "digitalocean-", "oracle-cloud-", "google-",
                          "microsoft-365-")):
            return ("cloud", "cloud")
        if kl.startswith("windows-"):
            return ("windows", "windows")
        return ("other", "other")

    # Track which family/parent keys we auto-created so we don't dup
    auto_family_node_added: set[str] = set()
    ...
    for bench, os_keys, n in bench_rows:
        ...
        if target_key and target_key in by_key:
            ...
            # Auto-create a leaf node for this key. Infer family from the
            # key prefix, attach under the inferred family rollup.
            fam, parent_key = _infer_family_from_key(target_key)
            families.add(fam)
            # Make sure the family-level rollup node exists too
            if parent_key not in by_key:
                by_key[parent_key] = {
                    "key": parent_key,
                    "label": parent_key.title(),
                    "family": fam,
                    "rule_count": rules_per_key.get(parent_key, 0),
                    "children": [],
                    "benchmarks": [],
                }
                # Register as a top-level (parent=None) node so it shows
                # under the family root.
                if parent_key not in auto_family_node_added:
                    by_parent.setdefault(None, []).append(by_key[parent_key])
                    auto_family_node_added.add(parent_key)
            # Build the leaf for this specific key (e.g. cassandra-4.0)
            leaf_label = target_key.replace("-", " ").title()
            leaf = by_key.setdefault(target_key, {
                "key": target_key,
                "label": leaf_label,
                "family": fam,
                "rule_count": rules_per_key.get(target_key, 0),
                "children": [],
                "benchmarks": [],
            })
            # Attach leaf under the family rollup if not already there
            if leaf not in by_key[parent_key]["children"]:
                by_key[parent_key]["children"].append(leaf)
            leaf["benchmarks"].append(bench_node)
        else:
            # Truly unclassified — empty os_keys. These are the only
            # cases that legitimately belong in "Other / unclassified".
    ...
    # Build family roots
    family_labels = {
        "windows": "Windows", "linux": "Linux / Unix", "apple": "Apple macOS",
        "cisco": "Cisco", "network": "Network / Firewalls",
        "cloud": "Cloud Accounts", "db": "Databases",
        "web": "Web / App Servers", "container": "Containers / Kubernetes",
        "hypervisor": "Hypervisors / VMware",
        "mainframe": "Mainframe",
        "other": "Other",
    }
    # Sum a node's own rules PLUS all descendants' rules + attached benchmarks.
    # Without this, auto-created family rollups (apple, web, db) show 0
    # because rules_per_key only has leaf-level keys (macos-14.4, not "apple").
    def _walk_total(node: dict) -> int:
        total = rules_per_key.get(node.get("key"), 0)
        total += sum(b.get("rule_count", 0) for b in (node.get("benchmarks") or []))
        for child in node.get("children") or []:
            total += _walk_total(child)
        return total
    ...
    for fam in sorted(families):
        ...
        # Stamp rule_count on each root (and descendants) so the UI shows
        # the true total at every level.
        for r in roots:
            r["rule_count"] = _walk_total(r)
            for grandchild in r.get("children") or []:
                grandchild["rule_count"] = _walk_total(grandchild)
        total_rules = sum(r["rule_count"] for r in roots)
        tree.append({
            "key": f"family::{fam}",
    ...
    # BUG-FIX: prune empty branches. The OS Registry contains every OS
    # the system *recognises* (Ubuntu 20.04, RHEL 7, etc.) — but only
    # some have benchmarks loaded. Showing "Ubuntu 20.04 LTS" with 0
    # rules confuses operators ("why is it there if I can't scan it?").
    # Walk the tree post-order and drop any node that has NEITHER:
    #   - own rules / benchmarks, nor
    #   - any non-empty descendant
    # Family nodes (top level) are pruned only if EVERY child is empty.
    def _prune(node: dict) -> bool:
        """Return True if this node has any content (own or descendant).
        Mutates the node's children to drop empty subtrees."""
        own = (node.get("rule_count") or 0) > 0 or bool(node.get("benchmarks"))
        kept_children = []
        for child in node.get("children", []) or []:
            if _prune(child):
                kept_children.append(child)
        node["children"] = kept_children
        return own or bool(kept_children)

    pruned_tree = [n for n in tree if _prune(n)]

    return {
        "tree": pruned_tree,
        "total_rules": sum(n for n in rules_per_key.values()),
...
def list_runs(
    ...
    # Also resolve every asset owner so each row carries "owned by" data.
    owner_ids = [getattr(a, "owner_id", None) for a in assets_by_id.values()]
    owner_ids = [oid for oid in owner_ids if oid]
    users_by_id = {
        u.id: u for u in db.query(GRCUser)
                          .filter(GRCUser.id.in_(list(set(user_ids + owner_ids)) or [0]))
                          .all()
    }
    return {
        "runs": [
            _run_to_dict(
                ...
                asset_owner=users_by_id.get(
                    getattr(assets_by_id.get(r.asset_id), "owner_id", None)
                ),
            )
            for r in runs
        ],
...
def scan_all(
    ...
    # Everything from here on MUST be inside try/finally so an uncaught
    # exception releases the lock — otherwise a single 500 leaves the
    # asset un-scannable until the backend process is restarted (real bug
    # observed in live walkthrough on 2026-06-05 with asset 46).
    try:
        return _do_scan_all(
...
def _do_scan_all(
    ...
    *, benchmark, runner_type, connection_id,
    ...
    # SessionLocal() and closes it on exit. Concurrency cap of 10 keeps
    # WinRM/SSH connection counts well under what a typical target host
    # will accept, and avoids overwhelming the backend's network/file-
    # descriptor budget on a 1000-rule scan.
    from concurrent.futures import ThreadPoolExecutor, as_completed
    from grc.models import SessionLocal as _SessionLocal

    def _one(plugin, eff_asset, conn):
        worker_db = _SessionLocal()
        try:
            run = execute_plugin(
                db=worker_db,
                tenant_id=tenant_id,
