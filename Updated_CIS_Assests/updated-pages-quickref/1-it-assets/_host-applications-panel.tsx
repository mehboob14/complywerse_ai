            ...
            {(() => {
              ...
              const connectHref = wizPlatform
                ? _OS_PLATFORMS.has(wizPlatform)
                  ? `/integrations/connect/method?platform=${wizPlatform}&asset_id=${g.id}&name=${encodeURIComponent(g.name ?? '')}&ip=${encodeURIComponent(ip ?? '')}`
                  : `/integrations/connect?platform=${wizPlatform}&asset_id=${g.id}`
                : null;
              const isSelf = g.is_self;

              return (
                <li key={g.id}
                  className={`flex items-center gap-3 px-4 py-3 transition ${isSelf ? 'bg-blue-50/40' : 'bg-white hover:bg-slate-50/60'}`}>
