...
const navigation: NavEntry[] = [
  ...
  },
  {
    ...
    items: [
      ...
      // ─── Plugin Automation / Risk Posture / Agents / Bulk Discovery
      // removed from the sidebar per product owner direction. The
      // compliance/scanning surface now lives UNDER IT Assets — open an
      // asset, jump to its Compliance tab, and you can see the AI-picked
      // benchmark, recent scan runs, and a Scan-now button without
      // bouncing between four sidebar items. The /compliance/plugins,
      // /risk-posture, and /admin/agents routes still resolve (for deep
      // links from other modules and from automated test scripts) but
      // are no longer first-class navigation targets. Bulk Discovery's
      // route was cut entirely — Compliverse is not a network scanner.
      // 'Plugin Library Import' removed from the sidebar — it duplicated
      // /compliance/plugins, which already exposes an 'Import CIS PDF'
      // tab. The /compliance/plugins/ingest route still works for deep
      // links and legacy bookmarks; it's just no longer surfaced here.
    ],
  },
  {
  ...
  // ─── IT Assets group ───────────────────────────────────────────────
  // Re-platformed per product owner direction: rather than scattering
  // the compliance-scanning surface across four top-level sidebar items
  // (Plugin Automation, Risk Posture, Agents, Bulk Discovery), they all
  // collapse into a single IT-Assets module — the same shape as
  // Vulnerability Mgmt above. The Inventory entry takes the original
  // /assets href so existing bookmarks keep working. Bulk Discovery is
  // gone for good (removed earlier this commit — Compliverse is not a
  // network scanner; assets arrive via the asset API from the bank's
  // CMDB or push agents). All four remaining children share the
  // 'compliance:frameworks:*' permission gate that the old top-level
  // entries used, so existing role grants don't break.
  {
    name: 'IT Assets',
    items: [
      // Per Hassan's consolidation: Plugin Automation + Rule Library = single
      // hierarchical page. AI Rule Classifier + OS Registry are admin-only
      // ops and live under Administration. Connections fold under Agents
      // (one screen, two tabs: endpoint agents + agentless credentials).
      { name: 'Inventory',           href: '/assets',                       icon: Bot,       requiredPermissions: ['dashboard:assets*'] },
      { name: 'Compliance Overview', href: '/compliance-overview',          icon: BarChart3, requiredPermissions: ['compliance:frameworks:*'] },
      { name: 'Compliance Rules',    href: '/compliance/plugins/library',   icon: Bot,       requiredPermissions: ['compliance:frameworks:*'] },
      { name: 'Risk Posture',        href: '/risk-posture',                 icon: BarChart3, requiredPermissions: ['compliance:frameworks:*'] },
      { name: 'Agents',              href: '/admin/agents',                 icon: Bot,       requiredPermissions: ['compliance:frameworks:*'] },
    ],
  },
...
export default function Sidebar() {
  ...
  const canAccessItem = (item: NavItem & { requiredModules?: string[]; adminOnly?: boolean }) => {
    // Hard gate — items flagged adminOnly stay off the menu for non-admins
    // (e.g. user-management, tenant settings). Everything else is visible
    // so a normal user still sees the same Compliverse layout the admin
    // sees; per-action gates inside each page take care of preventing
    // unauthorised actions (buttons render with a 🔒 + tooltip).
    if (item.adminOnly && !isAdmin) return false;
    return true;
  };

  // Same nav for everyone. Only adminOnly entries are dropped for non-admins.
  // The page-level RBAC still enforces what a normal user can DO.
  const filteredNavigation: NavEntry[] = loaded
    ? navigation.reduce<NavEntry[]>((acc, item) => {
        if (isGroup(item)) {
          if (item.adminOnly && !isAdmin) return acc;
          // Drop only the children explicitly marked adminOnly. Otherwise
          // keep them all so the layout is identical to the admin's view.
