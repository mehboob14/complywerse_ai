...
// ─── Onboarding banner shown on a fresh tenant ───────────────────────────────
function OnboardingBanner() {
  const steps = [
    {
      n: 1,
      title: 'Browse the CIS rule library',
      desc: '1,008 pre-loaded CIS checks for Windows, Linux, AWS — already approved for you.',
      cta: 'Open Plugin Library',
    ...
    },
    {
      ...
      desc: 'Add your first Windows / Linux / AWS connection so scans have a target.',
      cta: 'Add connection',
    ...
    },
    {
      ...
      desc: 'Pick a critical rule → Scan Now. PASS/FAIL evidence appears with cryptographic audit trail.',
      cta: 'Go to library',
      href: '/compliance/plugins?severity=critical',
  ...
  return (
    <div className="rounded-xl border border-blue-200 bg-gradient-to-br from-blue-50 to-white p-4 shadow-sm">
      <div className="flex items-start justify-between mb-3">
        <div>
          <div className="text-xs font-semibold uppercase tracking-wide text-blue-700">Welcome — Get Started</div>
          <div className="text-lg font-semibold text-gray-900">Run your first compliance scan in 3 steps</div>
          <p className="text-sm text-gray-600 mt-1">
            Your tenant already has 1,008 CIS Benchmark rules pre-loaded. Just connect a server and scan.
          </p>
        </div>
      </div>
        ...
        {steps.map((s) => (
          ...
          >
            <div className="flex items-center gap-2 mb-2">
              <span className={`inline-flex items-center justify-center w-7 h-7 rounded-full font-semibold text-sm border ${s.color}`}>
                {s.n}
              </span>
...
export default function MainDashboard() {
  ...
  return (
    ...
    >
      ...
      {/* ── Onboarding banner — only when no scans yet ─────────────────── */}
      {totalEvidence === 0 && totalMappedControls === 0 && (
        <OnboardingBanner />
      )}
