...
type Props = {
  ...
  saving: boolean;
  locked?: boolean;
  // Human-readable reason the workflow cannot currently be saved (e.g. invalid
  // trigger graph). When set, the Save button is disabled and shows the reason
  // in its tooltip. null/undefined means the graph is valid.
    ...
    <div className="flex items-center gap-2 px-3 py-2 bg-white border-b border-gray-200 h-12 shrink-0">
      ...
      {canCreate && (
        <button
          onClick={onNew}
          disabled={locked}
          className="flex items-center gap-1 text-xs px-2.5 py-1.5 rounded-md border border-gray-300 bg-white hover:bg-gray-50 text-gray-700 font-semibold transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
