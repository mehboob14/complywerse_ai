...
import { nodeTypes, humanizeTriggerEvent } from './components/CustomNodes';
...
type CatalogItem = NodeOptionItem & { functionality_name?: string; description?: string };
...
function normalizePlatformFunctionLabel(item: CatalogItem, moduleNameFallback?: string): string {
  ...
  if (label.includes(':')) {
    const aiMatch = label.match(/^(AI)\s*:\s*/i);
    const stripped = aiMatch ? label.slice(aiMatch[0].length) : label;
...
function buildPalette(catalog: CatalogResponse): PaletteItem[] {
  const defaults: PaletteItem[] = [
    ...Array.from(TRIGGER_KEYS).map((k) => {
      ...
      return {
        key: k,
        label: formatNodeLabel(k),
        description: PALETTE_DESCRIPTIONS[k] || '',
    ...
    ...Array.from(APPROVAL_KEYS).map((k) => ({ key: k, label: formatNodeLabel(k), description: PALETTE_DESCRIPTIONS[k] || '', group: 'approvals' as const })),
    ...Array.from(TIMER_KEYS).map((k) => ({ key: k, label: formatNodeLabel(k), description: PALETTE_DESCRIPTIONS[k] || '', group: 'timers' as const })),
    { key: 'subworkflow', label: 'Sub-Workflow', description: PALETTE_DESCRIPTIONS['subworkflow'] || '', group: 'control' },
    { key: 'end', label: 'End', description: PALETTE_DESCRIPTIONS['end'] || '', group: 'control' },
  ];
  ...
  for (const grp of flatGroups) {
    for (const item of grp.items) {
      // Actions: only allow keys explicitly in ACTION_KEYS — prevents stale backend
      // responses from re-introducing removed GRC domain action stubs.
      if (grp.group === 'actions' && !ACTION_KEYS.has(item.key)) {
        continue;
      }
      if (!seen.has(item.key)) {
        extras.push({ key: item.key, label: item.label, description: item.description || '', group: grp.group, module: item.module, submodule: item.submodule });
        seen.add(item.key);
      }
    }
  }
...
const HANDLE_ALIAS: Record<string, string> = {
  'condition-true': 'out_true',
  'condition-false': 'out_false',
};

// Top-down vertical auto-layout: BFS depth from start, siblings spread horizontally.
// Always overrides stored x/y so seeded horizontal graphs render vertically.
function layoutVertical(
  ...
  while (queue.length) {
    ...
    for (const next of out[cur] || []) {
      const cand = (depth[cur] ?? 0) + 1;
      if (depth[next] == null || depth[next] < cand) {
        depth[next] = cand;
        queue.push(next);
      }
    }
  ...
  // Anything still un-depthed (orphan): stack at the bottom.
  const maxKnown = Object.values(depth).reduce((a, b) => Math.max(a, b), 0);
  flowNodes.forEach((n) => {
    if (depth[n.id] == null) depth[n.id] = maxKnown + 1;
  });
  ...
  Object.entries(byDepth).forEach(([d, keys]) => {
    ...
    keys.forEach((k, i) => {
      const colOffset = (i - (count - 1) / 2) * COL_W;
      positions[k] = { x: CENTER_X + colOffset, y: TOP + dn * ROW_H };
    });
  });
...
function toFlowEdges(rawEdges: BackendEdge[]): Edge[] {
  return rawEdges.map((e, idx) => {
    const rawHandle = (e.condition?._handle as string) || (e.source_handle as string) || undefined;
    const sourceHandle = rawHandle ? (HANDLE_ALIAS[rawHandle] || rawHandle) : undefined;
    return {
...
function WorkflowEngineContent() {
  ...
  const triggerEventRef = useRef('manual.trigger');
  useEffect(() => { triggerEventRef.current = triggerEvent; }, [triggerEvent]);
  ...
  // ─── Trigger validation + node decoration ────────────────────────────────────
  // Re-runs whenever the graph changes. Drives the ⚡/⚠ overlays on the canvas,
  // the banner inside ConfigPanel, and the disabled state of the Save button.
  //
  // True empty drafts are REJECTED: a graph that contains only the Start
  // placeholder (and optionally the End terminal + a single Start→End edge)
  // produces no derivable trigger and would create a noise row in the
  // definitions table that can never run. The user must add at least one
  // real node before saving. This mirrors backend `_validate_workflow_graph`.
  // Note: dedicated trigger nodes from the palette DO count as real nodes
  // even though they serialize as nodeType==='start' with is_start=true.
  //
  // Defined HERE (above selectedNode) so the ConfigPanel sees the trigger
  // decoration on the selected node and so downstream effects/memos that read
  // selectedNode don't hit a temporal-dead-zone reference.
  const triggerValidation = useMemo<WorkflowGraphValidation>(() => {
    // The "has real work node" rule is now enforced inside
    // validateWorkflowGraph itself, so this memo is a thin adapter that
    // forwards the canvas state. Mirrors backend `_validate_workflow_graph`.
    return validateWorkflowGraph(
      nodes.map((n) => ({ id: n.id, data: { nodeType: n.data.nodeType, config: n.data.config, isStart: n.data.isStart } })),
  ...
  const decoratedNodes = useMemo(() => {
    ...
    const computedTrigger = triggerValidation.ok ? triggerValidation.computedTrigger : undefined;
    return nodes.map((n) => {
      if (n.id !== firstNodeId) return n;
      const status: 'valid' | 'invalid' = computedTrigger ? 'valid' : 'invalid';
      return {
        ...n,
        data: {
          ...n.data,
          ...
          inferredTriggerEvent: computedTrigger,
        },
      };
    });
  }, [nodes, edges, triggerValidation]);

  const selectedNode = useMemo(
    () => decoratedNodes.find((n) => n.id === selectedNodeId),
    [decoratedNodes, selectedNodeId],
  );
  ...
  useEffect(() => {
    ...
    const _flowEdges = toFlowEdges(rawEdges);
    const _flowNodes = layoutVertical(
      toFlowNodes(rawNodes, selectedDefinition.definition_json as Record<string, unknown>),
      _flowEdges,
    );
    setNodes(_flowNodes);
    setEdges(_flowEdges);
  ...
  }, [selectedDefinition, setNodes, setEdges, fitView]);
  ...
  const resetDraft = useCallback(() => {
    ...
    setSelectedNodeId(null);
    setSelectedEdgeId(null);
    // Start node with no trigger pre-selected — user sets it via the inspector
    const startConfig = enrichWorkflowNodeConfig('start', {}, 'start');
  ...
  const buildPayload = useCallback(() => {
    ...
    // Derive trigger_event from the first node connected after the Start placeholder.
    // Mirrors backend `_infer_trigger_event` so dedicated trigger nodes AND Platform
    // Function CRUD actions placed first after Start both yield a concrete event.
    let computedTriggerEvent = 'manual.trigger';
    const startEdge = edges.find((e) => e.source === 'start');
    if (startEdge) {
      const firstNode = nodes.find((n) => n.id === startEdge.target);
      const inferred = getTriggerEventForFirstNode(
        firstNode ? { nodeType: firstNode.data.nodeType, config: firstNode.data.config } : null,
      );
      if (inferred) computedTriggerEvent = inferred;
    }
    return {
  ...
  }, [serializeCanvas, triggerConditionsText, definitionJsonText, nodes, edges, name, description, isActive]);
  ...
  const saveDefinition = useCallback(async () => {
    ...
    if (!triggerValidation.ok) {
      toast({ title: 'Cannot save workflow', message: triggerValidation.error, type: 'error' });
      return;
    }
    setSaving(true);
    try {
  ...
  }, [buildPayload, selectedId, loadAll, toast, triggerValidation]);
  ...
  const generateFromNaturalLanguage = useCallback(async () => {
    ...
    try {
      ...
      if (data?.nodes && data?.edges) {
        ...
        setIsActive(true);
        const _ai_edges = toFlowEdges(rawEdges);
        setNodes(layoutVertical(toFlowNodes(rawNodes, {}), _ai_edges));
        setEdges(_ai_edges);
        setShowAI(false);
  ...
  const handleUseSuggestion = useCallback((suggestion: AISuggestion) => {
    ...
    if (rawNodes.length === 0) {
      ...
      if (cat.includes('risk')) {
        ...
        rawEdges = [
          ...
          { source_node_key: 'check_risk', target_node_key: 'escalate', condition: { _label: 'Critical / High', _handle: 'out_true' } },
          { source_node_key: 'check_risk', target_node_key: 'notify', condition: { _label: 'Medium / Low', _handle: 'out_false' } },
        ...
        rawEdges = [
          ...
          { source_node_key: 'check_severity', target_node_key: 'escalate', condition: { _label: 'Critical', _handle: 'out_true' } },
          { source_node_key: 'check_severity', target_node_key: 'assign_owner', condition: { _label: 'Normal', _handle: 'out_false' } },
        ...
        rawEdges = [
          ...
          { source_node_key: 'check_age', target_node_key: 'request_upload', condition: { _label: 'Expired / Expiring', _handle: 'out_true' } },
          { source_node_key: 'check_age', target_node_key: 'end', condition: { _label: 'Valid', _handle: 'out_false' } },
        ...
        rawEdges = [
          ...
          { source_node_key: 'check_approval', target_node_key: 'publish', condition: { _label: 'Approved', _handle: 'out_true' } },
          { source_node_key: 'check_approval', target_node_key: 'reject', condition: { _label: 'Rejected', _handle: 'out_false' } },
    ...
    setSelectedNodeId(null);
    setSelectedEdgeId(null);
    const _sug_edges = toFlowEdges(rawEdges);
    setNodes(layoutVertical(toFlowNodes(rawNodes, {}), _sug_edges));
    setEdges(_sug_edges);
      ...
      : d.trigger_event?.includes('risk')
        ...
        : d.trigger_event?.includes('policy')
          ...
          : d.trigger_event?.includes('asset')
            ? 'Asset Management'
            : 'Saved Workflows',
    description: d.description || `Trigger: ${humanizeTriggerEvent(d.trigger_event || 'manual.trigger')}`,
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-50 overflow-hidden">
              ...
              <tbody>
                {sortedDefinitions.map((d) => (
                  <tr key={d.id} className="border-t border-gray-100">
                    ...
                    <td className="px-4 py-2 text-gray-600">{humanizeTriggerEvent(d.trigger_event || '')}</td>
      ...
      <TopToolbar
        ...
        saveError={triggerValidation.ok ? null : triggerValidation.error}
        ...
        <div className="w-48 shrink-0 flex flex-col min-h-0">
          <NodePalette
            ...
            onAddNode={(item) => {
              ...
              setNodes((nds) => {
                // Smart positioning: chain to the right of the last node, or selected node
                const selectedNode = selectedNodeId ? nds.find((n) => n.id === selectedNodeId) : null;
                const anchorNode = selectedNode ?? (nds.length > 0 ? nds[nds.length - 1] : null);
                const position = anchorNode
                  ? { x: anchorNode.position.x, y: anchorNode.position.y + 150 }
                  : { x: 320, y: 200 };
              ...
              // Auto-connect: if a node was selected, draw an edge from it to the new node
              if (selectedNodeId) {
                const edgeId = `e_${selectedNodeId}_${nodeKey}`;
              ...
              // Select the new node in the config panel
