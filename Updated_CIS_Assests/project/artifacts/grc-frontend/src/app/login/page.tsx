...
function getTenantSlugFromHost(): string | null {
  ...
  if (host.endsWith('.replit.dev') || host.endsWith('.replit.app') || host.endsWith('.kirk.replit.dev')) return null;
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Replay the welcome animation on every login.
        try { localStorage.removeItem('intro_seen'); } catch { /* ignore */ }

        // Use a full page navigation so the route hits /welcome cleanly
        // (avoids React render races between client-side replace + reload).
  ...
  const handleMicrosoftLogin = async () => {
    ...
    try {
      ...
      setError(err.detail || 'Microsoft SSO is not configured. Please contact your administrator.');
    } catch {
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <button
            ...
            disabled={isLoading}
            className="flex w-full items-center justify-center gap-3 rounded-lg border border-slate-200 bg-white px-4 py-2.5 text-sm font-medium text-slate-700 hover:bg-slate-50 hover:border-slate-300 disabled:opacity-50 disabled:cursor-not-allowed transition-colors mb-5"
          >
