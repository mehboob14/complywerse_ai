...
export const useNodeCatalog = (filters?: Parameters<typeof workflowEngineApi.getNodeCatalog>[0]) => {
  return useQuery({
    ...
    queryFn: async () => {
      const res = await workflowEngineApi.getNodeCatalog(filters);
      return res.data;
    },
  });
};
...
export const useWorkflow = (id: number) => {
  return useQuery({
    ...
    queryFn: async () => {
      const res = await workflowEngineApi.getWorkflow(id);
      return res.data;
    },
    enabled: id > 0,
  });
...
export const useCreateWorkflow = () => {
  ...
  return useMutation({
    mutationFn: async (data: Parameters<typeof workflowEngineApi.createWorkflow>[0]) => {
      const res = await workflowEngineApi.createWorkflow(data);
      return res.data;
    },
    onSuccess: () => {
