...
const ConnectMethodPicker = lazy(() => import("@/pages/ConnectMethodPicker"));
...
// CompliancePluginsPage lazy-import removed — the flat rule-management
// page is no longer reachable as a tenant route. Per Hassan: one source
// of truth, not two pages of the same logic. The tenant-valuable bits
// (pass rate, Mine/Teammates activity, recent runs) moved to /my-runs.
// The page file stays on disk so platform-admin (Compliverse internal)
// can re-wire it under a guarded route later — only the tenant route is
// being dropped here.
// const CompliancePluginsPage = lazy(() => import("@/app/(dashboard)/compliance-plugins/page"));
const ComplianceOverviewPage = lazy(() => import("@/app/(dashboard)/compliance-overview/page"));
const RuleClassificationPage = lazy(() => import("@/app/(dashboard)/compliance-plugins/classify/page"));
const OsRegistryPage = lazy(() => import("@/app/(dashboard)/compliance-plugins/os-registry/page"));
const RuleLibraryPage = lazy(() => import("@/app/(dashboard)/compliance-plugins/library/page"));
const HowItWorksPage = lazy(() => import("@/app/(dashboard)/compliance-plugins/how-it-works/page"));
const ESGPreviewPage = lazy(() => import("@/app/(dashboard)/esg-preview/page"));
const CompliancePluginDetailPage = lazy(() => import("@/app/(dashboard)/compliance-plugins/[id]/page"));
// Ingest page lazy-import removed — tenants must never reach the
// benchmark upload UI per Hassan. The page file is preserved so the
// Compliverse platform team can re-wire it under a platform-admin
// route later, but no tenant-facing route renders it today.
...
const AdminOverviewPage = lazy(() => import("@/app/(dashboard)/admin/overview/page"));
const MyRunsPage = lazy(() => import("@/app/(dashboard)/my-runs/page"));
const ConnectionsAdminPage = lazy(() => import("@/app/(dashboard)/admin/connections/page"));
// CompliancePluginsRoute removed alongside CompliancePluginsPage.
    ...
    <DashboardLayout>
      <Suspense fallback={<PageLoader />}>
        <Switch>
          ...
          {/* /compliance/plugins/ingest route removed — see lazy-import comment above. */}
          ...
          <Route path="/my-runs" component={MyRunsPage} />
          <Route path="/admin/connections" component={ConnectionsAdminPage} />
          <Route path="/compliance/plugins/classify" component={withComplianceLayout(RuleClassificationPage)} />
          <Route path="/compliance/plugins/os-registry" component={withComplianceLayout(OsRegistryPage)} />
          <Route path="/compliance/plugins/library" component={withComplianceLayout(RuleLibraryPage)} />
          <Route path="/compliance/plugins/how-it-works" component={withComplianceLayout(HowItWorksPage)} />
          <Route path="/esg-preview" component={ESGPreviewPage} />
          <Route path="/compliance-overview" component={ComplianceOverviewPage} />
          <Route path="/compliance/plugins/:id" component={withComplianceLayout(CompliancePluginDetailPage)} />
          {/* /compliance/plugins (the flat rule-management page) is no
              longer routed for tenants — see the lazy-import comment up
              top for the rationale. /compliance/plugins/library (rules
              hierarchy), /compliance/plugins/:id (rule detail), and
              /compliance/plugins/asset/:id (per-asset compliance view)
              remain mounted because they're separate surfaces, not
              duplicates of the activity dashboard. */}
          ...
          {/* Integrations */}
          <Route path="/integrations/connect/method" component={ConnectMethodPicker} />
          ...
          {/* Admin → also expose the Connect Wizard and method picker here so
              a tenant admin can open them from the Admin menu. */}
          <Route path="/admin/integrations/connect/method" component={ConnectMethodPicker} />
