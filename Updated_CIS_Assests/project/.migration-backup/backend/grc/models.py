...
class AuditLog(Base):
    ...
    # Provenance flag — "user" for human-driven actions (default), "workflow" for
    # actions written by the workflow engine itself. The trigger dispatcher polls
    # for new audit logs to fire workflows; filtering on this column prevents
    # workflow-self-writes from triggering the same workflow recursively.
    actor_source = Column(String(20), nullable=True, index=True, default="user")
...
class ITAsset(Base):
    ...
    ip_address = Column(String(50), nullable=True)
    # ─── OS classification (added for benchmark-level scan filtering) ────
    # Without these columns, scan_all would run every Windows benchmark
    # against every Windows host — including Server 2022 rules against a
    # Windows 11 laptop — producing false positives and inflated risk
    # scores. Populated by a one-shot OS detect during Connect-Wizard
    # handshake / Bulk-Discovery import (WMI on Windows, /etc/os-release
    # on Linux, `show version` on Cisco) then used by the benchmark
    # matcher at scan time.
    #
    #   os_family     — broad class: windows / linux / cisco / aws /
    #                   oracle_db / macos. One per asset.
    #   os_version    — human-readable detected string for display.
    #   os_normalized — canonical match key the matcher compares against:
    #                   "windows-11", "ubuntu-22.04", "cisco-ios-xe-17".
    os_family = Column(String(40), nullable=True)
    os_version = Column(String(200), nullable=True)
    os_normalized = Column(String(80), nullable=True, index=True)
    # Block B/C — build-level OS detail. os_build captures the
    # Windows DisplayVersion (22H2, 23H2, 1909) or Cisco IOS XE minor
    # (17.9, 17.6). os_edition captures Pro/Enterprise/LTSC. Populated
    # from os_detector probe and from agent heartbeat OS profile.
    os_build = Column(String(40), nullable=True)
    os_edition = Column(String(80), nullable=True)
    # ── Host → Application children ("room and chair" model) ─────────
    # A Windows Server host can contain SQL Server / IIS / Tomcat child
    # assets. parent_asset_id links the chair to its room. asset_role
    # distinguishes hosts from the applications running on them.
    # detected_software_json holds the agent's raw software inventory so
    # the UI can offer "promote to child asset" for each detected app.
    parent_asset_id = Column(Integer, ForeignKey("grc_it_assets.id", ondelete="SET NULL"), nullable=True, index=True)
    asset_role = Column(String(20), default="host")  # host | application | service
    detected_software_json = Column(JSON, nullable=True)
    ...
    # ── Risk Posture upgrade phase 1: business-context columns. Drive
    # the 5th dimension of effective vuln risk, w5 × (business_impact - 1.0).
    # All human-owned (operator sets via Business Context panel on the
    # asset detail page). NEVER touched by AI or by any scanner. See
    # docs/RISK_POSTURE_PLAN.md §3 for multiplier table.
    is_customer_facing = Column(Boolean, default=False, nullable=False)
    is_internet_facing = Column(Boolean, default=False, nullable=False)
    regulated_data_type = Column(
        # enum stored as string for migration flexibility; values are
        # 'none' | 'pii' | 'pci' | 'phi' | 'financial'. Default 'none' so
        # existing rows contribute zero to the business-impact multiplier
        # until the operator sets them.
        String(20), default="none", nullable=False,
    )
    operational_dependency = Column(
        # 'low' | 'medium' | 'high' | 'critical'. Default 'medium' so the
        # multiplier is exactly 1.0 (neutral) — matches the "default state
        # doesn't bias the score" rule in §2.
        String(20), default="medium", nullable=False,
    )
    business_impact_notes = Column(
        # Free-text justification — required surface for audit trail.
        # Why we marked PCI scope, why operational_dependency=critical, etc.
        Text, nullable=True,
    )
    created_at = Column(DateTime, default=datetime.utcnow)
...
class PolicyGapFinding(Base):
    ...
    critical_rules_fired = Column(JSON, default=list)
    critical_decision = Column(String(20), nullable=True)
    critical_decision_by = Column(Integer, ForeignKey("grc_users.id"), nullable=True)
    critical_decision_at = Column(DateTime, nullable=True)
    critical_justification = Column(Text, nullable=True)
    ...
    critical_decision_user = relationship("GRCUser", foreign_keys=[critical_decision_by])
...
# =============================================================================
# Policy AI HITL: clause coverage, patch proposals, critical rules, email tokens
# (added by Task #46 — see .local/tasks/task-46.md)
# =============================================================================

class PolicyClauseCoverage(Base):
    """Per-(policy, framework-clause) coverage record produced by the hybrid mapping service.

    Used both for (a) the dedup pre-pass during AI policy generation, and
    (b) persisting user manual mapping overrides as ground truth."""
    __tablename__ = "grc_policy_clause_coverage"
    ...
    coverage_status = Column(String(20), default="missing", index=True)
    confidence = Column(Float, nullable=True)
    # Composite reasons: e.g. {"embedding": 0.82, "control_id_keyword": True, "ai": 0.75}
    signals = Column(JSON, default=dict)
    matching_excerpt = Column(Text, nullable=True)
    source = Column(String(20), default="auto")  # auto | manual
    user_choice = Column(String(30), nullable=True)  # skip_link | merge | gap_patch | sxs | regenerate | None
    user_choice_by = Column(Integer, ForeignKey("grc_users.id"), nullable=True)
    user_choice_at = Column(DateTime, nullable=True)
    is_locked = Column(Boolean, default=False)  # ground-truth overrides
    ...
    document = relationship("GovernanceDocument")
    parsed_control = relationship("ParsedFrameworkControl")
    user_choice_user = relationship("GRCUser", foreign_keys=[user_choice_by])

    __table_args__ = (
        UniqueConstraint("document_id", "parsed_control_id", name="uq_clause_cov_doc_ctrl"),
        Index("ix_clause_cov_tenant_fw", "tenant_id", "uploaded_framework_id"),
        Index("ix_clause_cov_status", "tenant_id", "coverage_status"),
    )


class PolicyPatchProposal(Base):
    """An AI-drafted patch proposing missing clause text for a policy. HITL-approved."""
    __tablename__ = "grc_policy_patch_proposals"

    id = Column(Integer, primary_key=True, index=True)
    tenant_id = Column(Integer, ForeignKey("grc_tenants.id"), nullable=False, index=True)
    document_id = Column(Integer, ForeignKey("grc_governance_documents.id"), nullable=False, index=True)
    parsed_control_id = Column(Integer, ForeignKey("grc_parsed_framework_controls.id"), nullable=True, index=True)
    finding_id = Column(Integer, ForeignKey("grc_policy_gap_findings.id"), nullable=True, index=True)

    clause_reference = Column(String(255), nullable=True)
    clause_title = Column(String(500), nullable=True)
    draft_text = Column(Text, nullable=False)
    diff_against_version = Column(Integer, nullable=True)
    rationale = Column(Text, nullable=True)

    # pending_approval | approved | rejected | superseded
    status = Column(String(30), default="pending_approval", index=True)

    # Approval chain configuration (snapshot at create-time): list of role/user steps.
    # e.g. [{"step": 1, "role": "Owner"}, {"step": 2, "role": "Compliance Lead"}]
    approver_chain = Column(JSON, default=list)
    current_step = Column(Integer, default=1)
    approval_history = Column(JSON, default=list)  # per-step: {step, decision, by, at, comment}

    drafted_by_user_id = Column(Integer, ForeignKey("grc_users.id"), nullable=True)
    decided_by = Column(Integer, ForeignKey("grc_users.id"), nullable=True)
    decided_at = Column(DateTime, nullable=True)
    decision_comment = Column(Text, nullable=True)
    edited_text = Column(Text, nullable=True)  # if approver chose Edit-then-Accept

    is_blocked_by_critical = Column(Boolean, default=False)
    ...
    document = relationship("GovernanceDocument")
    parsed_control = relationship("ParsedFrameworkControl")
    finding = relationship("PolicyGapFinding", foreign_keys=[finding_id])
    drafted_by = relationship("GRCUser", foreign_keys=[drafted_by_user_id])
    decided_by_user = relationship("GRCUser", foreign_keys=[decided_by])

    __table_args__ = (
        Index("ix_patch_tenant_status", "tenant_id", "status"),
        Index("ix_patch_doc_status", "document_id", "status"),
    )


class FrameworkCriticalRule(Base):
    """Per-tenant, per-framework toggleable rule that, when fired, marks a finding critical."""
    __tablename__ = "grc_framework_critical_rules"

    id = Column(Integer, primary_key=True, index=True)
    tenant_id = Column(Integer, ForeignKey("grc_tenants.id"), nullable=False, index=True)
    uploaded_framework_id = Column(Integer, ForeignKey("grc_uploaded_frameworks.id"), nullable=True, index=True)

    # mandatory_clause | high_severity | regulatory_breach
    rule_type = Column(String(50), nullable=False)
    enabled = Column(Boolean, default=True)
    params = Column(JSON, default=dict)

    # Approver chain config (used as default for patch proposals against findings under this framework)
    approver_chain = Column(JSON, default=list)
    ...
    __table_args__ = (
        UniqueConstraint("tenant_id", "uploaded_framework_id", "rule_type", name="uq_critrule_tenant_fw_type"),
    )


class ApprovalEmailToken(Base):
    """Single-use HMAC-signed approval link tokens (7-day expiry)."""
    __tablename__ = "grc_approval_email_tokens"

    id = Column(Integer, primary_key=True, index=True)
    tenant_id = Column(Integer, ForeignKey("grc_tenants.id"), nullable=False, index=True)
    proposal_id = Column(Integer, ForeignKey("grc_policy_patch_proposals.id"), nullable=False, index=True)
    approver_user_id = Column(Integer, ForeignKey("grc_users.id"), nullable=False, index=True)
    ...
    proposal = relationship("PolicyPatchProposal")
    approver = relationship("GRCUser")
...
class Vulnerability(Base):
    ...
    # ── Risk Posture upgrade phase 2: deterministic-feed signals + the
    # computed effective risk. Everything here is SYSTEM-owned: nightly
    # fetchers (FIRST.org EPSS, CISA KEV) write the inputs, the formula
    # module writes effective_risk_*. No AI in the path. See plan §4.
    epss_score = Column(Float, nullable=True)              # 0..1 from FIRST.org
    epss_percentile = Column(Float, nullable=True)         # 0..100 from FIRST.org
    epss_updated_at = Column(DateTime, nullable=True)      # when fetcher last ran for this CVE

    kev_flag = Column(Boolean, default=False, nullable=False)  # CISA KEV listed?
    kev_due_date = Column(DateTime, nullable=True)         # CISA federal remediation deadline
    kev_added_at = Column(DateTime, nullable=True)         # when CISA flagged this CVE

    exploit_maturity = Column(
        # 'none' | 'poc' | 'functional' | 'weaponized' from NVD CVSS
        # temporal vector if scanner didn't supply it. Nullable while
        # we backfill — formula treats null as 'none'.
        String(20), nullable=True,
    )

    effective_risk_score = Column(Float, nullable=True)    # 0..1 — combined output of the formula
    effective_risk_reason = Column(Text, nullable=True)    # explainable English breakdown (audit)
    effective_risk_computed_at = Column(DateTime, nullable=True)
...
class IntegrationConnection(Base):
    ...
    created_at = Column(DateTime, default=datetime.utcnow)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
    # ─── Collector-mode routing ───────────────────────────────────────
    # When set, scans for this connection route through the named
    # collector agent INSIDE the bank's LAN (the collector dials out to
    # us, we ship it the scan job + decrypted creds, it opens WinRM/SSH
    # to the target). When NULL, the legacy direct-from-backend path is
    # used (which only works when backend is inside the same network).
    assigned_collector_agent_id = Column(
        Integer,
        ForeignKey("grc_compliance_agents.id", ondelete="SET NULL"),
        nullable=True,
        index=True,
    )

    # ─── Scope (Block A) ──────────────────────────────────────────────
    # Defines which assets this stored credential applies to. Default
    # 'tenant_all' means every asset in tenant whose protocol matches.
    # 'asset_list' uses scope_value.asset_ids[]; 'asset_tag' matches any
    # of scope_value.tags[]; 'ip_range' matches assets whose ip falls
    # in any CIDR in scope_value.cidrs[]. The scan executor calls
    # services.scope.resolve_assets(conn, tenant_assets) to materialise.
    scope_mode = Column(String(20), default="tenant_all", nullable=False)
    scope_value = Column(JSON, default=dict)
    scope_updated_at = Column(DateTime, nullable=True)
    last_scope_resolution_count = Column(Integer, nullable=True)

    # ─── Multi-field credentials (Azure SP, K8s kubeconfig, LDAP bind DN) ─
    # The legacy `username`/`password` pair fits 1:1 cred shapes (WinRM,
    # SSH, single-value API keys). Newer integrations need richer payloads:
    # Azure needs (subscription_id, tenant_id, client_id, client_secret);
    # K8s needs (kubeconfig YAML | server+token+ca); LDAP needs bind DN as
    # distinct from a unix-style username. Encrypted blob lives here.
    credentials_extra_json = Column(JSON, nullable=True)
...
class ComplianceAgent(Base):
    ...
    instead of being pulled. Used for Phase 3 agent-mode scanning.

    Two operating modes:
      - collector: one agent per LAN, dials out, scans neighbors via
                   stored creds (Cywift-style, Option 1 from senior).
      - endpoint:  one agent per host, scans only itself (Option B
                   per-endpoint deployment via GPO/SCCM/Intune).
    """
    ...
    # One-time enrollment token (sha256 hash; raw token only shown once)
    enrollment_token_hash = Column(String(128), nullable=True, index=True)
    # Long-lived API token (sha256 hash) once enrollment completes
...
def _add_missing_columns():
    ...
    # CIS PDF ingest columns on grc_compliance_plugins (added 2026-05-07)
    try:
        if inspector.has_table('grc_compliance_plugins'):
            cp_cols = {col['name'] for col in inspector.get_columns('grc_compliance_plugins')}
            is_pg = not DATABASE_URL.startswith('sqlite')
            jsonb = 'JSONB' if is_pg else 'JSON'
            jsonb_default = " DEFAULT '[]'::jsonb" if is_pg else ""
            additions = [
                ('parent_plugin_id', f'INTEGER{ " REFERENCES grc_compliance_plugins(id)" if is_pg else ""}'),
                ('depth', 'INTEGER DEFAULT 0'),
                ('section_path', 'VARCHAR(500)'),
                ('level', 'VARCHAR(20)'),
                ('assessment_status', 'VARCHAR(20)'),
                ('audit_steps_text', 'TEXT'),
                ('references_json', f'{jsonb}{jsonb_default}'),
                ('cis_controls_json', f'{jsonb}{jsonb_default}'),
                ('mitre_techniques_json', f'{jsonb}{jsonb_default}'),
                ('confidence_score', 'FLOAT'),
                ('review_status', "VARCHAR(30) DEFAULT 'auto_approved'"),
                ('auto_generated_check', 'BOOLEAN DEFAULT FALSE'),
                ('source_ingest_job_id', 'INTEGER'),
            ]
            with engine.begin() as conn:
                for col, ddl in additions:
                    if col not in cp_cols:
                        logger.warning(f"Adding missing '{col}' column to grc_compliance_plugins...")
                        conn.execute(text(f"ALTER TABLE grc_compliance_plugins ADD COLUMN {col} {ddl}"))
    except Exception as e:
        logger.error(f"Error adding CIS ingest columns: {e}")

    # CIS PDF ingest counters on grc_cis_ingest_jobs (added 2026-05-11)
    # rules_toc_rejected surfaces how many headings the splitter dropped as
    # TOC bleed / bare section headers / quoted-value-only — see task #69.
    try:
        if inspector.has_table('grc_cis_ingest_jobs'):
            ij_cols = {col['name'] for col in inspector.get_columns('grc_cis_ingest_jobs')}
            ij_additions = [
                ('rules_toc_rejected', 'INTEGER DEFAULT 0'),
                # Bytes of the source PDF — kept so reviewers can hit
                # "Re-parse" after a parser fix without re-uploading.
                ('pdf_bytes', 'BYTEA'),
            ]
            with engine.begin() as conn:
                for col, ddl in ij_additions:
                    if col not in ij_cols:
                        logger.warning(f"Adding missing '{col}' column to grc_cis_ingest_jobs...")
                        conn.execute(text(f"ALTER TABLE grc_cis_ingest_jobs ADD COLUMN {col} {ddl}"))
    except Exception as e:
        logger.error(f"Error adding CIS ingest job columns: {e}")
    ...
    # Strict-single-stage matcher table — seed global defaults if empty.
    # These are derived from the benchmarks present in the library + the
    # asset OS keys we typically see. Operator can override per-tenant.
    try:
        if inspector.has_table('grc_benchmark_os_mappings'):
            with engine.begin() as conn:
                row = conn.execute(text("SELECT count(*) FROM grc_benchmark_os_mappings WHERE tenant_id IS NULL")).scalar()
                if row == 0:
                    # Pull benchmarks that have approved rules to make sure
                    # we don't seed defaults pointing at empty benchmarks.
                    seed_pairs = [
                        # (os_pattern, benchmark)
                        ('windows-11',     'CIS_Microsoft_Windows_11_Enterprise_Benchmark_v5.0.1'),
                        ('windows-10',     'CIS_Microsoft_Windows_10_Enterprise_Benchmark_v3.0.0'),
                        ('windows-server-2022', 'CIS_Microsoft_Windows_Server_2022_Benchmark_v3.0.0'),
                        ('windows-server-2019', 'CIS_Microsoft_Windows_Server_2019_Benchmark_v3.0.0'),
                        ('windows-server-2016', 'CIS_Microsoft_Windows_Server_2016_Benchmark_v3.0.0'),
                        ('ubuntu-24.04',   'CIS_Ubuntu_Linux_24.04_LTS_Benchmark_v1.0.0'),
                        ('ubuntu-22.04',   'CIS_Ubuntu_Linux_22.04_LTS_Benchmark_v3.0.0'),
                        ('debian-12',      'CIS_Debian_Linux_12_Benchmark_v1.1.0'),
                        ('almalinux-9',    'CIS_AlmaLinux_OS_9_Benchmark_v2.0.0'),
                        ('oraclelinux-9',  'CIS_Oracle_Linux_9_Benchmark_v2.0.0'),
                        ('amazonlinux-2023','CIS_Amazon_Linux_2023_Benchmark_v1.0.0'),
                        ('cisco-ios-xe',   'CIS_Cisco_IOS_XE_17.x_Benchmark_v2.2.1'),
                        ('cisco-nx-os',    'CIS_Cisco_NX-OS_Benchmark_v1.2.0'),
                        ('cisco-asa',      'CIS_Cisco_ASA_9.x_Firewall_Benchmark_v1.1.0'),
                        ('oracle-db-19c',  'CIS_Oracle_Database_19c_Benchmark_v2.0.0'),
                        ('oracle-db-23ai', 'CIS_Oracle_Database_23ai_Benchmark_v1.1.0'),
                    ]
                    for pattern, bench in seed_pairs:
                        # Only seed if the benchmark actually exists in the library.
                        ok = conn.execute(text(
                            "SELECT EXISTS(SELECT 1 FROM grc_compliance_plugins WHERE benchmark = :b)"
                        ), {"b": bench}).scalar()
                        if ok:
                            conn.execute(text("""
                                INSERT INTO grc_benchmark_os_mappings (tenant_id, os_pattern, benchmark_name, is_active, priority, notes, created_at, updated_at)
                                VALUES (NULL, :p, :b, true, 100, 'seeded default', now(), now())
                                ON CONFLICT DO NOTHING
                            """), {"p": pattern, "b": bench})
                    logger.info("grc_benchmark_os_mappings: seeded global defaults")
    except Exception as exc:
        logger.error(f"Error seeding grc_benchmark_os_mappings: {exc}")

    # Quarantine flag on grc_compliance_plugin_runs for runs from before
    # the Stage 2 fix (rules from wrong benchmark). UI filters them out.
    try:
        if inspector.has_table('grc_compliance_plugin_runs'):
            pr_cols = {col['name'] for col in inspector.get_columns('grc_compliance_plugin_runs')}
            with engine.begin() as conn:
                if 'is_leaked' not in pr_cols:
                    conn.execute(text("ALTER TABLE grc_compliance_plugin_runs ADD COLUMN is_leaked BOOLEAN DEFAULT FALSE NOT NULL"))
                if 'leaked_reason' not in pr_cols:
                    conn.execute(text("ALTER TABLE grc_compliance_plugin_runs ADD COLUMN leaked_reason TEXT"))
    except Exception as exc:
        logger.error(f"Error adding is_leaked to grc_compliance_plugin_runs: {exc}")

    # Multi-field credentials blob on grc_integration_connections.
    # Required for Azure (4 IDs), K8s (kubeconfig), LDAP (bind DN distinct
    # from username), and any future integration whose creds don't fit the
    # legacy username+password pair.
    try:
        if inspector.has_table('grc_integration_connections'):
            ic_cols = {col['name'] for col in inspector.get_columns('grc_integration_connections')}
            with engine.begin() as conn:
                if 'credentials_extra_json' not in ic_cols:
                    conn.execute(text("ALTER TABLE grc_integration_connections ADD COLUMN credentials_extra_json JSON"))
    except Exception as exc:
        logger.error(f"Error adding credentials_extra_json to grc_integration_connections: {exc}")

    # Fleet enrollment columns on grc_compliance_agents (added 2026-06-04).
    # Lets one .cmd installer enroll N hosts under a single revocable
    # parent token, instead of forcing the operator to download N files.
    try:
        if inspector.has_table('grc_compliance_agents'):
            ca_cols = {col['name'] for col in inspector.get_columns('grc_compliance_agents')}
            with engine.begin() as conn:
                if 'kind' not in ca_cols:
                    conn.execute(text("ALTER TABLE grc_compliance_agents ADD COLUMN kind VARCHAR(20) DEFAULT 'single' NOT NULL"))
                if 'enrollment_max_uses' not in ca_cols:
                    conn.execute(text("ALTER TABLE grc_compliance_agents ADD COLUMN enrollment_max_uses INTEGER"))
                if 'enrollment_uses' not in ca_cols:
                    conn.execute(text("ALTER TABLE grc_compliance_agents ADD COLUMN enrollment_uses INTEGER DEFAULT 0 NOT NULL"))
                if 'enrollment_expires_at' not in ca_cols:
                    conn.execute(text("ALTER TABLE grc_compliance_agents ADD COLUMN enrollment_expires_at TIMESTAMP"))
                if 'spawned_from_agent_id' not in ca_cols:
                    conn.execute(text("ALTER TABLE grc_compliance_agents ADD COLUMN spawned_from_agent_id INTEGER REFERENCES grc_compliance_agents(id)"))
                if 'pending_scan_at' not in ca_cols:
                    conn.execute(text("ALTER TABLE grc_compliance_agents ADD COLUMN pending_scan_at TIMESTAMP"))
                if 'pending_scan_user_id' not in ca_cols:
                    conn.execute(text("ALTER TABLE grc_compliance_agents ADD COLUMN pending_scan_user_id INTEGER REFERENCES grc_users(id)"))
    except Exception as exc:
        logger.error(f"Error adding fleet-enrollment columns to grc_compliance_agents: {exc}")
    ...
    # Ensure grc_policy_gap_findings has Task #46 critical-decision columns.
    try:
        gf_columns = {col['name'] for col in inspector.get_columns('grc_policy_gap_findings')}
        with engine.begin() as conn:
            if 'is_critical' not in gf_columns:
                conn.execute(text("ALTER TABLE grc_policy_gap_findings ADD COLUMN is_critical BOOLEAN DEFAULT FALSE"))
            if 'critical_rules_fired' not in gf_columns:
                if DATABASE_URL.startswith('sqlite'):
                    conn.execute(text("ALTER TABLE grc_policy_gap_findings ADD COLUMN critical_rules_fired TEXT"))
                else:
                    conn.execute(text("ALTER TABLE grc_policy_gap_findings ADD COLUMN critical_rules_fired JSON"))
            if 'critical_decision' not in gf_columns:
                conn.execute(text("ALTER TABLE grc_policy_gap_findings ADD COLUMN critical_decision VARCHAR(20)"))
            if 'critical_decision_by' not in gf_columns:
                conn.execute(text("ALTER TABLE grc_policy_gap_findings ADD COLUMN critical_decision_by INTEGER"))
            if 'critical_decision_at' not in gf_columns:
                conn.execute(text("ALTER TABLE grc_policy_gap_findings ADD COLUMN critical_decision_at TIMESTAMP"))
            if 'critical_justification' not in gf_columns:
                conn.execute(text("ALTER TABLE grc_policy_gap_findings ADD COLUMN critical_justification TEXT"))
    except Exception as e:
        if "does not exist" in str(e).lower() or "no such table" in str(e).lower():
            logger.debug("grc_policy_gap_findings table not found - will be created")
        else:
            logger.error(f"Error checking/adding gap-finding critical columns: {e}")

    # Ensure grc_audit_logs has actor_source for workflow-loop prevention.
    # See AuditLog model docstring; filtering on this column lets the trigger
    # dispatcher ignore audit rows it wrote itself.
    try:
        audit_columns = {col['name'] for col in inspector.get_columns('grc_audit_logs')}
        if 'actor_source' not in audit_columns:
            logger.warning("Adding missing 'actor_source' column to grc_audit_logs...")
            with engine.begin() as conn:
                conn.execute(text("ALTER TABLE grc_audit_logs ADD COLUMN actor_source VARCHAR(20)"))
                # Backfill existing rows so the column is meaningful for filters.
                conn.execute(text("UPDATE grc_audit_logs SET actor_source = 'user' WHERE actor_source IS NULL"))
                # Index speeds up the dispatcher's "exclude workflow self-writes" filter.
                if DATABASE_URL.startswith('sqlite'):
                    conn.execute(text("CREATE INDEX IF NOT EXISTS ix_grc_audit_logs_actor_source ON grc_audit_logs(actor_source)"))
                else:
                    conn.execute(text("CREATE INDEX IF NOT EXISTS ix_grc_audit_logs_actor_source ON grc_audit_logs(actor_source)"))
            logger.info("✓ Successfully added actor_source column to grc_audit_logs")
    except Exception as e:
        if "does not exist" in str(e).lower() or "no such table" in str(e).lower():
            logger.debug("grc_audit_logs table not found - will be created")
        else:
            logger.error(f"Error checking/adding grc_audit_logs.actor_source: {e}")


# =============================================================================
# CIS Benchmark Plugin Engine (Task #55)
# =============================================================================
...
class CompliancePlugin(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    tenant_id = Column(Integer, ForeignKey("grc_tenants.id"), nullable=True, index=True)
    plugin_key = Column(String(200), nullable=False, index=True)  # e.g. CIS_AWS_FOUNDATIONS_v3.0__1.4
    benchmark = Column(String(100), nullable=False, index=True)   # CIS_AWS_FOUNDATIONS_v3.0, CIS_UBUNTU_22_04_v2.0
    rule_id = Column(String(50), nullable=False)                  # e.g. "1.4"
    title = Column(String(500), nullable=False)
    description = Column(Text, nullable=True)
    rationale = Column(Text, nullable=True)
    remediation = Column(Text, nullable=True)
    severity = Column(String(20), default="medium")  # low, medium, high, critical
    runner_type = Column(String(30), nullable=False)  # aws_readonly, linux_ssh
    check_definition = Column(JSON, nullable=False, default={})   # runner-specific check spec
    enabled = Column(Boolean, default=True)
    is_builtin = Column(Boolean, default=True)
    source_url = Column(String(500), nullable=True)
    schedule_cron = Column(String(50), nullable=True)  # e.g. "daily", "weekly", "monthly", or future: cron expression
    # ── PDF-ingest provenance & hierarchy (added in CIS PDF Ingestion Pipeline) ──
    parent_plugin_id = Column(Integer, ForeignKey("grc_compliance_plugins.id"), nullable=True, index=True)
    depth = Column(Integer, default=0)            # 0 = top-level section
    section_path = Column(String(500), nullable=True)  # human breadcrumb e.g. "1 Identity > 1.1 IAM"
    level = Column(String(20), nullable=True)     # CIS profile applicability: Level 1 / Level 2 / L1 - Server …
    assessment_status = Column(String(20), nullable=True)  # Automated | Manual
    audit_steps_text = Column(Text, nullable=True)         # verbatim Audit section
    ...
    confidence_score = Column(Float, nullable=True)        # 0..1 — how confident the parser is
    review_status = Column(String(30), default="auto_approved")  # auto_approved | pending_review | approved | rejected
    auto_generated_check = Column(Boolean, default=False)  # check_definition was synthesised, not hand-written
...
class CisIngestJob(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    tenant_id = Column(Integer, ForeignKey("grc_tenants.id"), nullable=True, index=True)  # NULL = ingested into shared catalog
    uploaded_by = Column(Integer, ForeignKey("grc_users.id"), nullable=True)
    original_filename = Column(String(500), nullable=False)
    sha256 = Column(String(64), nullable=False, index=True)
    benchmark_label = Column(String(200), nullable=True)   # parsed/inferred (e.g. "CIS_AWS_FOUNDATIONS_v3.0")
    ...
    # TOC dot-leaders, bare section headings, quoted-value-only headings — all
    # the false positives the rule-splitter rejected before classification.
    rules_toc_rejected = Column(Integer, default=0)
    ocr_pages = Column(Integer, default=0)
    error_text = Column(Text, nullable=True)
    extraction_log = Column(JSON, nullable=True, default=list)  # per-stage breadcrumbs
    started_at = Column(DateTime, nullable=True)
    completed_at = Column(DateTime, nullable=True)
    created_at = Column(DateTime, default=datetime.utcnow)
    # Original PDF bytes — kept so reviewers can re-parse a job after we
    # ship a parser fix (or tweak rejection gates) without forcing them
    # to re-upload the same multi-MB benchmark. Capped at 50 MB by the
    # upload route.
...
class PluginScheduleOverride(Base):
    ...
    Built-in (tenant_id=NULL) plugins are shared across all tenants and
    cannot be edited by a single tenant — but tenants still need to set
    their own scan cadence. This table holds the per-tenant overrides;
    the effective cadence is `override.schedule_cron` if a row exists,
    otherwise `plugin.schedule_cron` (catalog default).
    """
    ...
    tenant_id = Column(Integer, ForeignKey("grc_tenants.id"), nullable=False, index=True)
    plugin_id = Column(Integer, ForeignKey("grc_compliance_plugins.id"), nullable=False, index=True)
    schedule_cron = Column(String(50), nullable=True)  # nullable = explicitly disabled (manual only)
    created_at = Column(DateTime, default=datetime.utcnow)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
...
class PluginAssetScope(Base):
    ...
    `mode` is one of:
      - 'all'     → run against every eligible target (default if no row).
      - 'include' → run only against assets in `asset_ids`.
      - 'exclude' → run against every eligible asset *except* those in `asset_ids`.
    """
    ...
    tenant_id = Column(Integer, ForeignKey("grc_tenants.id"), nullable=False, index=True)
    plugin_id = Column(Integer, ForeignKey("grc_compliance_plugins.id"), nullable=False, index=True)
    mode = Column(String(20), nullable=False, default="all")  # all|include|exclude
...
class CompliancePluginRun(Base):
    ...
    status = Column(String(30), nullable=False, default="pending")  # pending, running, passed, failed, error, skipped
    result_summary = Column(Text, nullable=True)
    raw_output = Column(JSON, nullable=True)              # full structured response
    result_detail = Column(Text, nullable=True)            # human-readable PASS/FAIL reasoning
    remediation_shown = Column(Text, nullable=True)        # snapshot of plugin.remediation at execution time
    evidence_snapshot = Column(JSON, nullable=True)        # canonical, immutable evidence payload (basis of evidence_hash)
    evidence_hash = Column(String(64), nullable=True, index=True)  # sha256 of evidence_snapshot
    duration_ms = Column(Integer, nullable=True)
    triggered_by = Column(String(30), default="manual")   # manual, scheduled, workflow, scan_all
    triggered_by_user_id = Column(Integer, ForeignKey("grc_users.id"), nullable=True)
    # ─── Collector-mode routing ────────────────────────────────────────
    # Which agent actually executed this run.
    #   NULL                    → backend executed directly (legacy /
    #                              endpoint-agent local execution)
    #   collector agent.id     → collector inside customer's LAN
    # Used by /agents/jobs to filter "give me jobs assigned to me" and
    # by the run-history UI to show "Scanned via collector #N".
    executed_by_agent_id = Column(
        Integer,
        ForeignKey("grc_compliance_agents.id", ondelete="SET NULL"),
        nullable=True,
        index=True,
    ...
    completed_at = Column(DateTime, nullable=True)
    error_message = Column(Text, nullable=True)
    # ─── Quarantine flag for runs from before the Stage 2 fix ───────────
    # Set to True for runs whose plugin's benchmark doesn't match the
    # Stage 2 pick for the asset's CURRENT os_normalized. UI queries
    # filter is_leaked=false by default so these don't poison dashboards,
    # but the rows stay in the table for forensic queries.
    is_leaked = Column(Boolean, nullable=False, server_default="false", default=False, index=True)
    leaked_reason = Column(Text, nullable=True)
...
class BenchmarkOsMapping(Base):
    """Strict-single-stage matcher map.
...
class TenantRiskWeights(Base):
    ...
    Senior's ask: "har bank apne hisaab se weights tune kare." A row in
    this table changes how CIS / vulnerability / CIA / control / linked-
    risk dimensions are blended for that tenant's Risk Posture screen.
    No row → fall back to the hard-coded defaults in risk_posture/service.py.

    Each row must sum to ~100% (DB-side CHECK constraint enforces this with
    ±1% tolerance to absorb floating-point rounding).
    """
    ...
    preset_name = Column(String(40), nullable=True)   # "Banking" | "SaaS-heavy" | "Custom"
    updated_at  = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
...
def init_grc_db():
    ...
    except:  # noqa: E722 - catch-all needed for fresh DB startup
        logger.warning("seed_uploaded_frameworks skipped on fresh DB (will succeed after first user registers)")
