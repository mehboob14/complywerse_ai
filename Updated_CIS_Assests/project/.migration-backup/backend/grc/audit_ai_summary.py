...
_SYSTEM_PROMPT = (
    ...
    "what it contains in plain English: list items, count, flow, and so on.\n"
    "3. NEVER use em dashes ('—') or en dashes ('–') anywhere in the "
    "output. Use commas, periods, parentheses, colons, or the word 'and' instead. "
    "Plain hyphens inside compound words (in-app, sub-workflow, real-time) are fine.\n"
    "4. Output format is GitHub-flavored markdown. The FIRST line is a single bold "
    "TL;DR sentence (12-20 words) written conversationally for a non-technical "
    "reader, e.g. '**Mehboob could not revoke agent #48 because the account "
    "lacks permission.**' or '**A CIS scan flagged that SMB signing is off on a "
    "Windows server.**'. The TL;DR should explain WHAT happened in plain English "
    "and WHY if a failure reason is present. Do not start it with 'TL;DR:'. Just "
    "the bold sentence on its own line. Follow it with a blank line.\n"
    "5. AFTER the TL;DR, output a level-3 heading restating the action: "
    "'### <Actor> <verb> <resource>'.\n"
    "6. Use these sub-sections, each as a '#### <name>' heading, only when the data "
    ...
    "   - '#### Steps': numbered list of workflow nodes. PREFER the top-level "
    "`workflow_steps` array (each entry is a step's persisted node_meta with "
    "node_key, node_type, status, label, description, action_name, endpoint, "
    "verb, module, submodule, inherited_from). For each entry render: "
    "'**<label or node_key>** *(<node_type>, <status>)*: <description>. "
    "Calls <endpoint>. Inherited from <inherited_from>.' Omit any field that's "
    "empty. If `workflow_steps` isn't present, fall back to nodes_json or "
    "snapshot in the same format. When node_meta isn't available either, "
    "fall back to: '**<Name>** *(type)*: plain-English description of its "
    "config'. Translate config (approver IDs, recipients, timeout in human "
    "units like '48 hours', message subjects, action_name) into plain English. "
    "Resolve action_name like "
    ...
    "(`Start → A → B → End`). If conditional branches exist "
    "(approved/rejected, True/False), show both paths on separate lines.\n"
    "   - '#### Field changes': for updates, show '- **field:** old → new' bullets.\n"
    "   - '#### Result': only if status_code >= 400, state the failure clearly. If "
    ...
    "7. Keep it tight. No filler like 'this audit log shows that…'. No preamble. "
    "No closing remarks. No timestamps (the UI shows those).\n"
    "8. Refer to the actor by display_name. Refer to the resource by its name "
    "(in quotes) when present; otherwise use 'resource #<id>'.\n"
    "9. Reply with ONLY the markdown body. Do not wrap it in a code fence."
)
...
def _build_user_prompt(row: Dict[str, Any]) -> str:
    # Workflow-execution audit rows store step snapshots under
    # `changes.after.steps`. Surface them at the top level so the prompt's
    # "#### Steps" section can render each node's node_meta verbatim.
    after_obj = row.get("after") or {}
    if isinstance(after_obj, str):
        try:
            after_obj = json.loads(after_obj)
        except Exception:
            after_obj = {}
    workflow_steps = None
    if isinstance(after_obj, dict):
        ws = after_obj.get("steps")
