...
def create_control(
    ...
    _ctrl_tenants = get_user_tenants(current_user, db)
    _ctrl_tenant = _ctrl_tenants[0] if _ctrl_tenants else None
    write_rich_audit_log(
        db=db,
        tenant_id=_ctrl_tenant,
        user_id=current_user.id,
        action="create",
        resource_type="control-library",
        resource_id=db_control.id,
        resource_name=f"[{db_control.code}] {db_control.name}",
        summary=f"Created Normalized Control '{db_control.code}': {db_control.name}",
        snapshot=model_to_dict(db_control),
        resource_url=f"/controls/{db_control.id}",
    )
    db.commit()
...
def update_control(
    ...
    _before_ctrl = model_to_dict(control)
    ...
    write_rich_audit_log(
        ...
        resource_name=f"[{control.code}] {control.name}",
        summary=f"Updated Normalized Control '{control.code}': {', '.join(update_data.keys()) or 'fields updated'}",
        before=_before_ctrl,
        after=model_to_dict(control),
        resource_url=f"/controls/{control_id}",
    )
    db.commit()
...
def delete_control(
    ...
    _snap_ctrl = model_to_dict(control)
    _ctrl_name = f"[{control.code}] {control.name}"
    ...
    write_rich_audit_log(
        ...
        resource_type="control-library",
        resource_id=control_id,
        resource_name=_ctrl_name,
        summary=f"Deleted Normalized Control {_ctrl_name}",
        snapshot=_snap_ctrl,
        resource_url=f"/controls/{control_id}",
    )
    db.commit()
...
def create_control_mapping(
    ...
    _map_tenants = get_user_tenants(current_user, db)
    _map_tenant = _map_tenants[0] if _map_tenants else None
    write_rich_audit_log(
        db=db,
        tenant_id=_map_tenant,
        user_id=current_user.id,
        action="create",
        resource_type="control-library",
        resource_id=db_mapping.id,
        resource_name=f"Control mapping #{db_mapping.id} for control #{control_id}",
        summary=f"Mapped control #{control_id} to framework control #{mapping.framework_control_id} ({mapping.mapping_type})",
        snapshot=model_to_dict(db_mapping),
        resource_url=f"/controls/{control_id}",
    )
    db.commit()
...
def delete_control_mapping(
    ...
    _snap_map = model_to_dict(mapping)
    ...
    write_rich_audit_log(
        ...
        resource_name=f"Control mapping #{mapping_id} for control #{control_id}",
        summary=f"Removed framework control mapping #{mapping_id} from control #{control_id}",
        snapshot=_snap_map,
        resource_url=f"/controls/{control_id}",
    )
    db.commit()
