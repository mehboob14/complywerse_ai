...
def preview_asset_posture(
    ...
    def _strip(eff: dict) -> dict:
        """Carry enough per-vuln data for the right-pane re-score list
        AND for the per-vuln breakdown cards to re-render their weighted
        equation when the operator is mid-preview. The fields below match
        what the per-vuln cards on the page already consume from the
        posture endpoint, so the frontend can use the same component."""
        return {
