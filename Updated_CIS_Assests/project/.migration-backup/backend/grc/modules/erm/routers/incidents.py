...
from ....models import (
    AuditLog, Risk, RiskIncident, GRCUser, get_db, NormalizedControl, FrameworkControl,
...
def create_incident(
    ...
    write_rich_audit_log(
        ...
        resource_name=db_incident.title,
        summary=f"Created Incident '{db_incident.title}' (severity: {db_incident.severity})",
        snapshot=model_to_dict(db_incident),
        resource_url=f"/erm/incidents/{db_incident.id}",
    )
    db.commit()
...
def update_incident(
    ...
    _before = {k: getattr(incident, k, None) for k in update_data}
    _full_before = model_to_dict(incident)
    ...
    write_rich_audit_log(
        ...
        summary=f"Updated Incident '{incident.title}': changed {', '.join(_changed_fields) or 'fields'}",
        before=_full_before,
        after=model_to_dict(incident),
        resource_url=f"/erm/incidents/{incident.id}",
    )
    db.commit()
...
def delete_incident(
    ...
    _saved_id = incident.id
    _saved_title = incident.title
    _saved_tenant = incident.tenant_id
    _saved_snapshot = model_to_dict(incident)
    ...
    write_rich_audit_log(
        ...
        resource_name=_saved_title,
        summary=f"Deleted Incident '{_saved_title}'",
        snapshot=_saved_snapshot,
        resource_url=f"/erm/incidents/{_saved_id}",
    )
    db.commit()
