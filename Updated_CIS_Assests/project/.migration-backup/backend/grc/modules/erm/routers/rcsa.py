...
def create_template(
    ...
    write_rich_audit_log(
        ...
        action="create",
        resource_type="rcsa_template",
        resource_id=db_template.id,
        resource_name=db_template.name,
        summary=f"Created RCSA template '{db_template.name}' (category: {db_template.category}, {len(template.questions)} questions)",
        after=model_to_dict(db_template),
    )
    db.commit()
...
def update_template(
    ...
    before_snapshot = model_to_dict(db_template)
    ...
    write_rich_audit_log(
        ...
        resource_name=db_template.name,
        summary=f"Updated RCSA template '{db_template.name}'",
...
def delete_template(
    ...
    write_rich_audit_log(
        ...
        resource_id=template_id,
        resource_name=db_template.name,
        summary=f"Deleted RCSA template '{db_template.name}'",
        before=model_to_dict(db_template),
    )
...
def clone_template(
    ...
    write_rich_audit_log(
        ...
        action="clone",
        resource_type="rcsa_template",
        resource_id=cloned_template.id,
        resource_name=cloned_template.name,
        summary=f"Cloned RCSA template '{source_template.name}' → '{cloned_template.name}' ({len(source_template.questions)} questions)",
        snapshot={"source_template_id": template_id, "source_template_name": source_template.name},
    )
    db.commit()
...
async def upload_template(
    ...
    write_rich_audit_log(
        ...
        resource_id=db_template.id,
        resource_name=db_template.name,
        summary=f"Uploaded RCSA template '{db_template.name}' from file '{file.filename}' ({len(questions)} questions)",
        snapshot={"source_file": file.filename, "category": category, "question_count": len(questions)},
    )
    db.commit()
    db.refresh(db_template)
...
def create_campaign(
    ...
    write_rich_audit_log(
        ...
        user_id=current_user.id,
        action="create",
        resource_type="rcsa_campaign",
        resource_id=db_campaign.id,
        resource_name=db_campaign.name,
        summary=f"Created RCSA campaign '{db_campaign.name}' using template '{template.name}' ({len(campaign.business_unit_ids)} business units)",
        after=model_to_dict(db_campaign),
    )
    db.commit()
...
def update_campaign(
    ...
    before_snapshot = model_to_dict(db_campaign)
    ...
    write_rich_audit_log(
        ...
        resource_name=db_campaign.name,
        summary=f"Updated RCSA campaign '{db_campaign.name}'",
        before=before_snapshot,
        after=model_to_dict(db_campaign),
    )
    db.commit()
...
def delete_campaign(
    ...
    write_rich_audit_log(
        ...
        resource_id=campaign_id,
        resource_name=db_campaign.name,
        summary=f"Deleted RCSA campaign '{db_campaign.name}'",
        before=model_to_dict(db_campaign),
    )
