...
from ....models import (
    AuditLog, Risk, RiskControlLink, RiskAssetLink, RiskEvidenceLink,
...
def create_risk(
    ...
    write_rich_audit_log(
        ...
        resource_name=db_risk.title,
        summary=f"Created Risk '{db_risk.title}' with status '{db_risk.status}'",
        snapshot=model_to_dict(db_risk),
        resource_url=f"/risks/{db_risk.id}",
    )
    db.commit()
...
def update_risk(
    ...
    _before = {k: getattr(risk, k, None) for k in update_data}
    _full_before = model_to_dict(risk)
    ...
    _full_after = model_to_dict(risk)
    write_rich_audit_log(
        db=db,
        tenant_id=risk.tenant_id,
        user_id=current_user.id,
        ...
        summary=f"Updated Risk '{risk.title}': changed {', '.join(_changed_fields) or 'fields'}",
        before=_full_before,
        after=_full_after,
        resource_url=f"/risks/{risk.id}",
    )
    db.commit()
...
def delete_risk(
    ...
    _saved_id = risk.id
    _saved_title = risk.title
    _saved_tenant = risk.tenant_id
    _saved_snapshot = model_to_dict(risk)
    ...
    write_rich_audit_log(
        ...
        summary=f"Deleted Risk '{_saved_title}'",
        snapshot=_saved_snapshot,
        resource_url=f"/risks/{_saved_id}",
    )
    db.commit()
    return None
