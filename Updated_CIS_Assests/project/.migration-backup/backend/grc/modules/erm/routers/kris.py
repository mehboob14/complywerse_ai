...
def create_kri(
    ...
    db.add(db_kri)
    db.flush()
    kri_tenant_id = tenant_id or user_tenants[0]
    write_rich_audit_log(
        db=db,
        tenant_id=kri_tenant_id,
        user_id=current_user.id,
        action="create",
        resource_type="kri",
        resource_id=db_kri.id,
        resource_name=db_kri.name,
        summary=f"Created KRI '{db_kri.name}' (metric: {metric_type}, frequency: {frequency})",
        after=model_to_dict(db_kri),
    )
    db.commit()
    db.refresh(db_kri)
    return db_kri
...
def update_kri(
    ...
    before_snapshot = model_to_dict(kri)
    update_data = kri_update.model_dump(exclude_unset=True)
    for key, value in update_data.items():
        setattr(kri, key, value)
    
    kri_tenant_id = db.query(Risk.tenant_id).filter(Risk.id == kri.risk_id).scalar() if kri.risk_id else user_tenants[0]
    write_rich_audit_log(
        db=db,
        tenant_id=kri_tenant_id,
        user_id=current_user.id,
        ...
        resource_type="kri",
        resource_id=kri_id,
        resource_name=kri.name,
        summary=f"Updated KRI '{kri.name}'",
        before=before_snapshot,
        after=model_to_dict(kri),
    )
    db.commit()
    db.refresh(kri)
...
def delete_kri(
    ...
    write_rich_audit_log(
        ...
        resource_id=kri_id,
        resource_name=kri.name,
        summary=f"Deleted KRI '{kri.name}'",
        before=model_to_dict(kri),
    )
    db.delete(kri)
    db.commit()
...
def record_kri_measurement(
    ...
    write_rich_audit_log(
        ...
        resource_id=kri_id,
        resource_name=kri.name,
        summary=f"Recorded measurement for KRI '{kri.name}': value={measurement.value} ({status_val})",
        snapshot={"value": measurement.value, "status": status_val, "notes": measurement.notes},
    )
    db.commit()
