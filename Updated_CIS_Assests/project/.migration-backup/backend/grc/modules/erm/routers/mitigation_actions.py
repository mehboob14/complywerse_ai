...
def update_action(
    ...
    before_snapshot = model_to_dict(action)
    update_data = action_update.model_dump(exclude_unset=True)
    for key, value in update_data.items():
        setattr(action, key, value)
    
    action.updated_at = datetime.utcnow()
    tenant_id = db.query(Risk.tenant_id).filter(Risk.id == action.risk_id).scalar() or user_tenants[0]
    write_rich_audit_log(
        ...
        summary=f"Updated mitigation action '{action.title}'",
        before=before_snapshot,
        after=model_to_dict(action),
    )
    db.commit()
    db.refresh(action)
...
def delete_action(
    ...
    write_rich_audit_log(
        ...
        resource_id=action_id,
        resource_name=action.title,
        summary=f"Deleted mitigation action '{action.title}'",
        before=model_to_dict(action),
    )
    db.delete(action)
    db.commit()
...
def complete_action(
    ...
    write_rich_audit_log(
        ...
        resource_id=action_id,
        resource_name=action.title,
        summary=f"Completed mitigation action '{action.title}'" + (f" (actual reduction: {actual_reduction}%)" if actual_reduction is not None else ""),
        snapshot={"status": "completed", "actual_residual_reduction": actual_reduction},
    )
    db.commit()
    db.refresh(action)
...
def create_risk_action(
    ...
    write_rich_audit_log(
        ...
        resource_name=db_action.title,
        summary=f"Created mitigation action '{db_action.title}' for risk #{risk_id} (priority: {db_action.priority})",
        after=model_to_dict(db_action),
    )
    db.commit()
