...
def set_clause_applicability(
    ...
    control_ref = control.original_reference or control.control_id
    write_rich_audit_log(
        ...
        resource_name=f"Control {control_ref} applicability",
        summary=f"Marked control {control_ref} as {'Applicable' if request.is_applicable else 'Not Applicable'} for framework '{framework.name}'",
        snapshot={
            "control_reference": control_ref,
            "framework_id": framework.id,
            "is_applicable": request.is_applicable,
            "justification": request.justification[:500],
            "status": "pending",
        },
    )
    db.commit()
...
def review_applicability(
    ...
    write_rich_audit_log(
        ...
        resource_type="clause_applicability",
        resource_id=record.id,
        resource_name=f"Clause applicability #{record.id}",
        summary=f"{'Approved' if request.status == 'approved' else 'Rejected'} applicability decision for control #{record.control_id}",
        snapshot={
