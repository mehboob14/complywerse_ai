...
def rollback_statement(
    ...
    write_rich_audit_log(
        ...
        resource_name=f"Statement {statement.statement_code}",
        summary=f"Rolled back statement '{statement.statement_code}' to version {target_display_version} in document #{document_id}",
        snapshot={
            "document_id": document_id,
            "statement_code": statement.statement_code,
...
def apply_reparse_proposals(
    ...
    write_rich_audit_log(
        ...
        resource_name=f"Document #{document_id}",
        summary=f"Applied re-parse proposals to document #{document_id}: {accepted} accepted, {rejected} rejected",
        snapshot={
            "accepted": accepted,
            "rejected": rejected,
