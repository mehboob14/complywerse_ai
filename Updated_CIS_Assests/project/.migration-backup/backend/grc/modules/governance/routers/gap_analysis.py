    ...
    try:
        ...
        for run_id in run_ids:
            ...
            try:
                ...
                # Task #46: per-finding critical-rules eval + auto-patch drafting.
                try:
                    from ..services.critical_rules import apply_critical_status
                    from ..services.patch_drafter import draft_patches_for_findings

                    db.flush()
                    new_findings = (
                        db.query(PolicyGapFinding)
                        .filter(PolicyGapFinding.analysis_run_id == run.id)
                        .all()
                    )
                    for nf in new_findings:
                        apply_critical_status(db, nf)
                    db.flush()
                    draft_patches_for_findings(db, new_findings, drafted_by_user_id=user_id)
                except Exception as e:  # pragma: no cover - keep main run resilient
                    import logging
                    logging.getLogger(__name__).warning(
                        "gap_analysis.policy_ai_hook.failed run_id=%s err=%s", run.id, e
                    )
