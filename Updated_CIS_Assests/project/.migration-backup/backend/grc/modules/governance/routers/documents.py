...
def create_document(
    ...
    write_rich_audit_log(
        ...
        resource_name=db_document.title,
        summary=f"Created Governance Document '{db_document.title}' [{db_document.doc_type}] v{db_document.current_version}",
        snapshot=model_to_dict(db_document),
        resource_url=f"/governance/documents/{db_document.id}",
    )
    db.commit()
...
def update_document(
    ...
    _full_before = model_to_dict(document)
    changed_fields = []
    ...
    _changed_names = [c['field'] for c in changed_fields]
    write_rich_audit_log(
        db=db,
        tenant_id=document.tenant_id,
        user_id=current_user.id,
        ...
        summary=f"Updated Governance Document '{document.title}': changed {', '.join(_changed_names) or 'fields'}",
        before=_full_before,
        after=model_to_dict(document),
        resource_url=f"/governance/documents/{document.id}",
    )
    db.commit()
...
def delete_document(
    ...
    _saved_id = document.id
    _saved_title = document.title
    _saved_tenant = document.tenant_id
    _saved_snapshot = model_to_dict(document)
    ...
    write_rich_audit_log(
        ...
        summary=f"Deleted Governance Document '{_saved_title}'",
        snapshot=_saved_snapshot,
        resource_url=f"/governance/documents/{_saved_id}",
    )
    db.commit()
