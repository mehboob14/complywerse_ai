...
def create_audit_log_entry(
    ...
    """Create an audit log entry for tracking actions via rich audit."""
    write_rich_audit_log(
        ...
        resource_name=resource_name or f"{resource_type} #{resource_id}",
        summary=summary or f"{action} on {resource_type} #{resource_id}",
        snapshot=changes or {},
    )
...
def create_regulatory_change(
    ...
    write_rich_audit_log(
        ...
        resource_id=db_change.id,
        resource_name=db_change.title,
        summary=f"Created regulatory change '{db_change.title}' (source: {source}, priority: {priority_value})",
        after=model_to_dict(db_change),
    )
    db.commit()
...
def update_regulatory_change(
    ...
    before_snapshot = model_to_dict(db_change)
    ...
    write_rich_audit_log(
        db=db,
        tenant_id=db_change.tenant_id,
        user_id=current_user.id,
        action="update",
        resource_type="regulatory_change",
        resource_id=change_id,
        resource_name=db_change.title,
        summary=f"Updated regulatory change '{db_change.title}'",
...
def delete_regulatory_change(
    ...
    write_rich_audit_log(
        ...
        resource_id=change_id,
        resource_name=db_change.title,
        summary=f"Deleted regulatory change '{db_change.title}'",
        before=model_to_dict(db_change),
    )
    db.delete(db_change)
    db.commit()
...
def create_impact_assessment(
    ...
    write_rich_audit_log(
        ...
        user_id=current_user.id,
        action="create",
        resource_type="regulatory_impact_assessment",
        resource_id=db_assessment.id,
        resource_name=f"{assessment_type} assessment for '{change.title}'",
        summary=f"Created {assessment_type} impact assessment for regulatory change '{change.title}' (impact: {impact_level}, gap: {gap_identified})",
        after=model_to_dict(db_assessment),
    )
    db.commit()
...
def create_implementation_task(
    ...
    write_rich_audit_log(
        ...
        user_id=current_user.id,
        action="create",
        resource_type="regulatory_task",
        resource_id=db_task.id,
        resource_name=db_task.title,
        summary=f"Created implementation task '{db_task.title}' for regulatory change #{change_id} (type: {task.task_type}, priority: {task.priority})",
        after=model_to_dict(db_task),
    )
    db.commit()
    db.refresh(db_task)
...
def update_implementation_task(
    ...
    before_snapshot = model_to_dict(db_task)
    for key, value in update_data.items():
    ...
    write_rich_audit_log(
        ...
        resource_id=task_id,
        resource_name=db_task.title,
        summary=f"Updated implementation task '{db_task.title}'",
        before=before_snapshot,
        after=model_to_dict(db_task),
    )
    db.commit()
...
def delete_implementation_task(
    ...
    write_rich_audit_log(
        ...
        resource_id=task_id,
        resource_name=db_task.title,
        summary=f"Deleted implementation task '{db_task.title}'",
        before=model_to_dict(db_task),
    )
    db.delete(db_task)
    db.commit()
...
def close_regulatory_change(
    ...
    write_rich_audit_log(
        ...
        summary=f"Closed regulatory change '{change.title}' (was: {previous_status})",
        snapshot={
