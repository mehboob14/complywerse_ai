...
from ....models import (
    AuditLog, Evidence, EvidenceVersion, EvidenceControlMapping, EvidenceAIAssessment,
...
def create_evidence(
    ...
    write_rich_audit_log(
        ...
        resource_name=db_evidence.name,
        summary=f"Created Evidence '{db_evidence.name}' (type: {db_evidence.evidence_type})",
        snapshot=model_to_dict(db_evidence),
        resource_url=f"/evidence/{db_evidence.id}",
    )
    db.commit()
...
async def upload_evidence(
    ...
    write_rich_audit_log(
        ...
        resource_id=db_evidence.id,
        resource_name=db_evidence.name,
        summary=f"Uploaded Evidence file '{db_evidence.name}' (type: {db_evidence.evidence_type})",
        snapshot=model_to_dict(db_evidence),
        resource_url=f"/evidence/{db_evidence.id}",
    )
    db.commit()
...
def update_evidence(
    ...
    _before = {k: getattr(evidence, k, None) for k in update_data}
    _full_before = model_to_dict(evidence)
    ...
    write_rich_audit_log(
        ...
        summary=f"Updated Evidence '{evidence.name}': changed {', '.join(_changed_fields) or 'fields'}",
        before=_full_before,
        after=model_to_dict(evidence),
        resource_url=f"/evidence/{evidence.id}",
    )
    db.commit()
...
def delete_evidence(
    ...
    _saved_id = evidence.id
    _saved_name = evidence.name
    _saved_tenant = evidence.tenant_id
    _saved_snapshot = model_to_dict(evidence)
    ...
    write_rich_audit_log(
        ...
        summary=f"Deleted Evidence '{_saved_name}'",
        snapshot=_saved_snapshot,
        resource_url=f"/evidence/{_saved_id}",
    )
    db.commit()
