...
def test_connection(
    ...
    """Live-test a stored connection.

    For the plugin-runner integration types (windows_winrm / linux_ssh /
    aws_readonly) we delegate to ``preflight_check`` which actually opens
    the protocol session — exactly the same check the Connect Wizard runs
    before saving. For legacy vendor adapters (Nexpose/Nessus) we keep the
    existing SyncService path.
    """
    tenant_id = get_user_primary_tenant(current_user, db)

    # Look up the connection up front so we can dispatch by integration_type.
    conn = (
    ...
    plugin_runner_types = {"windows_winrm", "linux_ssh", "aws_readonly"}
    if (conn.integration_type or "").lower() in plugin_runner_types:
        ...
        pf = preflight_check(conn.integration_type, creds)
        return {
            "success": pf.ok,
            "message": pf.detail,
            "code": pf.code,
    ...
    # Legacy vendor adapters (Nexpose, Nessus, etc.)
