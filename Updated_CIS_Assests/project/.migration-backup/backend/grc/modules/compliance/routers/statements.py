...
def update_statement(
    ...
    _full_before = model_to_dict(statement)
    ...
    _updated_fields = {k: v for k, v in update_data.dict().items() if v is not None}
    write_rich_audit_log(
        ...
        resource_name=statement.statement_code or f"Statement #{statement.id}",
        summary=f"Updated Policy Statement: changed {', '.join(_updated_fields.keys()) or 'fields'}",
        before=_full_before,
        after=model_to_dict(statement),
        resource_url=f"/compliance/statements/{statement.id}",
    )
    db.commit()
...
def update_compliance_status(
    ...
    _full_before_compliance = model_to_dict(compliance)
    ...
    write_rich_audit_log(
        ...
        resource_name=getattr(statement, 'statement_code', None) or f"Statement #{statement_id}",
        summary=f"Updated compliance status to '{compliance_data.compliance_status}' for statement #{statement_id}",
        before=_full_before_compliance,
        after=model_to_dict(compliance),
        resource_url=f"/compliance/statements/{statement_id}",
    )
    db.commit()
...
def link_evidence_to_compliance(
    ...
    write_rich_audit_log(
        ...
        resource_type="compliance",
        resource_id=statement_id,
        resource_name=statement.statement_code or f"Statement #{statement_id}",
        summary=f"Linked {len(newly_linked)} evidence item(s) to compliance statement #{statement_id}",
        snapshot={"evidence_ids_added": newly_linked, "total_evidence_ids": updated_ids},
    )
    db.commit()
    db.refresh(compliance)
