...
def create_assessment(
    ...
    write_rich_audit_log(
        ...
        resource_name=f"{assessment.assessment_type or 'assessment'} for vendor #{payload.vendor_id}",
        summary=f"Created {assessment.assessment_type or 'assessment'} for vendor '{vendor.name}'",
        after=model_to_dict(assessment),
    )
    db.commit()
...
def update_assessment(
    ...
    before_snapshot = model_to_dict(assessment)
    ...
    write_rich_audit_log(
        ...
        resource_name=f"{assessment.assessment_type or 'assessment'} #{assessment_id}",
        summary=f"Updated vendor assessment #{assessment_id}",
...
def delete_assessment(
    ...
    write_rich_audit_log(
        ...
        resource_type="vendor_assessment",
        resource_id=assessment_id,
        resource_name=assessment_label,
        summary=f"Deleted vendor {assessment_label}",
        before=model_to_dict(assessment),
    )
...
def score_assessment(
    ...
    write_rich_audit_log(
        ...
        resource_type="vendor_assessment",
        resource_id=assessment_id,
        resource_name=f"Assessment #{assessment_id}",
        summary=f"Scored vendor assessment #{assessment_id}: inherent={inherent_score}, rating={assessment.risk_rating}",
        snapshot={"inherent_score": inherent_score, "risk_rating": assessment.risk_rating, "status": "reviewed"},
    )
    db.commit()
    db.refresh(assessment)
    assessment = get_assessment_or_404(assessment_id, tenant_ids, db)
...
def approve_assessment(
    ...
    write_rich_audit_log(
        ...
        resource_id=assessment_id,
        resource_name=f"Assessment #{assessment_id}",
        summary=f"Approved vendor assessment #{assessment_id} for vendor #{assessment.vendor_id} (rating: {assessment.risk_rating})",
        snapshot={"status": "approved", "risk_rating": assessment.risk_rating, "inherent_score": assessment.inherent_score},
    )
    db.commit()
    db.refresh(assessment)
    assessment = get_assessment_or_404(assessment_id, tenant_ids, db)
