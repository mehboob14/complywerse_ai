...
def create_inheritance_relationship(
    ...
    write_rich_audit_log(
        ...
        action="create",
        resource_type="control_library",
        resource_id=inheritance.id,
        resource_name=f"Inheritance {inheritance.parent_type}:{inheritance.parent_control_id} → {inheritance.child_type}:{inheritance.child_control_id}",
        summary=f"Created control inheritance: {inheritance.parent_type} #{inheritance.parent_control_id} → {inheritance.child_type} #{inheritance.child_control_id}",
        snapshot=model_to_dict(inheritance),
        resource_url=f"/control-library/{inheritance.id}",
    )
    db.commit()
...
def update_inheritance_relationship(
    ...
    _before_inh = model_to_dict(inheritance)
    ...
    write_rich_audit_log(
        ...
        resource_id=inheritance.id,
        resource_name=f"Inheritance {inheritance.parent_type}:{inheritance.parent_control_id} → {inheritance.child_type}:{inheritance.child_control_id}",
        summary=f"Updated control inheritance #{inheritance.id}: coverage={inheritance.coverage_percentage}%, type={inheritance.inheritance_type}",
        before=_before_inh,
        after=model_to_dict(inheritance),
        resource_url=f"/control-library/{inheritance.id}",
    )
    db.commit()
...
def delete_inheritance_relationship(
    ...
    _snap_inh = model_to_dict(inheritance)
    _inh_tenant_id = inheritance.tenant_id
    _inh_name = f"Inheritance {inheritance.parent_type}:{inheritance.parent_control_id} → {inheritance.child_type}:{inheritance.child_control_id}"
    ...
    write_rich_audit_log(
        db=db,
        tenant_id=_inh_tenant_id,
        user_id=current_user.id,
        action="delete",
        resource_type="control_library",
        resource_id=inheritance_id,
        resource_name=_inh_name,
        summary=f"Deleted control inheritance #{inheritance_id}",
        snapshot=_snap_inh,
        resource_url=f"/control-library/{inheritance_id}",
    )
    db.commit()
