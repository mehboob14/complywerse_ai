...
from ....models import (
    AuditLog, CommonControlGroup, CommonControlGroupMapping, NormalizedControl,
...
def create_group(
    ...
    write_rich_audit_log(
        ...
        summary=f"Created Control Group '{group.name}' [{group.code}]",
        snapshot=model_to_dict(group),
        resource_url=f"/control-library/{group.id}",
    )
    db.commit()
...
def update_group(
    ...
    _before = {k: getattr(group, k, None) for k in update_data}
    _full_before = model_to_dict(group)
    ...
    write_rich_audit_log(
        ...
        resource_id=group.id,
        resource_name=group.name,
        summary=f"Updated Control Group '{group.name}': changed {', '.join(_changed_fields) or 'fields'}",
        before=_full_before,
        after=model_to_dict(group),
        resource_url=f"/control-library/{group.id}",
    )
    db.commit()
...
def delete_group(
    ...
    group_id_saved = group.id
    group_name_saved = group.name
    tenant_id_saved = group.tenant_id or 0
    _saved_snapshot = model_to_dict(group)
    ...
    write_rich_audit_log(
        ...
        action="delete",
        resource_type="control-library",
        resource_id=group_id_saved,
        resource_name=group_name_saved,
        summary=f"Deleted Control Group '{group_name_saved}'",
        snapshot=_saved_snapshot,
        resource_url=f"/control-library/{group_id_saved}",
    )
    db.commit()
    return None
...
def add_controls_to_group(
    ...
    _ctrl_tenant = group.tenant_id or (user_tenants[0] if user_tenants else None)
    write_rich_audit_log(
        db=db,
        ...
        resource_name=group.name,
        summary=f"Added {len(created_mappings)} control(s) to Control Group '{group.name}'",
        after={"group_id": group_id, "added_controls": created_mappings},
        resource_url=f"/control-library/{group_id}",
    )
    db.commit()
...
def remove_control_from_group(
    ...
    _snap_mapping = model_to_dict(mapping)
    _map_tenant = group.tenant_id or (user_tenants[0] if user_tenants else None)
    ...
    write_rich_audit_log(
        ...
        resource_name=f"Control mapping #{mapping_id} in group '{group.name}'",
        summary=f"Removed control mapping #{mapping_id} from Control Group '{group.name}'",
        snapshot=_snap_mapping,
        resource_url=f"/control-library/{group_id}",
    )
    db.commit()
