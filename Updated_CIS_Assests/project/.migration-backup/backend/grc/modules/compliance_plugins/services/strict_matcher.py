...
def applicable_plugins_for_asset_aware(
    db: Session,
    tenant_id: int,
    asset_os: str,
    asset_role: Optional[str] = None,
) -> Tuple[List[CompliancePlugin], Optional[str]]:
    """Asset-role-aware benchmark resolution.

    OS hosts stay STRICT — they only match via the operator-owned
    BenchmarkOsMapping table (no auto-match, archived benchmarks excluded).

    APPLICATION assets (asset_role='application', e.g. a promoted
    'postgresql-18' child) get a fallback: if no manual mapping exists, match
    the benchmark DIRECTLY from the library by software_key (os_keys contains
    the key). This makes promoted app-assets scannable without per-app mapping
    rows, and means a benchmark uploaded LATER for that app auto-applies.
    """
    plugins, bench = applicable_plugins_for_asset(db, tenant_id, asset_os)
    if bench is not None or (asset_role or "").lower() != "application" or not asset_os:
        return plugins, bench
    # ── Application fallback: resolve straight from the library ──
    from grc.modules.compliance_plugins.services.software_normaliser import (
        benchmark_for_software_key,
    )
    bench2 = benchmark_for_software_key(db, asset_os)
    if not bench2:
        return [], None
    plugins2 = (
        db.query(CompliancePlugin)
        .filter(
            ...
            CompliancePlugin.review_status.in_(["approved", "auto_approved"]),
            CompliancePlugin.enabled.is_(True),
            CompliancePlugin.benchmark == bench2,
        )
        .order_by(CompliancePlugin.rule_id.asc())
        .all()
    )
    return plugins2, bench2
