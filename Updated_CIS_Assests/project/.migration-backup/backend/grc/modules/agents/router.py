...
# Destructive actions (revoke) additionally require Administrator —
# Scanning Admin can create but not delete.
...
class HeartbeatPayload(BaseModel):
    ...
    # ─── Software inventory ("room and chair") ────────────────────────
    # Raw 3-layer inventory: Windows roles + registry apps + listening
    # processes. Backend normalizes each entry to a software_key and
    # stores the enriched list on the asset's detected_software_json so
    # the UI can offer "promote to child asset".
...
def download_self_enrolling_installer(
    ...
    cmd += ("powershell -NoProfile -ExecutionPolicy Bypass -Command "
            "\"try { Invoke-WebRequest -UseBasicParsing -Uri '%BASE%/grc/agent/setup.ps1' "
...
# Small helper used by the installer endpoint above. Mirrors the LAN-IP
# auto-detection in main.py so the colleague's PC knows where to phone
# home — falls back to the request's Host header.
def _backend_url_from_request(request: Request) -> str:
    host = (request.headers.get("host") or "").split(":")[0]
    if host and host not in ("localhost", "127.0.0.1", "0.0.0.0"):
        return f"http://{request.headers['host']}"
    # Discover LAN IP via outbound interface
    try:
...
import socket  # noqa: E402  (needed by _backend_url_from_request above)
...
def agent_heartbeat(
    ...
    if asset is None and (body.hostname or agent.hostname) and body.os_normalized:
        ...
        if existing is None:
            asset = ITAsset(
                ...
                criticality="medium",
                status="active",
                # The user who minted the enrollment token becomes the
                # owner of this auto-discovered host. Without this, every
                # agent-discovered asset shows as "unowned" and the team
                # activity view can't tell who's responsible for the box.
                owner_id=getattr(agent, "created_by_user_id", None),
            )
            db.add(asset)
            db.flush()
            agent.asset_id = asset.id
    ...
    # data updates. SAFEGUARD: collapse os_normalized to level-2 so an
    # older agent reporting "ubuntu-24.04.2" still ends up at "ubuntu-24.04"
    # for matching. Newer agents already truncate client-side; this is
    # belt-and-braces.
    if asset is not None:
        if body.os_family:    asset.os_family = body.os_family
        if body.os_version:   asset.os_version = body.os_version
        if body.os_normalized:
            from grc.modules.compliance_plugins.services.os_detector import truncate_os_key_to_second_level
            asset.os_normalized = truncate_os_key_to_second_level(body.os_normalized)
        if body.os_build:     asset.os_build = body.os_build
        if body.os_edition:   asset.os_edition = body.os_edition
        # Software inventory → enrich with software_keys + benchmark
        # availability, persist on the asset. Preserve promoted_asset_id
        # links from the previous inventory so re-heartbeats don't lose
        # track of which entries were already promoted to child assets.
        if body.installed_software is not None:
            try:
                from grc.modules.compliance_plugins.services.software_normaliser import enrich_inventory
                ...
                logger.warning("software inventory enrich failed: %s", exc)
...
def agent_results(
    ...
    for entry in body.runs or []:
        ...
        # Collector-mode: the job carries a run_id of the pending row that
        # was pre-created by run_service.py. Update that row in place so
        # the operator's progress UI sees the same row resolve, rather
        # than creating an orphan new one.
        run_id = entry.get("run_id")
        if run_id:
            existing_run = db.query(CompliancePluginRun).filter(
                CompliancePluginRun.id == run_id,
                CompliancePluginRun.tenant_id == agent.tenant_id,
                CompliancePluginRun.executed_by_agent_id == agent.id,
            ).first()
            if existing_run:
                existing_run.status = entry.get("status", "error")
                existing_run.completed_at = datetime.utcnow()
                existing_run.result_summary = entry.get("result_summary")
                existing_run.raw_output = entry.get("raw_output")
                existing_run.result_detail = entry.get("result_detail")
                if entry.get("duration_ms"):
                    existing_run.duration_ms = entry["duration_ms"]
                if entry.get("error_message"):
                    existing_run.error_message = entry["error_message"]
                updated += 1
                continue
            # If we get a run_id but can't find the row (collector posting
            # stale data after the run was deleted), fall through to the
            # plugin lookup so we don't lose evidence entirely.
        plugin = None
        if plugin_id:
...
def agent_jobs(
    ...
    # ─── Priority 1: pending runs explicitly assigned to this collector ──
    # The new collector-routing flow (run_service.py) writes a pending
    # CompliancePluginRun row with executed_by_agent_id = this collector
    # whenever an operator triggers a scan on a connection that's bound
    # to this collector. These take priority over the heuristic firehose
    # because they're explicit operator intent, not a guess.
    if agent.mode == "collector":
        pending = (
            ...
            .filter(
                ...
                CompliancePluginRun.executed_by_agent_id == agent.id,
                CompliancePluginRun.status == "pending",
            )
            .order_by(CompliancePluginRun.id.asc())
            .limit(limit)
            .all()
        )
        for run in pending:
            plg = db.query(CompliancePlugin).get(run.plugin_id)
            if not plg:
                continue
            job = {
                "run_id": run.id,                  # collector POSTs this back as the row to UPDATE
                "plugin_id": plg.id,
                "plugin_key": plg.plugin_key,
                "rule_id": plg.rule_id,
                "title": plg.title,
                "severity": plg.severity,
                "runner_type": plg.runner_type,
                "check_definition": plg.check_definition,
                "asset_id": run.asset_id,
                "connection_id": run.connection_id,
            }
            # Decrypt the connection's creds for this specific target.
            if run.connection_id:
                conn = db.query(IntegrationConnection).get(run.connection_id)
                if conn:
                    try:
                        from grc.modules.compliance_plugins.services.credentials import credentials_for
                        job["credentials"] = credentials_for(conn)
                    except Exception as exc:  # noqa: BLE001
                        logger.warning("credentials_for(run %s) failed: %s", run.id, exc)
            # Mark as 'running' so a duplicate /jobs poll doesn't pick it up
            # twice if the collector calls in burst-mode.
            run.status = "running"
            jobs.append(job)
        if pending:
            db.commit()
        if len(jobs) >= limit:
            # Don't dilute with heuristic jobs when operator-assigned work
            # is waiting. Collector returns to /jobs after posting results.
            return {"jobs": jobs, "agent_id": agent.id, "mode": agent.mode}

    # ─── Priority 2 (legacy): heuristic match by runner + OS benchmark ──
    for p in plugins_q:
