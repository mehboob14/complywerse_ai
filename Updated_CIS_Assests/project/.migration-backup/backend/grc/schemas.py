...
class ITAssetCreate(ITAssetBase):
    ...
    # Block D — operators can optionally supply OS fields on create. If
    # os_version is provided but os_normalized is empty, create_asset will
    # call the AI OS normaliser (same path the agent + connect wizard use)
    # so the asset is benchmark-resolvable from the moment it's inserted,
    # without requiring the operator to also "Re-detect OS" by hand.
...
class ITAssetUpdate(BaseModel):
    ...
    # Block D — allow PUT to set OS profile fields directly (used by the
    # admin "Re-detect OS" action and by manual OS overrides).
    os_family: Optional[str] = None
    os_version: Optional[str] = None
    os_normalized: Optional[str] = None
    os_build: Optional[str] = None
    os_edition: Optional[str] = None
    # Risk Posture v2 — business-context fields the operator sets via the
    # Business Context panel on /assets/[id]. Drives the w5 multiplier in
    # the effective-risk formula. See docs/RISK_POSTURE_PLAN.md §3.
...
class ITAssetResponse(BaseModel):
    ...
    status: str
    cde_environment: Optional[bool] = False
    # OS profile fields (Block B/C)
    os_family: Optional[str] = None
    os_version: Optional[str] = None
    os_normalized: Optional[str] = None
    os_build: Optional[str] = None
...
class AssetDetailResponse(BaseModel):
    ...
    business_impact_notes: Optional[str] = None
    created_at: datetime
    # Room-and-chair fields — present on every asset, critical for the
    # app-asset detail page to know whether to show the Connect button.
    asset_role: Optional[str] = "host"
