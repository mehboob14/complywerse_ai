...
def heartbeat() -> dict:
    ...
    # ── OS profile + software inventory ("room and chair") ──────────────
    # Endpoint agents inventory their own host so the cloud can route the
    # exact CIS benchmark and populate the asset's "Applications on this
    # host" panel. Best-effort: collect() returns {} on non-Windows or any
    # probe failure, so the heartbeat itself never breaks.
    try:
        # Dispatch to the right OS collector. Each collect() guards its own
        # platform and returns {} otherwise, so this is purely an import
        # optimisation — we never import the wrong OS's subprocess probe.
        sysname = platform.system().lower()
        if sysname.startswith("win"):
            from . import inventory_windows as _inv
        elif sysname == "linux":
            from . import inventory_linux as _inv
        else:
            _inv = None
        if _inv is not None:
            body.update(_inv.collect())
    except Exception as exc:  # noqa: BLE001
        logger.warning("inventory collection failed, sending bare heartbeat: %s", exc)
    return post(
