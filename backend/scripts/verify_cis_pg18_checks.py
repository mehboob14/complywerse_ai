"""Verify authored postgres_sql CIS PG18 checks against the live cluster.

Uses the platform DB login from TENANT_DB_URL_TEMPLATE (grc_app @ 5433).
Runs every AUTHORED rule whose runner_type is postgres_sql and prints
pass/fail/error with the observed first value — never fabricates a verdict.

Usage (from backend/):
  python scripts/verify_cis_pg18_checks.py
"""
...
from grc.modules.compliance_plugins.seed_cis_pg18 import AUTHORED
...
def main() -> None:
    ...
    for rule_id, spec in sorted(
        ...
        try:
            ...
            # ascii-clean for Windows consoles
