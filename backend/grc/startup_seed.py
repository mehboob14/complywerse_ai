...
def _first_user_id(db: Session) -> int:
    # NB: Query.scalar() calls one() and raises MultipleResultsFound on a
    # multi-user tenant DB; first() takes the lowest id safely.
...
def _run_backend_seed_script(script_name: str, db: Session, slug: str) -> bool:
    ...
    try:
        ...
        # Roll back so a failed seeder doesn't leave the shared session in a
        # needs-rollback state — otherwise every subsequent query (later seeders,
        # or the request itself) fails with PendingRollbackError.
        try:
            db.rollback()
        except Exception:
            pass
