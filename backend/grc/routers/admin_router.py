...
# ────────────────────────────────────────────────────────────────────────
# Audit-log AI summary — Workflow/Audit-AI integration (v2)
# On-demand natural-language summary of an audit row, generated by OpenAI
# and cached inline at `changes.ai_summary` so each row is summarised at
# most once. Falls back to the template summary on any failure (missing
# key, rate limit, network) — the UI degrades gracefully.
# ────────────────────────────────────────────────────────────────────────
...
def generate_audit_log_ai_summary(
    ...
    """Generate (or return cached) AI summary for one audit row.

    Cache behaviour: if `changes.ai_summary` is already set and `force=false`
    (default), returns the cached value. Otherwise calls OpenAI, persists
    the result back to `changes.ai_summary`, and returns it.

    Response shape: `{ai_summary, cached, fallback}`.
    `fallback=true` means OpenAI was unavailable (no key / error) and the
    caller should render the row's template `summary` instead.
    """
    ...
    if not force and changes.get("ai_summary"):
        return {"ai_summary": changes["ai_summary"], "cached": True, "fallback": False}

    # Resolve the actor display name so the prompt can use a human reference
    # rather than just a numeric user id.
    actor_display = None
    if log.user_id:
        u = db.query(GRCUser).filter(GRCUser.id == log.user_id).first()
        if u:
            actor_display = u.display_name or u.username
    if not actor_display:
        actor_display = changes.get("actor_display") or changes.get("actor") or "System"

    # Build the row dict consumed by the AI summary helper. The helper reads
    # straight off the dict (not the ORM model) so callers can synthesise
    # missing fields if needed.
    row_for_ai = {
        "user_name": actor_display,
    ...
    ai_text = generate_ai_summary(row_for_ai)
    if not ai_text:
        # Graceful fallback — UI renders existing template summary.
        return {
            "ai_summary": changes.get("summary") or "",
            "cached": False,
            "fallback": True,
        }

    # Persist back into the audit row so subsequent opens are instant.
    changes["ai_summary"] = ai_text
    log.changes = changes
    flag_modified(log, "changes")
    db.commit()
    return {"ai_summary": ai_text, "cached": False, "fallback": False}
