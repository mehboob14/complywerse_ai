...
def backfill_host_asset_links(
    ...
    `dry_run=true` reports what WOULD link without writing. Returns a match
    report (assets, findings with a host, matched, newly-linked, unmatched)."""
    user_tenants = get_user_tenants(current_user, db)
    if not user_tenants:
        raise HTTPException(status_code=403, detail="User not associated with any tenant")
    from ....services.finding_asset_linker import backfill_host_links
    report = backfill_host_links(db, user_tenants[0], commit=not dry_run)
