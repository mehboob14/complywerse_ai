...
def seed(db, tids):
    """Idempotent, defensive entry point.

    Must NEVER raise: a demo-seed failure cannot be allowed to crash app startup
    or poison the shared DB session (an un-rolled-back error cascades into
    PendingRollbackError for every later query). On any failure it rolls back and
    returns a status dict instead of propagating.
    """
    try:
        return _seed_impl(db, tids)
    except Exception:
        try:
            db.rollback()
        except Exception:
            pass
        logger.exception("seed_demo_discovery: seeding failed (non-fatal, skipped)")
        return {"status": "error"}


def _seed_impl(db, tids):
    now = datetime.utcnow()
    tid = tids[0]
    ...
    # GET-OR-CREATE, not blind insert. The unique constraint is (tenant_id, name);
    # a pre-existing [DEMO] credential (from a prior partial run, or a tenant the
    # previous cleanup scope missed) must be reused, not re-inserted — that
    # collision is exactly what was crashing the seed.
    try:
    ...
    for name, kind, uname, cidrs in [
        ...
        if db.query(CredentialProfile.id).filter(
                ...
                CredentialProfile.name == name).first():
    ...
    # If the demo campaigns already exist for this tenant we're done — recreating
    # them would collide on the campaign name (also unique) and duplicate the
    # runs/observations. Idempotent re-entry.
    if db.query(DiscoveryCampaign.id).filter(
            DiscoveryCampaign.tenant_id == tid,
            DiscoveryCampaign.name.like(f"{TAG}%")).first():
        return {"campaigns": 0, "credentials": "ensured", "note": "already seeded"}
