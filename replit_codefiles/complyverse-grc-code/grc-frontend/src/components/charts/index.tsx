...
export function ChartEmptyState({ title, description, icon }: EmptyStateProps) {
  return (
    <div className="flex flex-col items-center justify-center h-full min-h-[200px] text-center p-6">
      <div className="rounded-xl p-4 mb-4" style={{ backgroundColor: 'var(--color-subtle)' }}>
        {icon || <BarChart3 className="h-8 w-8" style={{ color: 'var(--color-muted)' }} />}
      </div>
      <p className="text-sm font-medium" style={{ color: 'var(--color-muted)' }}>{title}</p>
      {description && (
        <p className="text-xs mt-1 max-w-[200px]" style={{ color: 'var(--color-muted)' }}>{description}</p>
      )}
    </div>
  );
}
...
export function RiskHeatmap({ data, onCellClick }: RiskHeatmapProps) {
  ...
  const getCellStyles = (likelihood: number, impact: number) => {
    const score = likelihood * impact;
    if (score >= 20) return { backgroundColor: 'rgba(155, 28, 28, 0.7)' };
    if (score >= 12) return { backgroundColor: 'rgba(146, 87, 14, 0.7)' };
    if (score >= 6) return { backgroundColor: 'rgba(146, 87, 14, 0.4)' };
    return { backgroundColor: 'rgba(45, 106, 79, 0.5)' };
  };
  ...
  if (data.length === 0) {
    return (
      <ChartEmptyState
        ...
        icon={<AlertTriangle className="h-8 w-8" style={{ color: 'var(--color-muted)' }} />}
      />
    );
  }
  ...
  return (
    <div className="w-full">
      <div className="flex items-end gap-2">
        <div className="flex flex-col justify-between h-[200px] text-xs pr-2" style={{ color: 'var(--color-muted)' }}>
          <span>5</span>
          <span>4</span>
          <span>3</span>
          <span>2</span>
        ...
        <div className="flex-1">
              ...
              row.map((count, colIdx) => {
                ...
                return (
                  <button
                    ...
                    className="aspect-square rounded-md flex items-center justify-center text-xs font-medium transition-all duration-200"
                    style={{
                      ...(count > 0 ? getCellStyles(likelihood, impact) : { backgroundColor: 'var(--color-subtle)' }),
                      color: count > 0 ? 'var(--color-surface)' : 'var(--color-muted)',
                    }}
                  >
                    {count > 0 ? count : ''}
                  </button>
          ...
          <div className="flex justify-between text-xs mt-2 px-1" style={{ color: 'var(--color-muted)' }}>
            <span>1</span>
            <span>2</span>
            <span>3</span>
            <span>4</span>
      ...
      <div className="flex justify-center gap-4 mt-4 text-xs">
        <div className="flex items-center gap-1.5">
          ...
          <span style={{ color: 'var(--color-muted)' }}>Low</span>
        </div>
        <div className="flex items-center gap-1.5">
          <div className="w-3 h-3 rounded" style={{ backgroundColor: 'rgba(146, 87, 14, 0.6)' }} />
          <span style={{ color: 'var(--color-muted)' }}>Medium</span>
        </div>
        <div className="flex items-center gap-1.5">
          <div className="w-3 h-3 rounded" style={{ backgroundColor: 'var(--color-warning)' }} />
          <span style={{ color: 'var(--color-muted)' }}>High</span>
        </div>
        <div className="flex items-center gap-1.5">
          <div className="w-3 h-3 rounded" style={{ backgroundColor: 'var(--color-danger)' }} />
          <span style={{ color: 'var(--color-muted)' }}>Critical</span>
        </div>
      </div>
      <div className="text-center mt-2">
        <span className="text-xs" style={{ color: 'var(--color-muted)' }}>Likelihood ↑ | Impact →</span>
      </div>
    </div>
  );
}
  ...
  if (data.length === 0) {
    return (
      <ChartEmptyState
        ...
        icon={<PieChart className="h-8 w-8" style={{ color: 'var(--color-muted)' }} />}
      />
    );
  }
...
export function TrendLine({ data, color = '#1C2B3A', height = 60, showLabels = true }: TrendLineProps) {
  if (data.length === 0) {
    return (
      <ChartEmptyState
        title="No trend data"
  ...
  return (
    <div className="w-full">
      ...
      {showLabels && (
        <div className="flex justify-between text-[10px] mt-1 px-1" style={{ color: 'var(--color-muted)' }}>
          {data.map((d, i) => (
        ...
        {(centerLabel || centerValue !== undefined) && (
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            {centerValue !== undefined && (
              <span className="text-xl font-semibold" style={{ color: 'var(--color-text)' }}>{centerValue}</span>
            )}
            {centerLabel && (
              <span className="text-xs" style={{ color: 'var(--color-muted)' }}>{centerLabel}</span>
            )}
          </div>
        )}
      </div>
    ...
    <div className="w-full">
      {(label || showPercentage) && (
        <div className="flex justify-between items-center mb-1.5">
          ...
          {showPercentage && <span className="text-sm font-medium" style={{ color: 'var(--color-muted)' }}>{Math.round(percentage)}%</span>}
        </div>
      )}
      <div className={`w-full rounded-full ${heightClasses[size]} overflow-hidden`} style={{ backgroundColor: 'var(--color-subtle)' }}>
        <div
          className={`${heightClasses[size]} rounded-full transition-all duration-500`}
          style={{ width: `${percentage}%`, backgroundColor: colorStyles[color] }}
        />
      </div>
    </div>
  );
...
export function KPICard({ title, value, subtitle, trend, trendValue, icon, color = 'blue', onClick }: KPICardProps) {
  ...
  return (
    <div 
      onClick={onClick}
      className="rounded-xl p-5 transition-all duration-300 shadow-sm hover:shadow-md"
      style={{
    ...
    >
      <div className="flex items-start justify-between">
        <div className="flex-1">
          ...
          {trend && trendValue && (
            <div className="flex items-center gap-1 mt-2 text-xs" style={{ color: trendStyles[trend] }}>
        ...
        {icon && (
          <div className="rounded-xl p-3" style={{ backgroundColor: 'var(--color-subtle)' }}>
            {icon}
          </div>
        )}
      </div>
  ...
  if (total === 0) {
    return (
      <ChartEmptyState
        ...
        icon={<Info className="h-8 w-8" style={{ color: 'var(--color-muted)' }} />}
      />
    );
  }
        ...
        return (
          <div key={key}>
            <div className="flex justify-between items-center text-sm mb-1">
              <span className="capitalize" style={{ color: 'var(--color-text)' }}>{key.replace(/_/g, ' ')}</span>
              <span style={{ color: 'var(--color-muted)' }}>{value} ({Math.round(percentage)}%)</span>
            </div>
