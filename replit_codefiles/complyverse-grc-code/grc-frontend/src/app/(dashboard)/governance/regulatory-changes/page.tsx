    ...
    return (
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="rounded-xl p-5" style={{ border: '1px solid var(--color-border)', backgroundColor: 'var(--color-surface)' }}>
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: 'var(--color-text)' }}>Regulatory Change Management</h1>
          <p className="mt-1" style={{ color: 'var(--color-muted)' }}>Track and manage regulatory changes and their implementation</p>
        </div>
        <button
        ...
        {stats.map((stat) => (
          ...
          >
            <div className="flex items-start justify-between mb-4">
              <div className="flex h-12 w-12 items-center justify-center rounded-xl" style={{ backgroundColor: stat.bgColor }}>
            ...
            <p className="text-sm mt-1" style={{ color: 'var(--color-muted)' }}>{stat.name}</p>
          </div>
        ))}
      </div>

      <div className="rounded-xl" style={{ border: '1px solid var(--color-border)', backgroundColor: 'var(--color-surface)' }}>
        <div className="flex flex-col gap-4 p-4 lg:flex-row lg:items-center lg:justify-between" style={{ borderBottom: '1px solid var(--color-border)' }}>
          <div className="relative flex-1 max-w-md">
            ...
            <input
              ...
              className="w-full rounded-lg pl-10 pr-4 py-2 focus:outline-none focus:ring-1 focus:ring-primary-500"
          ...
          {(!changes || changes.length === 0) ? (
            ...
            <table className="w-full">
              <thead style={{ backgroundColor: 'var(--color-subtle)' }}>
                <tr>
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase" style={{ color: 'var(--color-muted)' }}>Change</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase" style={{ color: 'var(--color-muted)' }}>Source</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase" style={{ color: 'var(--color-muted)' }}>Status</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase" style={{ color: 'var(--color-muted)' }}>Priority</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase" style={{ color: 'var(--color-muted)' }}>Effective Date</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase" style={{ color: 'var(--color-muted)' }}>Gaps</th>
              ...
              <tbody>
                {changes.map((change, idx) => {
                  ...
                  return (
                    <tr key={change.id} className="transition-colors" style={{ borderBottom: idx < changes.length - 1 ? '1px solid var(--color-border)' : undefined }}>
                      <td className="px-4 py-4">
                        <div className="flex items-start gap-3">
                          <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-lg" style={{ backgroundColor: 'rgba(28, 43, 58, 0.08)' }}>
                            <FileWarning className="h-5 w-5" style={{ color: 'var(--color-base)' }} />
                          </div>
                          <div className="min-w-0">
                            <Link href={`/governance/regulatory-changes/${change.id}`} className="font-medium hover:text-primary-400" style={{ color: 'var(--color-text)' }}>
                              {change.title}
                            </Link>
                            {change.reference_number && (
                              <p className="text-xs font-mono mt-0.5" style={{ color: 'var(--color-muted)' }}>{change.reference_number}</p>
                            )}
                          </div>
                        </div>
                      </td>
                      <td className="px-4 py-4">
                        <div className="flex items-center gap-2">
                          <Building2 className="h-4 w-4" style={{ color: 'var(--color-muted)' }} />
                          <span className="text-sm" style={{ color: 'var(--color-text)' }}>{change.source}</span>
                        </div>
                      </td>
                      <td className="px-4 py-4">
                        <span className="inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-medium" style={{ backgroundColor: statusStyle.bg, color: statusStyle.text }}>
                          <StatusIcon className="h-3 w-3" />
                          {change.status.replace(/_/g, ' ')}
                        </span>
                      </td>
                      <td className="px-4 py-4">
                        <span className="inline-flex items-center rounded-full px-2.5 py-1 text-xs font-medium" style={{ backgroundColor: priorityStyle.bg, color: priorityStyle.text }}>
                          {change.priority}
                        </span>
                      </td>
                      ...
                      <td className="px-4 py-4">
                        {(change.gap_count || 0) > 0 ? (
                          <span className="inline-flex items-center gap-1 text-sm" style={{ color: 'var(--color-danger)' }}>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="w-full max-w-2xl rounded-xl p-6 shadow-2xl max-h-[90vh] overflow-y-auto" style={{ border: '1px solid var(--color-border)', backgroundColor: 'var(--color-surface)' }}>
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-semibold" style={{ color: 'var(--color-text)' }}>New Regulatory Change</h2>
              <button
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium mb-1" style={{ color: 'var(--color-text)' }}>Source *</label>
                  <select
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium mb-1" style={{ color: 'var(--color-text)' }}>Regulatory Body</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-sm font-medium mb-1" style={{ color: 'var(--color-text)' }}>Reference Number</label>
                  <input
                    type="text"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium mb-1" style={{ color: 'var(--color-text)' }}>Publication Date</label>
                  <input
                    type="date"
              ...
              <div>
                <label className="block text-sm font-medium mb-1" style={{ color: 'var(--color-text)' }}>Impact Summary</label>
                <textarea
