    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-slate-400">Crosswalk mapping between regulatory frameworks</p>
        </div>
      ...
      <div className="card">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:gap-4">
          <div className="flex-1">
            <label className="mb-1.5 block text-sm font-medium text-slate-300">Source Framework</label>
            <div className="relative">
              <select
                ...
                className="w-full appearance-none rounded-lg border border-slate-600 bg-slate-800 px-4 py-2.5 pr-10 text-white hover:border-slate-500 focus:border-primary-500 focus:outline-none focus:ring-1 focus:ring-primary-500"
              >
          ...
          <div className="flex-1">
            <label className="mb-1.5 block text-sm font-medium text-slate-300">Destination Framework</label>
            <div className="relative">
              <select
        ...
        <div className="card">
          ...
          {crosswalkLoading || crosswalkFetching ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <h3 className="text-lg font-medium text-white">No crosswalk data available</h3>
              <p className="mt-1 text-slate-400">No controls found for the selected frameworks</p>
            </div>
          ) : (
            <>
                ...
                <table className="w-full min-w-[1200px]">
                  <thead>
                    <tr className="border-b border-slate-700">
                      <th className="px-3 py-3 text-left text-xs font-medium uppercase text-slate-400 w-24">Source Ref</th>
                      <th className="px-3 py-3 text-left text-xs font-medium uppercase text-slate-400 w-64">Source Requirement</th>
                      <th className="px-3 py-3 text-left text-xs font-medium uppercase text-slate-400 w-32">Source Domain</th>
                      <th className="px-3 py-3 text-left text-xs font-medium uppercase text-slate-400 w-24">Dest Ref(s)</th>
                      <th className="px-3 py-3 text-left text-xs font-medium uppercase text-slate-400 w-64">Dest Requirement</th>
                      <th className="px-3 py-3 text-left text-xs font-medium uppercase text-slate-400 w-32">Dest Domain</th>
                      <th className="px-3 py-3 text-left text-xs font-medium uppercase text-slate-400 w-48">Evidence</th>
                      <th className="px-3 py-3 text-center text-xs font-medium uppercase text-slate-400 w-20">AI Map</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-700/50">
                    {crosswalkData.crosswalk.map((row) => {
                      ...
                      return (
                        <tr key={row.source_control.id} className="hover:bg-slate-700/20 align-top">
                          <td className="px-3 py-3">
                            <button
                              onClick={() => toggleRow(row.source_control.id)}
                              className="font-mono text-sm font-medium text-primary-400 hover:text-primary-300"
                            >
                              {row.source_control.reference}
                            </button>
                          </td>
                          <td className="px-3 py-3">
                            <p className="text-sm font-medium text-white line-clamp-2">{row.source_control.title}</p>
                            {isExpanded && row.source_control.description && (
                              <p className="mt-1 text-xs text-slate-400 line-clamp-4">{row.source_control.description}</p>
                            )}
                          </td>
                          <td className="px-3 py-3">
                            {row.source_control.domain && (
                            ...
                            {row.source_control.category && row.source_control.category !== row.source_control.domain && (
                              <span className="mt-1 inline-block rounded bg-slate-700/50 px-2 py-0.5 text-xs text-slate-400">
                            ...
                            {hasDest ? (
                              <div className="space-y-1">
                                {row.destination_controls.map((dc) => (
                                  <span key={dc.id} className="block font-mono text-sm text-blue-400">
                                    {dc.reference}
                                  </span>
                                ))}
                                {row.match_count > 5 && (
                                  <span className="text-xs text-slate-500">+{row.match_count - 5} more</span>
                                )}
                              ...
                              <span className="text-xs italic text-slate-500">No mapping found</span>
                            )}
                          </td>
                          <td className="px-3 py-3">
                            {hasDest ? (
                              <div className="space-y-2">
                                {row.destination_controls.slice(0, isExpanded ? 5 : 2).map((dc) => (
                                  <div key={dc.id}>
                                    <p className="text-sm text-slate-200 line-clamp-2">{dc.title}</p>
                                    {isExpanded && dc.description && (
                                      <p className="mt-0.5 text-xs text-slate-400 line-clamp-3">{dc.description}</p>
                                    )}
                                  </div>
                                ))}
                              </div>
                          ...
                          <td className="px-3 py-3">
                            {hasDest && (
                              <div className="space-y-1">
                                {row.destination_controls.slice(0, isExpanded ? 5 : 2).map((dc) => (
                                  dc.domain ? (
                                    <span key={dc.id} className="block rounded bg-slate-700 px-2 py-0.5 text-xs text-slate-300">
                                      {dc.domain}
                                    </span>
                                  ) : null
                                ))}
                          ...
                          <td className="px-3 py-3">
                            {row.evidence_recommendations.length > 0 ? (
                              <div className="space-y-1">
                                ...
                                {!isExpanded && row.evidence_recommendations.length > 2 && (
                                  <span className="text-xs text-slate-500">+{row.evidence_recommendations.length - 2} more</span>
                                )}
                              </div>
                            ) : (
                              <span className="text-xs italic text-slate-500">—</span>
                            ...
                            {aiResult && (
                              <div className="mt-2 rounded border border-purple-500/30 bg-purple-500/10 p-2">
                                <p className="mb-1 text-xs font-medium text-purple-300 flex items-center gap-1">
                                ...
                                {aiResult.ai_mappings.map((m, idx) => (
                                  <div key={idx} className="mb-1 last:mb-0">
                                    ...
                                    {isExpanded && m.rationale && (
                                      <p className="text-xs text-purple-300/60 mt-0.5">{m.rationale}</p>
                                    )}
                                    {m.evidence_recommendations.length > 0 && (
                                      <div className="mt-0.5">
                                        {m.evidence_recommendations.slice(0, 2).map((er, i) => (
                                          <p key={i} className="text-xs text-purple-300/50">• {er}</p>
                                        ))}
                                      </div>
                                    )}
                                  </div>
                          ...
                          <td className="px-3 py-3 text-center">
                            <button
                              ...
                              className="inline-flex items-center gap-1 rounded bg-purple-600/30 px-2 py-1 text-xs font-medium text-purple-300 hover:bg-purple-600/50 disabled:opacity-50"
              ...
              {totalPages > 1 && (
                <div className="mt-4 flex items-center justify-between border-t border-slate-700 px-4 pt-4">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      disabled={page === 0}
                      className="flex items-center gap-1 rounded-lg border border-slate-600 px-3 py-1.5 text-sm text-slate-300 hover:bg-slate-700 disabled:opacity-50"
                    >
      ...
      {!compareTriggered && (
        <div className="card">
          <div className="flex flex-col items-center justify-center py-16 text-center">
            <GitCompare className="mb-4 h-16 w-16 text-slate-600" />
            <h3 className="text-xl font-medium text-white">Framework Crosswalk Comparison</h3>
