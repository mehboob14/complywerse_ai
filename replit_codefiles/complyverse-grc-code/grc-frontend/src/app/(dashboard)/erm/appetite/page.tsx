      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {appetiteConfigs.map(config => {
          ...
          return (
            <div
              key={config.id}
              className={`rounded-xl border ${isEditing ? 'border-primary-500' : 'border-slate-700'} bg-slate-800 overflow-hidden`}
            >
                    ...
                    {isEditing ? (
                      <div className="flex items-center gap-1">
                        <button
                          ...
                          disabled={isSaving}
                          className="p-1 rounded hover:bg-slate-700/50 text-green-400 hover:text-green-300 transition-colors"
                          title="Save"
                        >
                        ...
                        <button
                          ...
                          className="p-1 rounded hover:bg-slate-700/50 text-red-400 hover:text-red-300 transition-colors"
                          title="Cancel"
                        >
                          <X className="h-4 w-4" />
                        </button>
              ...
              <div className="p-4 space-y-3">
                {isEditing ? (
                  <>
                    <div>
                      <label className="text-xs text-slate-400 mb-1 block">Appetite Level</label>
                      <select
                    ...
                    <div>
                      <label className="text-xs text-slate-400 mb-1 block">Tolerance Threshold</label>
                      <input
                        type="number"
                        min="1"
                        max="25"
                    ...
                    <div>
                      <label className="text-xs text-slate-400 mb-1 block">Max Acceptable Score</label>
                      <input
                        type="number"
                        min="1"
                        max="25"
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        ...
                        className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                          editForm.alert_enabled ? 'bg-primary-600' : 'bg-slate-600'
                        }`}
                      >
                        <span
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        rows={2}
                        className="w-full rounded-lg border border-slate-600 bg-slate-900 px-3 py-2 text-sm text-white focus:border-primary-500 focus:outline-none resize-none"
                      />
                    </div>
                  </>
                ) : (
                  <>
                    ...
                    {config.description && (
                      <p className="text-xs text-slate-500 pt-1 border-t border-slate-700">{config.description}</p>
                    )}
                  </>
                )}
                ...
                {showAiSuggestion && (
                  <div className="mt-3 p-3 rounded-lg border border-purple-500/30 bg-gradient-to-br from-purple-500/10 to-blue-500/10">
                    ...
                    <div className="space-y-1 text-xs">
                      <p className="text-slate-300"><span className="text-slate-400">Level:</span> {getAppetiteLevelLabel(aiSuggestion.appetite_level as AppetiteLevel)}</p>
                      <p className="text-slate-300"><span className="text-slate-400">Threshold:</span> {aiSuggestion.tolerance_threshold}</p>
                      <p className="text-slate-300"><span className="text-slate-400">Max Score:</span> {aiSuggestion.max_acceptable_score}</p>
                      <p className="text-slate-400 mt-1">{aiSuggestion.rationale}</p>
                    </div>
                    {!isEditing && (
                      <button
