...
export default function RiskAssessmentsPage() {
  ...
  const uploadMutation = useMutation({
    mutationFn: (formData: FormData) => riskAssessmentApi.uploadExcel(formData),
    onSuccess: (response) => {
      queryClient.invalidateQueries({ queryKey: ['risk-assessments'] });
  ...
  const handleUpload = () => {
    ...
    if (uploadFormData.name) fd.append('name', uploadFormData.name);
    fd.append('assessment_type', uploadFormData.assessment_type);
    fd.append('methodology', uploadFormData.methodology);
    if (uploadFormData.scope) fd.append('scope', uploadFormData.scope);
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="flex items-center gap-2 rounded-lg border border-emerald-500/50 bg-emerald-500/10 px-4 py-2 font-medium text-emerald-400 hover:bg-emerald-500/20"
          >
            <Upload size={18} />
          ...
          {filteredAssessments.map((assessment) => {
            ...
            return (
              <div
                ...
                className="group cursor-pointer rounded-xl border border-slate-700 bg-slate-800 p-5 transition-all hover:border-slate-600 hover:bg-slate-750 hover:shadow-lg"
              >
                <div className="mb-3 flex items-start justify-between">
                  <div className="flex-1 min-w-0">
                    <h3 className="truncate text-base font-semibold text-white group-hover:text-primary-400 transition-colors">
                      {assessment.name}
                    </h3>
          ...
          <div className="mx-4 w-full max-w-lg rounded-xl border border-slate-700 bg-slate-900 shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-700 px-6 py-4">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-lg font-semibold text-white">Upload Risk Register</h2>
              </div>
              <button
            ...
            <div className="space-y-4 px-6 py-4 max-h-[70vh] overflow-y-auto">
              {uploadResult ? (
                <div className="space-y-4">
                  <div className="flex flex-col items-center gap-3 rounded-lg border border-emerald-500/30 bg-emerald-500/10 p-6">
                    ...
                    <div className="flex gap-4 text-sm">
                      <span className="text-emerald-400">{uploadResult.risks_created} created</span>
                      {uploadResult.rows_skipped > 0 && (
                        <span className="text-amber-400">{uploadResult.rows_skipped} skipped</span>
                      )}
                      {uploadResult.rows_errored > 0 && (
                        <span className="text-red-400">{uploadResult.rows_errored} errors</span>
                      )}
                    </div>
                  </div>
                ...
                <>
                    ...
                    >
                      {uploadFile ? (
                        <div className="flex items-center gap-3">
                          ...
                          <button
                            onClick={() => { setUploadFile(null); setUploadFormData(prev => ({ ...prev, name: '' })); }}
                            className="ml-2 rounded p-1 text-slate-400 hover:bg-slate-700 hover:text-white"
                          >
                            <X size={16} />
                          </button>
                        </div>
                        ...
                        <>
                          ...
                          <p className="text-sm text-slate-300">Click to select or drag an Excel file</p>
                  ...
                  <div>
                    <label className="mb-1 block text-sm font-medium text-slate-300">Assessment Name</label>
                    <input
                      type="text"
                      value={uploadFormData.name}
                      onChange={(e) => setUploadFormData({ ...uploadFormData, name: e.target.value })}
                      className="w-full rounded-lg border border-slate-600 bg-slate-800 px-3 py-2 text-white placeholder-slate-400 focus:border-primary-500 focus:outline-none"
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="mb-1 block text-sm font-medium text-slate-300">Assessment Type</label>
                      <select
                  ...
                  <div className="rounded-lg border border-slate-700 bg-slate-800/50 p-3">
                    <p className="text-xs text-slate-400">
                      Expected columns: Risk ID, Risk Category, Risk Title, Risk Description, Risk Owner, 
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="mx-4 w-full max-w-lg rounded-xl border border-slate-700 bg-slate-900 shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-700 px-6 py-4">
              <h2 className="text-lg font-semibold text-white">New Risk Assessment</h2>
              <button
