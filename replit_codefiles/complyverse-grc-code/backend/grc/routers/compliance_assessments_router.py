...
COLUMN_MAPPINGS = {
    ...
    "control_description": ["control", "control description", "description", "requirement", "control statement", "control text", "control requirement", "security control", "checklist item", "control measure", "control measure / activity", "parameter"],
    "assessment_question": ["assessment question", "audit question", "question", "question/parameter", "checklist question", "test procedure", "assessment criteria"],
    ...
    "gaps_identified": ["gap", "gaps", "gaps identified", "gap identified", "finding", "findings", "observations", "observation", "issues", "issue", "audit remarks", "information security remarks", "internal audit"],
    ...
    "timeline": ["timeline", "due date", "target date", "deadline", "completion date", "expected date", "target", "timeline for compliance", "last updated"],
    ...
    "score": ["score", "score (0-5)", "rating score", "assessment score", "control score", "compliance score"],
    "maturity_level": ["maturity level", "maturity", "maturity rating", "capability level", "capability maturity"],
    "framework_source": ["framework", "source framework", "standard", "regulation", "framework name"],
}

COLUMN_KEYWORDS_PRIORITY = [
    ("compliance_status", ["status", "compliance", "compliant"]),
    ("control_description", ["control measure", "control description", "control statement"]),
    ("assessment_question", ["assessment question", "audit question", "question/parameter", "checklist question", "test procedure", "question"]),
    ...
    ("timeline", ["timeline", "due date", "deadline", "target date", "last updated"]),
    ("item_number", ["sr", "s#", "s.no", "no.", "item"]),
    ("priority", ["priority", "severity", "criticality", "risk level", "risk rating"]),
    ("evidence_reference", ["evidence", "documentation"]),
    ("score", ["score", "rating score"]),
    ("maturity_level", ["maturity", "capability"]),
    ("framework_source", ["framework", "standard", "regulation"]),
...
def find_exact_column_mapping(header: str) -> Optional[str]:
    if not header:
        return None
...
def find_keyword_column_mapping(header: str, already_mapped: set) -> Optional[str]:
    if not header:
        return None
    header_lower = header.lower().strip()
...
def build_column_map_from_headers(headers, use_index=False):
    column_map = {}
    mapped_fields = set()
    unmatched_indices = []
    ...
    for idx, header in enumerate(headers):
        if not header:
            continue
        field = find_exact_column_mapping(str(header))
        if field and field not in mapped_fields:
            column_map[field] = idx if use_index else str(header)
    ...
    for idx in unmatched_indices:
        ...
        if not header:
            continue
        field = find_keyword_column_mapping(str(header), mapped_fields)
        if field:
            column_map[field] = idx if use_index else str(header)
            mapped_fields.add(field)

    header_strs = [str(h) for h in headers if h]
    mapped_headers = [str(headers[v]) if use_index else v for v in column_map.values()]
    unmapped = [h for h in header_strs if h not in mapped_headers]
    logger.info(f"Column mapping result: {column_map}")
    if unmapped:
        logger.info(f"Unmapped headers: {unmapped}")
    return column_map
...
def parse_excel_file(file_content: bytes, file_name: str) -> tuple[List[dict], dict]:
    ...
    try:
        if file_name.endswith('.csv'):
            ...
            column_map = build_column_map_from_headers(headers, use_index=False)
            ...
            for row_idx in range(1, min(10, ws.max_row + 1)):
                ...
                if len(row_values) >= 3:
                    ...
                    matches = sum(1 for h in potential_headers if h and (find_exact_column_mapping(str(h)) or find_keyword_column_mapping(str(h), set())))
                    if matches >= 2:
            ...
            column_map = build_column_map_from_headers(headers, use_index=True)
            ...
            for row_idx in range(header_row_idx + 1, ws.max_row + 1):
                ...
                assessment_q = row_data.pop("assessment_question", None)
                if assessment_q:
                    if row_data.get("control_description"):
                        row_data["control_description"] = f"{row_data['control_description']} | Assessment Question: {assessment_q}"
                    else:
                        row_data["control_description"] = assessment_q

                extra_parts = []
                score_val = row_data.pop("score", None)
                if score_val:
                    extra_parts.append(f"Score: {score_val}")
                maturity_val = row_data.pop("maturity_level", None)
                if maturity_val:
                    extra_parts.append(f"Maturity Level: {maturity_val}")
                framework_val = row_data.pop("framework_source", None)
                if framework_val:
                    extra_parts.append(f"Framework: {framework_val}")
                if extra_parts:
                    extra_text = " | ".join(extra_parts)
                    if row_data.get("remarks"):
                        row_data["remarks"] = f"{row_data['remarks']} | {extra_text}"
                    else:
                        row_data["remarks"] = extra_text
...
def extract_row_data(row, column_map: dict, current_area: str) -> dict:
    ...
    assessment_q = item.pop("assessment_question", None)
    if assessment_q:
        if item.get("control_description"):
            item["control_description"] = f"{item['control_description']} | Assessment Question: {assessment_q}"
        else:
            item["control_description"] = assessment_q

    extra_parts = []
    score_val = item.pop("score", None)
    if score_val:
        extra_parts.append(f"Score: {score_val}")
    maturity_val = item.pop("maturity_level", None)
    if maturity_val:
        extra_parts.append(f"Maturity Level: {maturity_val}")
    framework_val = item.pop("framework_source", None)
    if framework_val:
        extra_parts.append(f"Framework: {framework_val}")
    if extra_parts:
        extra_text = " | ".join(extra_parts)
        if item.get("remarks"):
            item["remarks"] = f"{item['remarks']} | {extra_text}"
        else:
            item["remarks"] = extra_text
