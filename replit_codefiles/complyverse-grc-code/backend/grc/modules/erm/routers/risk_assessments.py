...
from ....models import (
    ...
    GRCUser, BusinessUnit, Framework, ITAsset, get_db
)
...
@router.get("/{assessment_id}/cde-assets")
def get_assessment_cde_assets(
    assessment_id: int,
    db: Session = Depends(get_db),
    current_user: GRCUser = Depends(require_auth)
):
    ...
    cde_assets = db.query(ITAsset).options(
        joinedload(ITAsset.owner)
    ).filter(
        ITAsset.tenant_id == assessment.tenant_id,
        ITAsset.cde_environment == True
    ).order_by(ITAsset.criticality.desc(), ITAsset.name).all()

    assessed_risks = db.query(RiskAssessmentRisk).filter(
        RiskAssessmentRisk.assessment_id == assessment_id
    ).all()

    risk_ids = [ar.risk_id for ar in assessed_risks]
    linked_risks = []
    if risk_ids:
        linked_risks = db.query(Risk).filter(Risk.id.in_(risk_ids)).all()
    ...
    return {
        ...
        "total_cde_assets": len(cde_assets),
        "assessed_risks_count": len(assessed_risks),
        "risks_summary": [
            {
                "id": r.id,
                "title": r.title,
...
EXCEL_HEADER_MAPPINGS = {
    ...
    "category": ["risk category", "category", "risk_category", "risk type", "type"],
    "title": ["risk title", "title", "risk name", "name", "risk", "risk_title"],
    ...
    "inherent_likelihood": ["inherent likelihood", "inherent_likelihood", "inh likelihood", "inh. likelihood", "inherent probability"],
    "inherent_impact": ["inherent impact", "inherent_impact", "inh impact", "inh. impact"],
    "inherent_score": ["inherent score", "inherent_score", "inh score", "inh. score", "inherent risk score", "inherent rating"],
    "controls": ["controls", "control", "existing controls", "control description", "current controls", "mitigating controls"],
    "control_effectiveness": ["control effectiveness", "control_effectiveness", "effectiveness", "control rating"],
    "residual_likelihood": ["residual likelihood", "residual_likelihood", "res likelihood", "res. likelihood"],
    "residual_impact": ["residual impact", "residual_impact", "res impact", "res. impact"],
    ...
    "target_date": ["target date", "target_date", "due date", "deadline", "completion date", "target completion"],
}
...
def _map_headers(header_row):
    ...
    for idx, cell in enumerate(header_row):
        ...
        for field, aliases in EXCEL_HEADER_MAPPINGS.items():
            if normalized in [_normalize(a) for a in aliases]:
                col_map[field] = idx
                break
    return col_map
...
async def upload_excel_risk_register(
    ...
    try:
        ...
        if "title" not in col_map:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Could not identify a 'Risk Title' or 'Title' column in the header row. Found headers: " + str([str(c) for c in rows[0] if c])
            )
