...
def add_policy_statement(
    ...
    statement = PolicyStatement(
        ...
        document_id=document_id,
        statement_code=statement_code,
        statement_text=data["statement_text"][:10000],
        statement_summary=data.get("statement_summary", "")[:500] if data.get("statement_summary") else None,
        category=data.get("category"),
        priority=data.get("priority", "medium"),
        is_mandatory=data.get("is_mandatory", True),
        source_section=data.get("source_section"),
        ai_extracted_keywords=data.get("ai_extracted_keywords", []),
        status="active",
...
def update_policy_statement(
    ...
    _create_version_snapshot(db, statement, "manual_edit", current_user.id, data.get("change_reason"))

    if "statement_text" in data:
        statement.statement_text = data["statement_text"][:10000]
    if "statement_summary" in data:
        statement.statement_summary = data["statement_summary"][:500] if data["statement_summary"] else None
    if "category" in data:
        statement.category = data["category"]
    if "priority" in data:
        statement.priority = data["priority"]
    if "is_mandatory" in data:
        statement.is_mandatory = data["is_mandatory"]
    if "source_section" in data:
        statement.source_section = data["source_section"]
