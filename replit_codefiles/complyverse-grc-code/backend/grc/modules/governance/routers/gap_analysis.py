...
from ....models import (
    ...
    UploadedFramework, ParsedFrameworkControl, GRCUser, Risk, get_db, SessionLocal
)
...
def serialize_finding(finding: PolicyGapFinding, db: Session) -> dict:
    ...
    linked_risk = db.query(Risk).filter(Risk.gap_finding_id == finding.id).first()
    if linked_risk:
        linked_risk_id = linked_risk.id
        linked_risk_title = linked_risk.title
    ...
    return {
        ...
        "linked_risk_title": linked_risk_title,
    }
...
def accept_risk(
    ...
    likelihood, impact = severity_map.get(finding.risk_severity or "medium", (3, 3))

    risk_entry = Risk(
        tenant_id=finding.tenant_id,
        title=f"Accepted Gap: {finding.clause_reference} - {finding.clause_title or finding.framework_name}",
        description=(
            f"Gap Finding from {finding.framework_name} framework analysis.\n\n"
            f"Clause: {finding.clause_reference} - {finding.clause_title or ''}\n"
            f"Requirement: {finding.clause_requirement_text or 'N/A'}\n\n"
            f"Gap Description: {finding.gap_description or 'N/A'}\n"
            f"Missing Requirement: {finding.missing_requirement or 'N/A'}\n"
            f"Remediation Recommendation: {finding.remediation_recommendation or 'N/A'}"
        ),
        category="compliance",
        risk_category="compliance",
        risk_sub_category="gap_acceptance",
        owner_id=current_user.id,
        status="accepted",
        treatment_plan=f"Risk Accepted: {acceptance.risk_acceptance_justification}",
        ...
        residual_score=float(likelihood * impact),
