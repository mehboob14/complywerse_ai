...
def gather_framework_context(committee_type: str, user_tenants: list, db: Session) -> dict:
    frameworks = db.query(UploadedFramework).filter(
        UploadedFramework.tenant_id.in_(user_tenants),
        UploadedFramework.upload_status.in_(["published", "parsed", "classified"])
    ).all()
...
def ai_generate_charter(
    ...
    if not context["frameworks"]:
        raise HTTPException(
            status_code=400,
            detail="No published frameworks found. Upload and publish at least one framework to generate an AI charter."
        )
