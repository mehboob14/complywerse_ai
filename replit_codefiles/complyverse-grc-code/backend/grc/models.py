...
class Tenant(Base):
    ...
    compliance_calendar_events = relationship("ComplianceCalendarEvent", back_populates="tenant", cascade="all, delete-orphan")
    policy_exceptions = relationship("PolicyException", back_populates="tenant", cascade="all, delete-orphan")
    evidence_approval_workflows = relationship("EvidenceApprovalWorkflow", back_populates="tenant", cascade="all, delete-orphan")
...
class Risk(Base):
    ...
    gap_finding_id = Column(Integer, ForeignKey("grc_policy_gap_findings.id"), nullable=True, index=True)
    created_at = Column(DateTime, default=datetime.utcnow)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
    ...
    assessment_risks = relationship("RiskAssessmentRisk", back_populates="risk", cascade="all, delete-orphan", overlaps="risk")
    gap_finding = relationship("PolicyGapFinding", foreign_keys=[gap_finding_id], overlaps="linked_risk")
...
class GovernanceDocument(Base):
    ...
    gap_analysis_runs = relationship("PolicyGapAnalysisRun", back_populates="document", cascade="all, delete-orphan")
...
class PolicyStatement(Base):
    ...
    versions = relationship("PolicyStatementVersion", back_populates="statement", order_by="PolicyStatementVersion.version_number")
...
class PolicyStatementVersion(Base):
    ...
    __table_args__ = (
        Index("ix_stmt_version_stmt_id", "statement_id"),
        Index("ix_stmt_version_tenant", "tenant_id"),
    )
...
# =============================================================================
# Policy Gap Analysis Models
# =============================================================================

class PolicyGapAnalysisRun(Base):
    """Tracks each gap analysis run against a document"""
    ...
    run_type = Column(String(50), default="manual")  # manual, automatic, re-run
    ...
    tenant = relationship("Tenant")
    document = relationship("GovernanceDocument", back_populates="gap_analysis_runs")
    uploaded_framework = relationship("UploadedFramework")
    creator = relationship("GRCUser", foreign_keys=[created_by])
...
class PolicyGapFinding(Base):
    """Individual gap finding for a framework clause against a policy document"""
    ...
    clause_reference = Column(String(255), nullable=False)  # e.g., "SAMA 3.1.1"
    ...
    compliance_status = Column(String(50), nullable=False)  # fully_compliant, partially_compliant, not_addressed, not_applicable
    ...
    # Risk rating
    risk_severity = Column(String(20), default="medium")  # critical, high, medium, low
    ...
    override_status = Column(String(50), nullable=True)  # user's override compliance status
    ...
    overrider = relationship("GRCUser", foreign_keys=[overridden_by])
    linked_risk = relationship("Risk", foreign_keys="[Risk.gap_finding_id]", uselist=False, overlaps="gap_finding")
    
    __table_args__ = (
        Index("ix_gap_finding_tenant_doc", "tenant_id", "document_id"),
        Index("ix_gap_finding_run", "analysis_run_id"),
        Index("ix_gap_finding_status", "tenant_id", "remediation_status"),
        Index("ix_gap_finding_framework", "tenant_id", "uploaded_framework_id"),
    )


class ClauseApplicability(Base):
    __tablename__ = "clause_applicability"
    
    id = Column(Integer, primary_key=True, autoincrement=True)
    tenant_id = Column(Integer, ForeignKey("grc_tenants.id"), nullable=False)
    uploaded_framework_id = Column(Integer, ForeignKey("grc_uploaded_frameworks.id"), nullable=False)
    control_id = Column(Integer, ForeignKey("grc_parsed_framework_controls.id"), nullable=False)
    
    is_applicable = Column(Boolean, default=True, nullable=False)
    justification = Column(Text, nullable=True)
    
    status = Column(String(50), default="pending")
    requested_by = Column(Integer, ForeignKey("grc_users.id"), nullable=False)
    requested_at = Column(DateTime, default=datetime.utcnow)
    reviewed_by = Column(Integer, ForeignKey("grc_users.id"), nullable=True)
    reviewed_at = Column(DateTime, nullable=True)
    ...
    __table_args__ = (
        Index("ix_clause_applicability_tenant", "tenant_id"),
        Index("ix_clause_applicability_framework", "uploaded_framework_id"),
        UniqueConstraint("tenant_id", "control_id", name="uq_clause_applicability_control"),
    )
...
class PolicyReviewHistory(Base):
    __tablename__ = "policy_review_history"
    
    id = Column(Integer, primary_key=True, autoincrement=True)
    tenant_id = Column(Integer, ForeignKey("grc_tenants.id"), nullable=False)
    document_id = Column(Integer, ForeignKey("grc_governance_documents.id", ondelete="CASCADE"), nullable=False)
    
    review_type = Column(String(50), default="periodic")
    review_status = Column(String(50), default="pending")
    ...
    __table_args__ = (
        Index("ix_policy_review_tenant", "tenant_id"),
        Index("ix_policy_review_document", "document_id"),
        Index("ix_policy_review_status", "review_status"),
    )
...
class RiskAssessment(Base):
    ...
    name = Column(String(255), nullable=False)
    description = Column(Text, nullable=True)
    assessment_type = Column(String(50), default="periodic")
    ...
    status = Column(String(50), default="draft")
    lead_assessor_id = Column(Integer, ForeignKey("grc_users.id"), nullable=True, index=True)
    business_unit_id = Column(Integer, ForeignKey("grc_business_units.id"), nullable=True, index=True)
    framework_id = Column(Integer, ForeignKey("grc_frameworks.id"), nullable=True, index=True)
    approved_by = Column(Integer, ForeignKey("grc_users.id"), nullable=True)
    approved_at = Column(DateTime, nullable=True)
    ...
    __table_args__ = (
        Index("ix_risk_assessment_tenant", "tenant_id"),
        Index("ix_risk_assessment_status", "tenant_id", "status"),
    )
...
class RiskAssessmentRisk(Base):
    ...
    treatment_decision = Column(String(50), nullable=True)
    rationale = Column(Text, nullable=True)
    control_effectiveness = Column(String(50), nullable=True)
    notes = Column(Text, nullable=True)
    ...
    __table_args__ = (
        Index("ix_ra_risk_assessment", "assessment_id"),
        Index("ix_ra_risk_risk", "risk_id"),
...
class RiskAssessmentKRI(Base):
    ...
    threshold_status = Column(String(20), nullable=True)
    notes = Column(Text, nullable=True)
    created_at = Column(DateTime, default=datetime.utcnow)
    ...
    __table_args__ = (
        Index("ix_ra_kri_assessment_risk", "assessment_risk_id"),
    )
...
class RiskAssessmentIncident(Base):
    ...
    assessment_risk_id = Column(Integer, ForeignKey("grc_risk_assessment_risks.id"), nullable=False, index=True)
    incident_id = Column(Integer, ForeignKey("grc_risk_incidents.id"), nullable=False, index=True)
    impact_on_rating = Column(Text, nullable=True)
    created_at = Column(DateTime, default=datetime.utcnow)
    ...
    __table_args__ = (
        Index("ix_ra_incident_assessment_risk", "assessment_risk_id"),
    )
...
class RiskAssessmentRCSAFinding(Base):
    ...
    __table_args__ = (
        Index("ix_ra_rcsa_finding_assessment_risk", "assessment_risk_id"),
    )


# =============================================================================
# NEW FEATURE MODELS: Compliance Calendar, Policy Exceptions, Evidence Approval
# =============================================================================

class ComplianceCalendarEvent(Base):
    __tablename__ = "grc_compliance_calendar_events"
    ...
    created_at = Column(DateTime, default=datetime.utcnow)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
    recurrence_type = Column(String(20), default="none")
    recurrence_end_date = Column(DateTime, nullable=True)
    metadata_ = Column("metadata", JSON, default={})

    tenant = relationship("Tenant", back_populates="compliance_calendar_events")
    assignee = relationship("GRCUser", foreign_keys=[assigned_to])
    creator = relationship("GRCUser", foreign_keys=[created_by])

    __table_args__ = (
        Index("ix_cal_event_tenant_due", "tenant_id", "due_date"),
        Index("ix_cal_event_tenant_status", "tenant_id", "status"),
        Index("ix_cal_event_tenant_type", "tenant_id", "event_type"),
    )
...
class EvidenceApprovalWorkflow(Base):
    __tablename__ = "grc_evidence_approval_workflows"
    ...
    tenant = relationship("Tenant", back_populates="evidence_approval_workflows")
    evidence = relationship("Evidence")
    creator = relationship("GRCUser", foreign_keys=[created_by])
    steps = relationship("EvidenceApprovalStep", back_populates="workflow", cascade="all, delete-orphan", order_by="EvidenceApprovalStep.step_order")

    __table_args__ = (
        Index("ix_ev_wf_tenant_status", "tenant_id", "status"),
        Index("ix_ev_wf_evidence", "evidence_id"),
    )


class EvidenceApprovalStep(Base):
    __tablename__ = "grc_evidence_approval_steps"

    id = Column(Integer, primary_key=True, index=True)
    workflow_id = Column(Integer, ForeignKey("grc_evidence_approval_workflows.id"), nullable=False, index=True)
    step_order = Column(Integer, nullable=False)
    reviewer_id = Column(Integer, ForeignKey("grc_users.id"), nullable=False, index=True)
    status = Column(String(50), default="pending")
    comments = Column(Text, nullable=True)
    reviewed_at = Column(DateTime, nullable=True)
    created_at = Column(DateTime, default=datetime.utcnow)

    workflow = relationship("EvidenceApprovalWorkflow", back_populates="steps")
    reviewer = relationship("GRCUser")

    __table_args__ = (
        Index("ix_ev_step_workflow_order", "workflow_id", "step_order"),
        Index("ix_ev_step_reviewer", "reviewer_id"),
    )
