...
/**
 * HipaaEphiInventoryTab — the HIPAA "ePHI Inventory" template tab. Mirror of
 * _PciCdeInventoryTab: each row IS a real IT asset flagged as ePHI
 * (`ephi_environment`). Add / Edit reuse the SAME AssetModal as the IT Assets
 * module (forceEphi on), so the form is identical field-for-field — every
 * standard asset field plus the HIPAA ePHI attributes — and everything stays in
 * sync with the Assets module.
 */
...
interface EphiSystem {
  ...
  hipaa?: Record<string, string | null> | null;
}
...
export default function HipaaEphiInventoryTab({ frameworkName }: {
  journeyId?: number;
  ...
  const [confirmDel, setConfirmDel] = useState<EphiSystem | null>(null);

  const { data, isLoading } = useQuery({
    queryKey: ['ephi-systems'],
    queryFn: async () => (await certificationsApi.getEPHISystems()).data as { systems: EphiSystem[]; summary: unknown },
  });
  const systems = data?.systems || [];

  const refresh = () => {
    qc.invalidateQueries({ queryKey: ['ephi-systems'] });
    qc.invalidateQueries({ queryKey: ['assets'] });
  };
  const errMsg = (e: unknown) => (e as { response?: { data?: { detail?: string } } })?.response?.data?.detail || 'Please try again.';

  const createMut = useMutation({
    mutationFn: (payload: Parameters<typeof assetsApi.create>[0]) => assetsApi.create({ ...payload, ephi_environment: true }),
    onSuccess: () => { toast({ type: 'success', title: 'ePHI asset added', message: 'Synced with the Assets module.' }); refresh(); setModalOpen(false); },
  ...
  const updateMut = useMutation({
    ...
    onSuccess: () => { toast({ type: 'success', title: 'ePHI asset updated', message: 'Synced with the Assets module.' }); refresh(); setModalOpen(false); setEditingAsset(null); },
    onError: (e: unknown) => toast({ type: 'error', title: 'Save failed', message: errMsg(e) }),
  });
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="flex items-start gap-2">
          <HeartPulse className="mt-0.5 h-5 w-5 shrink-0 text-primary-600" strokeWidth={1.75} />
          <div>
            <h3 className="text-lg font-semibold text-slate-800">ePHI Inventory</h3>
            <p className="text-xs text-slate-500">
              ePHI assets in scope for {frameworkName || 'HIPAA'}. Managed here or in{' '}
        ...
        <button onClick={openAdd} className="inline-flex items-center gap-1.5 rounded-lg bg-primary-600 px-3 py-2 text-sm font-medium text-white hover:bg-primary-700">
          <Plus className="h-4 w-4" strokeWidth={2} /> Add ePHI asset
        </button>
      </div>

      {/* Sync note */}
      <div className="flex items-start gap-2 rounded-lg border border-indigo-100 bg-indigo-50/50 px-3 py-2 text-xs text-slate-600">
        <ShieldCheck className="mt-0.5 h-4 w-4 shrink-0 text-indigo-600" strokeWidth={1.9} />
        <p>Each row is a real IT asset flagged <span className="font-medium text-slate-800">ePHI Environment</span>. Add / edit uses the same form as the Assets module — every asset field plus the HIPAA ePHI attributes — so changes reflect in both places.</p>
      </div>

      {/* Table */}
      {isLoading ? (
        <div className="flex items-center justify-center py-10 text-sm text-slate-400"><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Loading ePHI assets…</div>
      ) : systems.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-slate-300 bg-slate-50 py-10 text-center">
          <HeartPulse className="mb-2 h-7 w-7 text-slate-300" />
          <p className="text-sm font-medium text-slate-700">No ePHI assets yet</p>
          <p className="mt-1 text-xs text-slate-500">Add the systems that store, process or transmit electronic protected health information.</p>
          <button onClick={openAdd} className="mt-3 inline-flex items-center gap-1.5 rounded-lg bg-primary-600 px-3 py-1.5 text-sm font-medium text-white hover:bg-primary-700"><Plus className="h-4 w-4" /> Add ePHI asset</button>
        </div>
      ) : (
          ...
          <table className="w-full text-left text-sm">
            <thead>
              <tr className="border-b border-slate-100 text-[11px] uppercase tracking-wide text-slate-400">
                ...
                <th className="px-3 py-2.5 font-medium">ePHI data</th>
            ...
            <tbody>
              {systems.map((s) => (
                <tr key={s.id} className="border-b border-slate-50 last:border-0 hover:bg-slate-50/60">
                  ...
                  <td className="px-3 py-2.5 align-top text-slate-700">{s.hipaa?.ephi_data || '—'}</td>
                  <td className="px-3 py-2.5 align-top text-slate-700">{s.hipaa?.encrypted || '—'}</td>
                  <td className="px-3 py-2.5 align-top text-slate-700">{s.hipaa?.retention || '—'}</td>
                  ...
                  <td className="px-3 py-2.5 align-top text-slate-700">{s.hipaa?.assessment || '—'}</td>
      ...
      {/* Add / edit — the SAME AssetModal as the IT Assets module (ePHI pre-set). */}
      {modalOpen && (
        <AssetModal
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={() => setConfirmDel(null)}>
          <div className="w-full max-w-sm rounded-xl bg-white p-5 shadow-xl" onClick={(e) => e.stopPropagation()}>
            <div className="mb-2 flex items-center justify-between">
              <h3 className="text-sm font-semibold text-slate-800">Delete ePHI asset</h3>
              <button onClick={() => setConfirmDel(null)} className="text-slate-400 hover:text-slate-700"><X className="h-4 w-4" /></button>
            </div>
            <p className="text-sm text-slate-600">Delete <span className="font-medium text-slate-800">{confirmDel.name}</span>? This removes it from the ePHI inventory <span className="font-medium">and the Assets module</span> — this can&apos;t be undone.</p>
