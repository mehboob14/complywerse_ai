...
import { SqlDbForm, type SqlPlatformId } from '@/cyber-assurance/components/integrations/SqlDbForm';
...
export const PLATFORMS: Array<{ id: Platform; label: string; logo: string; subtitle: string }> = [
...
export const PLATFORM_GROUPS: Array<{ key: string; title: string; hint: string; ids: Platform[] }> = [
  {
    key: 'hosts',
    title: 'Hosts & servers',
    hint:
...
export default function ConnectWizardPage({
  ...
  /** Compact mode for embedding inside Asset Discovery "Add connection" popup. */
  embedded?: boolean;
  /** Skip the platform picker and open directly on this platform. */
  ...
  // ALL hooks must run unconditionally — permission early-return goes AFTER
  // the hook block. Returning above useState/useEffect reintroduces the
  // "Rendered fewer hooks than expected" crash when perms resolve to denied.
  ...
  useEffect(() => {
    ...
    // Prefer explicit initialPlatform (embedded discovery popup) over URL.
    const boot = initialPlatform || p;
    if (boot && (valid as readonly string[]).includes(boot) && !picked) {
      // Defer to next tick so the empty-state UI doesn't flash first.
      void startWizard(boot as Platform);
    }
  ...
  async function startWizard(p: Platform) {
    ...
    if (p === 'ad') {
      router.push('/cyber-assurance/admin/integrations/connect/ad-discover');
      return;
    }
    setPicked(p);
    setError(null);
  ...
  // opened standalone (no anchor). Embedded mode stays put — the parent
  // popup owns dismissal.
  useEffect(() => {
    if (embedded) return;
  ...
  }, [status?.state, navigate, prefillAssetId, embedded]);

  // Permission gate — AFTER every hook above. Returning earlier crashes React
  // when perms flip from loading → denied (fewer hooks on the next render).
  if (!permsLoading && !canConnect) {
    return (
      <div className={embedded ? 'p-6' : 'min-h-screen bg-slate-50'}>
        <div className={embedded ? '' : 'max-w-2xl mx-auto px-4 py-16'}>
          <div className="bg-white rounded-xl shadow-lg border border-slate-200 p-8 text-center">
            ...
            {!embedded && (
              <button
                onClick={() => navigate('/asset-discovery')}
  ...
  return (
    <div className={embedded ? '' : 'min-h-screen bg-slate-50'}>
      <div className={embedded ? 'px-2 py-2' : 'max-w-3xl mx-auto px-4 py-8'}>
        {!embedded && (
          <div className="text-center mb-8">
            <h1 className="text-3xl font-bold text-slate-900 mb-2">Connect your first server</h1>
            <p className="text-slate-600">
              Complyverse needs at least one connected server to scan against. Pick a platform — we&apos;ll generate a one-liner that sets up everything for you.
            </p>
          </div>
        )}

        {!tokenData && !initialPlatform && (
          <>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="mt-1 text-xs text-slate-600 leading-relaxed">
                    Each connection pairs ONE credential (username + password) with ONE target host. Complyverse encrypts the password with your tenant&apos;s Fernet key and only uses it at scan time.
                  </p>
                  <p className="mt-2 text-xs text-slate-600">
                    <strong>Got assets in IT Assets already?</strong>{' '}
                    Skip this picker — go to{' '}
                    <a href="/cyber-assurance/assets" className="underline font-semibold text-primary-700 hover:text-primary-800">
                      IT Assets
                    </a>{' '}
        ...
        {/* Embedded / URL-bootstrapped: wait for issue-token before the form. */}
        {!tokenData && initialPlatform && (
            ...
            {error ? (
              <div className="text-rose-700">
                ...
                <button
                  ...
                  onClick={() => void startWizard(initialPlatform)}
                >
                  Retry
                </button>
              </div>
            ) : (
              <p>{issuing ? 'Starting connection…' : 'Preparing form…'}</p>
            )}
          </div>
        )}
        ...
        {tokenData && status?.state !== 'ready' && picked === 'aws' && (
          <AwsForm
            ...
            onCancel={() => {
              setTokenData(null); setPicked(null); setStatus(null);
              if (initialPlatform) void startWizard(initialPlatform);
            }}
          />
        )}
        ...
        {tokenData && status?.state !== 'ready' && picked && !['windows', 'linux', 'digitalocean', 'digitalocean_api', 'aws', 'postgres', 'mssql', 'mysql', 'oracle', 'snmp'].includes(picked) && (
          <div className="bg-white rounded-xl shadow-md p-8 border border-amber-200 max-w-3xl mx-auto text-center">
            ...
            <p className="text-xs text-slate-500 mb-4">
              ...
              {' '}<a href="/cyber-assurance/integrations/connections" className="underline">Integrations → Connections</a> page —
...
function ManualCredsForm({
  ...
  const userHint = platform === 'windows'
    ? 'Service account on the Windows server (e.g., ava_scanner)'
    : 'Linux user with read-only audit rights (e.g., ava_scanner)';
  ...
  if (success) {
    return (
      <div className="bg-white rounded-xl shadow-md p-8 border-2 border-emerald-300 text-center">
        ...
        <button
          onClick={() => window.location.href = '/asset-discovery'}
            ...
            <div className="grid grid-cols-1 gap-4">
              <div>
                ...
                <input
                  ...
                  placeholder={platform === 'windows' ? 'ava_scanner' : 'audit_readonly'}
                  required
                  autoComplete="off"
                  spellCheck={false}
                  className={inputCls + " font-mono"}
  ...
  if (success) {
    return (
      <div className="bg-white rounded-xl shadow-md p-8 border-2 border-emerald-300 text-center">
        ...
        <p className="text-slate-600 mb-4">Complyverse discovered your account resources (droplets, volumes, VPCs, databases, Kubernetes…). Open the asset to see them.</p>
  ...
  if (success) {
    return (
      <div className="bg-white rounded-xl shadow-md p-8 border-2 border-emerald-300 text-center">
        ...
        <p className="text-slate-600 mb-4">Complyverse can now run read-only CIS AWS Foundations scans against this account.</p>
    ...
    <div className="bg-white rounded-xl shadow-md p-6 border border-slate-200">
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          ...
          <p className="text-xs text-slate-600">
            Paste an IAM <strong>ReadOnlyAccess</strong> key. Complyverse never modifies your AWS — only reads config for scanning.
          </p>
        </div>
      </div>
      ...
      <details className="mb-4 text-xs text-slate-600 bg-primary-50 rounded-lg p-3 border border-primary-200">
        ...
        <ol className="list-decimal ml-5 mt-2 space-y-1">
          ...
          <li>Username: <code className="bg-white px-1 rounded">ava-scanner</code></li>
