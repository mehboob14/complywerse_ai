...
import { ctemScopesApi, vulnManagementApi, apiClient } from '@/cyber-assurance/lib/api';
  ...
  const renderScope = () => (
    <>
      {sectionDesc(<>A scope is a fixed set of assets. Every stage below counts findings on <b>these machines only</b>; membership freezes into each closed cycle.</>,
        <><Btn sm onClick={() => notify('Edit scope assets — searchable multi-select (coming soon).')}>＋ Edit assets</Btn><Link href="/cyber-assurance/assets" style={{ textDecoration: 'none' }}><Btn sm>Open in Inventory →</Btn></Link></>)}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(min(250px,100%),1fr))', gap: 10 }}>
        {s.machines.map((m) => {
          ...
          return (
            <Link key={m.id} href={`/cyber-assurance/assets/${m.id}`} style={{ textDecoration: 'none', color: 'inherit', border: `1px solid ${BORDER}`, borderRadius: 11, padding: '10px 13px', display: 'block' }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
            ...
            <tbody>
              ...
              {rows.map((v: any) => {
                ...
                return (
                  <tr key={v.id} onClick={() => router.push(`/cyber-assurance/vulnerabilities/${v.id}`)} style={{ cursor: 'pointer' }}>
          ...
          : (showAllRanked ? ranked : ranked.slice(0, 12)).map((v: any, i: number) => {
            ...
            return (
              <div key={v.id} style={{ display: 'flex', alignItems: 'center', gap: 13, borderBottom: `1px solid ${BORDER2}`, padding: '11px 4px' }}>
                ...
                <Link href={`/cyber-assurance/vulnerabilities/${v.id}`} style={{ flex: 1, minWidth: 0, textDecoration: 'none', color: 'inherit' }}>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,.42)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, zIndex: 60 }} onClick={(e) => { if (e.target === e.currentTarget) setPeek(null); }}>
          <div style={{ ...CARD, width: 520, maxWidth: '100%', padding: 20, boxShadow: '0 20px 50px rgba(2,6,23,.3)', maxHeight: '85vh', overflowY: 'auto' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 14 }}>
              <a href={`/cyber-assurance/vulnerabilities/${peek.id}`} style={{ fontSize: 11.5, fontWeight: 600, color: ACS, textDecoration: 'none' }}>Open full finding →</a>
            </div>
          </div>
        </div>
      )}
