...
const AC = '#005B96', ACSTRONG = '#014A81';
...
const BAND: Record<BandKey, { label: string; bar: string; fg: string; bg: string }> = {
  ...
  contained: { label: 'Contained', bar: '#005B96', fg: '#1F7A54', bg: '#E7F5EE' },
...
const sevCol = (pct: number) => pct >= 75 ? '#C2453F' : pct >= 50 ? '#DB7B45' : pct >= 25 ? '#E0AF33' : '#005B96';
...
export default function AssetPostureDetail({ assetId }: { assetId: number }) {
  ...
  const goBack = () => {
    ...
    else router.push('/cyber-assurance/risk-posture');
  };
    ...
    const extraPill = (
      <>
        ...
        {d.subdomain_rollup && d.subdomain_rollup.count > 0 && (
          <span title="This score includes a weighted Subdomain-exposure component" style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 11, fontWeight: 600, padding: '2px 9px', borderRadius: 999, background: '#EFF5FA', color: '#014A81' }}>+{d.subdomain_rollup.count} subdomains in score</span>
        )}
      </>
    );
    ...
    return (
      <div className="inv2" style={root}>
        ...
        {shell(extraPill, host, subLine, (
          <>
            <Link href={`/cyber-assurance/assets/${d.asset?.id}`} style={btnSm}>Open asset record <ExternalLink size={13} /></Link>
...
<b>Externally-discovered</b> — scored on outside-in exposure (TLS, headers, transport, email auth, known CVEs), <b>not</b> CIA / CIS / controls.
        ...
        <Card title="Why this score" sub={`weighted outside-in signals · total ${data.score ?? '—'}/100 · bar = signal severity, number = points added`} grow>
          {comps.length === 0 ? (
            ...
            const link = k === 'exploitability' ? <Link href="/cyber-assurance/vulnerabilities" style={lnkStyle}>Findings →</Link>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(min(280px,100%),1fr))', gap: 12, alignItems: 'stretch' }}>
          ...
          <Card title="Outside-in probe" sub={probe.probed_at ? `last sweep · ${String(probe.probed_at).slice(0, 10)}` : 'not yet probed'} grow>
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 10, flexWrap: 'wrap' }}>
              <Link href="/cyber-assurance/vulnerabilities" style={lnkStyle}>Exposed-service findings →</Link>
  ...
  const dimLink = (k: 'vuln' | 'cis' | 'cia' | 'ctrl' | 'risk', muted: boolean): ReactNode => {
    switch (k) {
      case 'vuln': return <Link href={`/cyber-assurance/assets/${asset.id}?tab=vulnerabilities`} style={lnkStyle}>Findings →</Link>;
      case 'cis': return <Link href={`/cyber-assurance/compliance-plugins/asset/${asset.id}`} style={lnkStyle}>CIS →</Link>;
      case 'cia': return <button style={lnkBtn} onClick={() => setOpen(true)}>{muted ? 'Set ratings →' : 'Edit →'}</button>;
      case 'ctrl': return <Link href={`/cyber-assurance/assets/${asset.id}`} style={lnkStyle}>Link →</Link>;
  ...
  return (
    <div className="inv2" style={root}>
      ...
      {shell(extraPill, host, subLine, (
        <>
          <Link href={`/cyber-assurance/assets/${asset.id}`} style={btnSm}>Open asset record <ExternalLink size={13} /></Link>
      ...
      <div style={{ ...card, marginBottom: 12 }}>
        <div onClick={() => setOpen(true)} style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap', padding: '10px 14px', cursor: 'pointer' }}>
          <span style={{ width: 30, height: 30, borderRadius: 9, background: '#EFF5FA', color: ACSTRONG, display: 'grid', placeItems: 'center', flex: 'none' }}><SlidersHorizontal size={15} /></span>
          <div style={{ flex: 1, minWidth: 200 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(min(300px,100%),1fr))', gap: 12, alignItems: 'stretch' }}>
        ...
        <Card title="Top contributors" sub="the findings adding the most risk — fix these to move the score" grow>
          {topContrib.length === 0 ? (
            <div style={{ fontSize: 11.5, color: MUTED }}>{comp.vuln.active_count > 0 ? <Link href={`/cyber-assurance/assets/${asset.id}?tab=vulnerabilities`} style={lnkStyle}>See this asset&apos;s findings →</Link> : 'No active findings contributing to this score.'}</div>
          ) : topContrib.map((v) => {
            const vb = BAND[bandOf(v.band, Math.round(v.score * 100))];
            return <ContribRow key={v.vuln_id} dot={vb.bar} title={<span><span className="mono">{v.cve_id || `VULN-${v.vuln_id}`}</span>{v.title ? ` · ${v.title}` : ''}</span>} meta={`${v.severity || vb.label}${v.kev_flag ? ' · KEV' : ''}${v.epss_score != null ? ` · EPSS ${Math.round(Number(v.epss_score) * 100)}%` : ''} · ${(v.score * 10).toFixed(1)}/10`} href={`/cyber-assurance/vulnerabilities/${v.vuln_id}`} />;
          })}
