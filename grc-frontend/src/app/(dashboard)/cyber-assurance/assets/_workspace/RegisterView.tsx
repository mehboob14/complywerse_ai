...
import {
  ...
  type ColumnDef,
  type BulkAction,
} from '@/cyber-assurance/components/ui';
...
// hidden. Apex = registrableDomain from @/cyber-assurance/lib/domains (full Public Suffix List).
  ...
  const columns: ColumnDef<ITAsset>[] = [
    ...
    },
    {
      ...
      render: (a) => {
        ...
        const col = k === 'critical' ? { fg: '#7A2D17', bg: '#F7E4DC' } : k === 'high' ? { fg: '#8A4A0F', bg: '#F6E8D4' } : k === 'medium' ? { fg: '#8A4A0F', bg: '#F4ECD2' } : { fg: '#014A81', bg: '#EFF5FA' };
    ...
    },
    {
      ...
      render: (a) => {
        ...
        const st = k === 'planned' ? { n: 1, c: '#014A81' } : (k === 'maintenance' || k === 'aging') ? { n: 3, c: '#B08420' } : (k === 'decommissioned' || k === 'retired' || k === 'retiring') ? { n: 4, c: '#C2542E' } : { n: 2, c: '#014A81' };
    ...
    },
    {
      ...
      render: (a) => {
        ...
        const m: Record<string, { c: string; label: string }> = { active: { c: '#014A81', label: 'Active' }, inactive: { c: '#55606B', label: 'Inactive' }, decommissioned: { c: '#C2542E', label: 'Decommissioned' } };
    ...
    },
    {
      ...
      render: (a) => a.criticality_score != null
        ? <span className="as-mono" style={{ fontSize: 12, fontWeight: 600, color: a.criticality_score >= 7.5 ? '#C2542E' : a.criticality_score >= 5 ? '#B08420' : '#014A81' }}>{a.criticality_score.toFixed(1)}</span>
        : <span style={{ color: 'var(--as-disabled)' }}>—</span>,
    },
    {
      id: 'actions',
    ...
    return (
      <button
        ...
        style={{
          ...
          border: '1px solid ' + (active ? '#014A81' : 'var(--as-border)'),
          background: active ? '#014A81' : '#fff', color: active ? '#fff' : 'var(--as-ink)', cursor: 'pointer',
        }}
      >
