...
import { assetsApi, softwareIdentifiersApi, compliancePluginsApi, entityExtrasApi } from '@/cyber-assurance/lib/api';
  ...
  const Stat = ({ present, label, products }: { present: boolean; label: string; products: string[] }) => (
    <div style={{ flex: 1, minWidth: 160, padding: '12px 14px', border: '1px solid var(--as-border)', borderRadius: 10,
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
        {present
          ? <ShieldCheck className="h-4 w-4" strokeWidth={2} style={{ color: '#014A81' }} />
          : <ShieldAlert className="h-4 w-4" strokeWidth={2} style={{ color: '#8A5A00' }} />}
        <span style={{ fontSize: 12.5, fontWeight: 700, color: present ? '#014A81' : '#6E5410' }}>
  ...
  return (
    <div>
      <PanelHead
        ...
        note="Software detected on this asset. Click a row to set it up as its own asset (credentials when needed)."
        right={<>
                  ...
                  >
                    ...
                    <td style={{ padding: '9px 12px' }}>
                      <span className="as-pill" style={ok
                        ? { background: 'var(--as-good-bg)', color: '#014A81' }
    ...
    <div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
          ...
          {items.map((r) => (
            <div key={r.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 13px', border: '1px solid var(--as-border)', borderRadius: 9, background: 'var(--as-card)' }}>
              ...
              <Link href={`/cyber-assurance/assets/${r.other_asset_id}`} style={{ fontSize: 13, fontWeight: 600, color: 'var(--as-ink)', textDecoration: 'none' }}>
      ...
      <div style={{ marginTop: 22, paddingTop: 18, borderTop: '1px solid var(--as-divider)' }}>
        ...
        {peers.length === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
            {peers.map((p: any) => (
              <Link key={p.id} href={`/cyber-assurance/assets/${p.id}`} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 13px', border: '1px solid var(--as-border)', borderRadius: 9, background: 'var(--as-card)', textDecoration: 'none' }}>
                <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--as-ink)' }}>{p.name ?? `Asset #${p.id}`}</span>
                {p.score != null && (
                  <span className="as-mono" style={{ marginLeft: 'auto', fontSize: 12, color: p.score >= 80 ? '#014A81' : p.score >= 55 ? '#B08420' : '#A33B1F', fontWeight: 600 }}>{p.score.toFixed(1)}%</span>
                )}
      ...
      <PanelHead
        ...
        right={
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <span className="as-pill" style={{ background: filled === rows.length ? 'var(--as-good-bg)' : 'var(--as-warn-bg)', color: filled === rows.length ? '#014A81' : '#6E5410' }}>
          ...
          {items.map((r: any, i: number) => {
            ...
            return (
              <div key={r.id ?? i} style={{ display: 'flex', gap: 12, padding: '12px 2px', borderBottom: i < items.length - 1 ? '1px solid var(--as-divider)' : 'none' }}>
                <span style={{ width: 8, height: 8, borderRadius: '50%', marginTop: 6, flex: 'none', background: ok ? '#014A81' : '#A33B1F' }} />
