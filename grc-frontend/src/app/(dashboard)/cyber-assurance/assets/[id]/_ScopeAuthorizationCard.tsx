...
const AC = '#005B96', ACS = '#014A81', AMBER = '#9A6410', AMBER_BG = '#FBF2DF';
...
function scopeToken(root: string): string {
  let h = 2166136261 >>> 0;
  const s = root.toLowerCase() + '::ava-scope';
...
export default function ScopeAuthorizationCard({ asset, canManage }: { asset: any; canManage: boolean }) {
  ...
  const txtHost = `_ava-scope.${root || defaultRoot}`;
  const txtValue = `ava-site-verification=${token}`;
