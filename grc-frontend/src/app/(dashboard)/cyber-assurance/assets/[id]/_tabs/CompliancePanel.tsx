...
import { apiClient, assetsApi, compliancePluginsApi } from '@/cyber-assurance/lib/api';
import { GuideMarker } from '@/cyber-assurance/components/guide';
import { useRoomScan } from '../_room-scan-context';
import { softwareKeyToSqlPlatform } from '@/cyber-assurance/components/integrations/SqlDbForm';
import ManualChecksPanel from './ManualChecksPanel';

/* eslint-disable @typescript-eslint/no-explicit-any */

/* ── design tokens, verbatim from the mock's :root ──
   --ac #005B96 · --ac-strong #014A81 · --ac-soft #EFF5FA · --blue #2E63A8 /
...
const BTN_AC = 'border border-[#005B96] bg-[#005B96] text-[#06342B] font-semibold hover:bg-[#014A81] disabled:opacity-50 disabled:cursor-not-allowed';
...
function Callout({ tone, children }: { tone: 'warn' | 'info' | 'error' | 'success'; children: React.ReactNode }) {
  const map: Record<string, { border: string; bg: string; dot: string }> = {
    ...
    info: { border: '#B3CDE0', bg: '#EFF5FA', dot: '#014A81' },
...
function Mchip({ ac, children, title }: { ac?: boolean; children: React.ReactNode; title?: string }) {
  return (
    <span title={title} className={'inline-flex items-center rounded-lg border px-2 py-0.5 text-[9.5px] font-semibold max-w-[240px] truncate ' + MONO + ' ' + (ac ? 'bg-[#EFF5FA] text-[#014A81] border-[#B3CDE0]' : 'bg-[#F1F4F6] text-[#3A4653] border-[#E4E8EC]')}>
      {children}
    </span>
  );
}
...
export default function CompliancePanel({ asset }: { asset: any }) {
  ...
  return (
    <div className={wrap}>
                  ...
                  <div className="mt-3 flex flex-wrap items-center gap-2">
                    {hostInGroup ? (
                      <Link
                        ...
                        className="inline-flex items-center gap-1.5 h-[33px] rounded-[9px] bg-[#005B96] px-3.5 text-[12.5px] font-semibold text-[#06342B] border border-[#005B96] hover:bg-[#014A81]"
                      >
      ...
      {selfIpPeersQ.data && !selfIsConnected && !isBrowserAsset && (
        <div className={CARD + ' border-l-[3px] border-l-[#005B96] overflow-hidden'}>
          <div className="px-4 py-3.5">
            <div className="flex items-start gap-3">
              <div className="mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-[10px] bg-[#EFF5FA] text-[#014A81]">
                <Network className="h-[18px] w-[18px]" />
              </div>
              <div className="min-w-0 flex-1">
        ...
        {scanProgress.running && scanProgress.total > 0 && (
          <div className="px-[18px] pb-[15px]">
            ...
            <div className="h-[9px] rounded-full bg-[#EAEEF1] overflow-hidden">
              <i className="block h-full rounded-full" style={{ width: `${progressPct}%`, background: 'linear-gradient(90deg,#005B96,#014A81)' }} />
            </div>
      ...
      <div className="inline-flex w-fit bg-[#EAEEF1] rounded-[11px] p-[3px] gap-0.5">
        {([
          ...
          <button
            ...
            className={'h-8 px-3.5 rounded-[9px] text-[12px] font-semibold inline-flex items-center gap-1.5 ' + (cTab === key ? 'bg-white text-[#014A81] shadow-[0_1px_2px_rgba(16,24,40,0.06)]' : 'text-[#6B7787] hover:text-[#3A4653]')}
          >
            {label}
            ...
            <>
              ...
              {swWithBench.map((s: any) => (
                <div key={s.id} className="flex items-center gap-3 px-[18px] py-[11px] border-b border-[#F0F3F5]">
                  ...
                  <div className="flex-1 min-w-0">
                    <Link href={`/cyber-assurance/assets/${s.id}?tab=compliance`} className="text-[12px] font-semibold text-[#0F1F2B] hover:text-[#014A81]">{s.name}</Link>{' '}
              ...
              {hostBench.map((s: any, i: number) => (
                <div key={'hb' + (s.software_key || s.name || i)} className="flex items-center gap-3 px-[18px] py-[11px] border-b border-[#F0F3F5]">
                  ...
                  <Link href={softwareSetupHref(s)} className="shrink-0 inline-flex items-center gap-1 rounded-[8px] bg-[#014A81] px-2.5 py-1.5 text-[11px] font-semibold text-white hover:bg-[#0d8a72]" title="Set up a read-only credential for this software so its CIS rules can scan.">Set up scan →</Link>
                </div>
              ))}
            </>
          )}
        ...
        <>
            ...
            <article className={CARD}>
              ...
              {benchmark ? (
                <div className="px-4 pt-3.5 pb-3">
                  ...
                  <div className="flex items-baseline gap-2 mt-1 mb-2.5">
                    <b className={'text-[30px] font-semibold text-[#014A81] leading-none ' + MONO}>{ruleCount.toLocaleString()}</b>
                    <span className="text-[11.5px] text-[#8A95A1]">
                  ...
                  <div className={'bg-[#EFF5FA] border border-[#B3CDE0] rounded-[10px] px-[13px] py-[9px] text-[11px] font-semibold text-[#014A81] flex items-center gap-2 ' + MONO}>
              ...
              <div className="px-4 pt-1.5 pb-2.5 flex flex-col gap-1.5">
                {swWithBench.length === 0 && hostBench.length === 0 ? (
                  ...
                  <>
                    {swWithBench.map((s: any) => (
                      <label key={s.id} className="flex items-center gap-2.5 border border-[#E8ECEE] rounded-[10px] px-3 py-2 text-[11.5px] cursor-pointer hover:border-[#005B96]">
                        <input
                          type="checkbox"
                          checked={roomScan.isSelected(s.id)}
                          onChange={() => roomScan.togglePeer(s.id)}
                          style={{ accentColor: '#014A81', width: 15, height: 15 }}
                        />
                        <b className="font-semibold flex-1 min-w-0 truncate">{s.name}</b>
          ...
          <article className={CARD}>
            ...
            <div className="grid gap-3 px-4 py-3.5" style={{ gridTemplateColumns: 'repeat(auto-fit,minmax(190px,1fr))' }}>
              ...
              <div className="bg-[#F7F9FA] rounded-[11px] px-[15px] py-[13px]">
                <b className={'text-[21px] font-semibold text-[#014A81] ' + MONO}>
              ...
              <div className="bg-[#EFF5FA] rounded-[11px] px-[15px] py-[13px]">
                <b className={'text-[21px] font-semibold text-[#014A81] ' + MONO}>{applyTotal.toLocaleString()}</b>
                <span className="block text-[11px] text-[#014A81] font-semibold mt-0.5">Applicable to scan</span>
          ...
          {scanProgress.running && (
            <div className="px-[18px] py-3.5 border-b border-[#F0F3F5]">
              <div className="flex items-center gap-2.5 text-[12px]">
                <span className="w-[9px] h-[9px] rounded-full bg-[#005B96] animate-pulse shrink-0" />
              ...
              <div className="h-[7px] rounded-full bg-[#EAEEF1] overflow-hidden mt-2.5">
                <i className="block h-full rounded-full bg-[#005B96]" style={{ width: `${progressPct}%` }} />
              </div>
            </div>
          )}
          ...
          {sessions.length === 0 ? (
            ...
            return (
              <div key={session.id} className="flex items-center gap-[11px] px-[18px] py-[13px] border-b border-[#F0F3F5] last:border-b-0">
                <span className="w-[9px] h-[9px] rounded-full shrink-0" style={{ background: running ? '#D8A93B' : '#014A81' }} />
                <div className="flex-1 min-w-0">
          ...
          {suggestion.benchmark_name ? (
            <>
              ...
              <p className="mt-2 text-[11px] text-[#8A95A1]">
                ...
                <Link href="/cyber-assurance/compliance-plugins/os-registry" className="text-[#014A81] underline">admin → mappings</Link>
            ...
            <p className="mt-1 text-[12px] text-[#3A4653]">
              ...
              upload it via{' '}
              <Link href="/cyber-assurance/compliance-plugins/ingest" className="text-[#014A81] underline">Rules library → Ingest</Link>{' '}
