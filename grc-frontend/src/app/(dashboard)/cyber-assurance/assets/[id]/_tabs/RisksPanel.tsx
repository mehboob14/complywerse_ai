...
import { assetsApi, riskPostureApi } from '@/cyber-assurance/lib/api';
import { GuideMarker } from '@/cyber-assurance/components/guide';
import { InlineLinkPicker, PageLoader } from '@/cyber-assurance/components/ui';
...
const BTN_PRIMARY = 'inline-flex items-center gap-1.5 text-[12px] font-semibold px-3.5 py-2 rounded-[9px] whitespace-nowrap border bg-[#005B96] text-[#06342B] border-[#005B96] hover:bg-[#014A81] disabled:opacity-50';
...
const BAND: Record<'high' | 'medium' | 'low', { headerBg: string; pill: string }> = {
  high:   { headerBg: 'bg-[#EFF5FA]', pill: 'text-[#014A81] bg-[#EFF5FA]' },
...
function ResidualRiskCard({ assetId, asset }: { assetId: number; asset: any }) {
  ...
  const card = (
    <BigCard
      ...
      right={
        <Link href={`/cyber-assurance/risk-posture/asset/${assetId}`} className="flex items-center gap-1 text-[11px] font-semibold text-[#014A81] hover:text-[#005B96] whitespace-nowrap mt-px">
...
function FindingsBanner({ assetId }: { assetId: number }) {
  ...
  return (
    <Link href="/cyber-assurance/vulnerabilities" className={CARD + ' flex items-center gap-3 px-4 py-[14px] hover:border-[#AEB8C2] transition-colors'}>
      ...
      <span className="text-[12px] font-semibold text-[#014A81] whitespace-nowrap flex items-center gap-1 flex-none">
...
function CIACard({ assetId, asset }: { assetId: number; asset: any }) {
  ...
  return (
    ...
    >
      ...
      {rationale && (
        <p className="mt-3 rounded-lg bg-[#EFF5FA] px-3 py-2 text-[12px] leading-relaxed text-[#0F1F2B]">
          <b>AI:</b> {rationale}
        </p>
      )}
  ...
  return (
    <BigCard
      ...
      right={onOpenCompliance ? (
        <button onClick={onOpenCompliance} className="flex items-center gap-1 text-[11px] font-semibold text-[#014A81] hover:text-[#005B96] whitespace-nowrap">
                ...
                <div className="py-[30px] px-4 text-center">
                  ...
                  <div className="text-[12px] text-[#8A95A1] inline-flex items-center gap-1.5 flex-wrap justify-center">
                    ...
                    <InlineLinkPicker
                      ...
                      triggerIcon={<></>}
                      triggerClassName="text-[12px] font-semibold text-[#014A81] hover:text-[#005B96]"
                      items={controlPickerItems}
                      isLoading={controlsLoading || isLinkingControl}
                      emptyText="No controls available"
                      searchPlaceholder="Search controls"
                  ...
                  <tr key={`${c.kind}-${c.id}-${idx}`} className="border-b border-[#F0F3F5] last:border-0 hover:bg-[#F4F6F7]">
                    <td className="px-4 py-2.5">
                      <div className="flex items-center gap-2 justify-between">
                        <div className="min-w-0">
                          {c.code && <span className={'block text-[10.5px] font-bold text-[#014A81] ' + MONO}>{c.code}</span>}
  ...
  return (
    <div className="space-y-3">
      <h3 className="flex items-center gap-2 text-[13px] font-semibold text-[#0F1F2B]">
        <AlertTriangle size={16} className="text-[#014A81]" />
        Associated Risks
        ...
        <div className="space-y-2">
          {asset.linked_risks.map((risk) => (
            <div key={risk.risk_id} className="flex items-center justify-between gap-3 rounded-xl border border-[#F0F3F5] bg-[#F4F6F7] px-3.5 py-2.5">
              ...
              <Link href={`/erm/risks/${risk.risk_id}`} className="text-[12px] font-semibold text-[#014A81] hover:text-[#005B96] whitespace-nowrap">
                View Details
              </Link>
            </div>
          ))}
    ...
    <div className="space-y-4">
      {bannerMessage && (
        <div className="flex items-center justify-between rounded-lg bg-[#E7F5EE] px-3 py-2 text-[13px] text-[#1F7A54]">
          ...
          <button type="button" onClick={() => setBannerMessage(null)} className="text-[#1F7A54] hover:text-[#014A81]" aria-label="Dismiss">
            <X className="h-4 w-4" />
          </button>
        </div>
      )}
        ...
        <div className="flex flex-wrap items-center gap-3">
          <label className="flex items-center gap-2 text-[12px] text-[#3A4653]">
            ...
            <select
              ...
              className="rounded-[9px] border border-[#E8ECEE] bg-white px-2 py-1 text-[12px] text-[#0F1F2B] focus:border-[#005B96] focus:outline-none"
            >
          ...
          <label className="flex items-center gap-2 text-[12px] text-[#3A4653]">
            Min score
            <input type="range" min={1} max={12} value={minScore} onChange={(e) => setMinScore(Number(e.target.value))} className="h-1 w-32 accent-[#005B96]" />
            <span className="w-6 text-center font-semibold text-[#0F1F2B]">{minScore}</span>
          </label>
          <label className="flex items-center gap-2 text-[12px] text-[#3A4653]">
            <input type="checkbox" checked={includeLinked} onChange={(e) => setIncludeLinked(e.target.checked)} className="h-3.5 w-3.5 rounded border-[#E8ECEE] text-[#005B96] focus:ring-[#005B96]" />
          ...
          return (
            <div key={band} className="overflow-hidden rounded-[15px] border border-[#E8ECEE] bg-white">
              <div className={`flex items-center justify-between border-b border-[#F0F3F5] ${cls.headerBg} px-3 py-2`}>
                ...
                <button type="button" onClick={() => selectAllOfConfidence(band)} className="text-[12px] font-semibold text-[#014A81] hover:underline">
                  Select all
                </button>
              </div>
              <ul className="divide-y divide-[#F0F3F5]">
                {list.map((r) => {
                  ...
                  return (
                    <li key={r.framework_control_id} className={isSelected ? 'bg-[#EFF5FA]' : ''}>
                      <div className="flex items-start gap-3 px-3 py-2.5">
                        <input
                          type="checkbox"
                          checked={isSelected}
                          onChange={() => toggle(r.framework_control_id)}
                          className="mt-1 h-4 w-4 rounded border-[#E8ECEE] text-[#005B96] focus:ring-[#005B96]"
                        />
                        <div className="flex-1 min-w-0">
