...
import { assetsApi, entityExtrasApi } from '@/cyber-assurance/lib/api';
...
const STYLE = `
.relpanel{
  --ac:#005B96; --ac-strong:#014A81; --ac-soft:#EFF5FA;
            ...
            {subdomains.length > 0 && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {subdomains.map((r: any) => (
                  <LinkRow key={r.id} href={`/cyber-assurance/assets/${r.other_asset_id}`}>↳ {r.other_asset_name || `Asset #${r.other_asset_id}`}</LinkRow>
                ))}
              </div>
            )}
          </div>
                ...
                <table className="reltable">
                  ...
                  <tbody>
                    {items.map((r: any) => (
                      <tr key={r.id}>
                        <td>
                          <Link href={`/cyber-assurance/assets/${r.other_asset_id}`} style={{ fontWeight: 600, color: 'var(--ink)' }}>{r.other_asset_name ?? `Asset #${r.other_asset_id}`}</Link>
      ...
      <Card>
        ...
        <div style={{ padding: '14px 16px 16px' }}>
          ...
          {peers.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {peers.map((p: any) => (
                <LinkRow key={p.id} href={`/cyber-assurance/assets/${p.id}`} right={<>
