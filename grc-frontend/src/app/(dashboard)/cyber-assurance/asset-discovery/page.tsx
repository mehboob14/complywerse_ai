...
function RadarCanvas({ labels = [] }: { labels?: string[] }) {
  ...
  useEffect(() => {
    ...
    const W = cv.width, H = cv.height, cx = W / 2, cy = H / 2, R = Math.min(cx, cy) - 12, A = 'rgba(0,91,150,';
    ...
    const frame = () => {
      ...
      blips.forEach((b) => {
        ...
        if (b.lit > 0.14) {
          ...
          ctx.fillStyle = 'rgba(0,91,150,' + Math.min(1, b.lit) + ')';
...
const RD_HOVER = '#EFF5FA';           // indigo tint on hover
const RD_TIER_DOT: Record<RdTier, string> = { identified: '#014A81', partial: '#a06a12', dark: '#a1a79f' };
    ...
    <div style={{ padding: '14px 16px', background: 'var(--as-subtle)', borderTop: '1px solid var(--as-row)' }}>
      ...
      {saved.length > 0 && !showForm && (
        <div style={{ marginBottom: 12 }}>
          ...
          <button onClick={() => setAddNew(true)} style={{ marginTop: 8, background: 'none', border: 'none', color: 'var(--as-link, #005B96)', font: '600 11.5px inherit', cursor: 'pointer', padding: 0 }}>＋ Enter a different login</button>
        </div>
      )}
      {showForm && (
        <>
...
const C = {
  ...
  green: '#014A81', link: '#005B96', linkHover: '#014A81', ink: '#0F172A',
  teal: '#3279A3', tealDim: '#B3CDE0', amber: '#e0a92e', amberText: '#a06a12', amberFill: '#fbf1dc',
  dimDate: '#c98a5a', muted: '#67766f', muted2: '#5f6f69', muted3: '#7a8a84', label: '#8a9089',
  faint: '#a1a79f', mono: '#4a5a53', chipBg: '#e6e4db', fillKnown: '#EFF5FA', fillUnknown: '#eef0ea',
...
const CsPrimary = ({ children, onClick, disabled, style }: any) => (
  <button onClick={onClick} disabled={disabled}
    style={{ display: 'flex', alignItems: 'center', gap: 9, border: 'none', color: '#fff', font: `700 14px ${FONT}`,
      ...
      background: disabled ? C.tealDim : C.teal, boxShadow: '0 2px 8px rgba(0,91,150,.28)', ...style }}>
    {children}
  </button>
);
        ...
        {q.isLoading ? <div style={{ padding: 26, textAlign: 'center', color: 'var(--muted)' }}>Loading…</div>
          ...
          <table className="data-table">
            <thead><tr>
              ...
              <th>Device</th><th>IP</th><th title="Hardware address — the one identifier that does not change when the IP or name changes. Complyverse recognises an existing asset by this.">MAC</th><th>Type</th><th title="Which login doors answered at the last scan — checked automatically for every device">Login methods</th><th>Discovered by</th><th>Last seen</th><th>Status</th><th></th>
            </tr></thead>
            <tbody>
              {shown.map((d) => {
                const st2 = qStatus(d);
