...
// ------------------------------------------------------------------
// Complyverse stores 4000+ CIS rules across ~25 unique benchmark
...
export default function RuleClassificationPage() {
  ...
  const refreshStats = async () => {
    try {
      const r = await fetch('/api/compliance-plugins/classification-stats', { credentials: 'include' });
  ...
  const start = () => {
    ...
    setRunning(true);
    const es = new EventSource('/api/compliance-plugins/classify-stream', { withCredentials: true });
    esRef.current = es;
    es.onmessage = (e) => {
      let payload: any;
