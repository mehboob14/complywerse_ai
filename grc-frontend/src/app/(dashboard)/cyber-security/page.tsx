...
/**
 * Cyber Security — one workspace page with a top-bar of tabs (PDPL-style).
 *
...
const FEATURES: Feature[] = [
  { key: 'asvs', label: 'OWASP ASVS', icon: Shield, group: 'Application Security', format: 'asvs_checklist' },
  { key: 'owasp_testing', label: 'OWASP Testing', icon: Bug, group: 'Application Security', format: 'owasp_v4_testing_checklist' },
  { key: 'cs_mobile', label: 'Mobile App Security', icon: Smartphone, group: 'Application Security', format: 'mobile_app_security' },
  { key: 'cs_csir', label: 'CSIR Maturity', icon: BarChart3, group: 'Maturity Assessments', format: 'csir_maturity' },
  { key: 'cs_cti', label: 'CTI Maturity', icon: Radar, group: 'Maturity Assessments', format: 'cti_maturity' },
  { key: 'cs_incident', label: 'Incident Management', icon: AlertTriangle, group: 'Maturity Assessments', format: 'incident_maturity' },
  { key: 'cs_itsecops', label: 'IT Security Operations', icon: Activity, group: 'Maturity Assessments', format: 'itsecops_maturity' },
];
...
function renderFeature(tab: TabKey): React.ReactNode {
  switch (tab) {
    case 'asvs': return <ASVSAssessmentTab />;
    case 'owasp_testing': return <OwaspTestingTab />;
    case 'cs_mobile': return <MobileAppSecurityTab />;
    case 'cs_csir': return <MaturityAssessmentTab format="csir_maturity" />;
...
function useCyberRows(): { rows: Row[]; isLoading: boolean } {
  const { data, isLoading } = useQuery({
    queryKey: ['cyber-security-assessments'],
    queryFn: async () => {
...
function OverviewDashboard({ onDrill, onOpen }: { onDrill: (k: DrillKind) => void; onOpen: (t: TabKey) => void }) {
  const { rows, isLoading } = useCyberRows();
  ...
  return (
    <div className="space-y-5">
      ...
      {/* ── Compliance by assessment + What to fix first ── */}
...
const DRILL_META: Record<DrillKind, {
  ...
  started: {
    title: 'Assessments', blurb: 'Every cyber-security assessment and whether it has been started.',
...
function CyberSecurityWorkspace() {
  ...
  const openTab = (t: TabKey) => {
    setDrill(null);
    setActive(t);
    router.replace(t === 'overview' ? '/cyber-security' : `/cyber-security?tab=${t}`, { scroll: false });
  };
