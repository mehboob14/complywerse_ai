    ...
    <div className="space-y-6">
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-5">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div className="min-w-0">
            <h2 className="text-lg font-semibold text-slate-900">Custom RCSA Templates</h2>
        ...
        <div className="mt-5 grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="md:col-span-2">
            ...
            <input
              ...
              className="block w-full text-sm text-slate-700 file:mr-3 file:py-2 file:px-3 file:rounded-md file:border-0 file:text-sm file:font-medium file:bg-primary-50 file:text-primary-700 hover:file:bg-primary-100"
            />
          ...
          <div>
            <label className="block text-xs font-semibold uppercase text-slate-500 mb-1">
              Name <span className="text-slate-400 font-normal">(optional, defaults to the file&apos;s title)</span>
            </label>
            <input
              type="text"
              value={name}
              ...
              className="block w-full text-sm rounded-md border border-slate-300 px-3 py-2 focus:border-primary-500 focus:ring-1 focus:ring-primary-500"
            />
          </div>
          <div>
            <label className="block text-xs font-semibold uppercase text-slate-500 mb-1">
              Function area <span className="text-slate-400 font-normal">(optional)</span>
            </label>
            <input
              type="text"
              value={functionArea}
        ...
        <div className="mt-4 flex items-center gap-3">
          <button
            ...
            className="inline-flex items-center gap-2 px-4 py-2 bg-primary-600 text-[#0a0a0a] text-sm font-medium rounded-md hover:bg-primary-700 disabled:opacity-50 disabled:cursor-not-allowed"
          >
          ...
          {uploadM.isSuccess && (
            <p className="text-xs text-emerald-700">Saved &mdash; the new template appears below.</p>
          )}
        </div>
      </section>
            ...
            return (
              ...
              >
                ...
                {key === 'mine' && (myQ.data?.length ?? 0) > 0 && (
                  <span className="ml-0.5 inline-flex items-center justify-center min-w-[18px] h-4 px-1 rounded-full bg-primary-100 text-[10px] font-semibold text-primary-700">
                    {myQ.data?.length}
                  </span>
                )}
              </button>
      ...
      <section>
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-sm font-semibold text-slate-900">Your custom templates</h3>
        ...
        {listQ.isLoading ? (
          ...
          <div className="rounded-lg border border-dashed border-slate-300 bg-white p-8 text-center">
            ...
            <p className="mt-3 text-sm font-medium text-slate-700">No custom templates yet</p>
            <p className="mt-1 text-xs text-slate-500">
                ...
                {templates.map((t) => (
                  <tr key={t.id} className="hover:bg-slate-50">
                    <td className="px-4 py-3">
                      ...
                      <div className="text-[11px] text-slate-500 font-mono truncate max-w-[260px]" title={t.original_filename}>
                        {t.original_filename}
                      </div>
                    </td>
                    <td className="px-4 py-3 text-slate-700">{t.function_area || '—'}</td>
                    <td className="px-4 py-3 text-right text-slate-700">{t.column_count}</td>
                    <td className="px-4 py-3 text-right text-slate-700">{t.row_count}</td>
                    <td className="px-4 py-3 text-slate-700 truncate max-w-[160px]" title={t.sheet_name || ''}>
                      {t.sheet_name || '—'}
                    </td>
                    <td className="px-4 py-3">
                      <span
      ...
      {tab === 'mine' && (
        <section>
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-semibold text-slate-900">Assessment items assigned to me</h3>
            <button
              type="button"
          ...
          {myQ.isLoading ? (
            ...
            <div className="rounded-lg border border-dashed border-slate-300 bg-white p-8 text-center">
              ...
              <p className="mt-3 text-sm font-medium text-slate-700">No items assigned to you yet</p>
              <p className="mt-1 text-xs text-slate-500">
            ...
            <div className="overflow-hidden rounded-lg border border-slate-200 bg-white">
              <table className="min-w-full text-sm">
                ...
                <tbody className="divide-y divide-slate-100">
                  {(myQ.data ?? []).map((row) => (
                    <tr key={`${row.template_id}-${row.row_id}`} className="hover:bg-slate-50">
                      <td className="px-4 py-3">
                        <div className="font-medium text-slate-900">{row.template_name}</div>
                        <div className="text-[11px] text-slate-500">Item #{row.row_id}</div>
                      </td>
                      <td className="px-4 py-3 text-slate-700 font-mono text-xs">{row.risk_id_text || '—'}</td>
                      ...
                      <td className="px-4 py-3 text-right text-slate-700">{row.evidence_count}</td>
