...
export function ImportIncidentsModal({
  onClose,
}: {
  onClose: () => void;
}) {
  ...
  const handleUpload = async () => {
    ...
    setIsUploading(true);
    try {
      const response = await ermApi.incidents.importIncidents(file);
      setResult(response.data);
      if (response.data.imported > 0) {
        qc.invalidateQueries({ queryKey: ['erm-incidents'] });
        qc.invalidateQueries({ queryKey: ['erm-incident-dashboard'] });
      }
    } catch (error: unknown) {
      const detail =
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4">
      <div className="w-full max-w-lg rounded-xl border border-slate-200 bg-white shadow-xl">
        <div className="flex items-center justify-between border-b border-slate-200 px-4 py-3">
          <div>
            <h2 className="text-sm font-semibold text-slate-900">Import Incidents</h2>
        ...
        <div className="space-y-3 px-4 py-4">
          <button
            type="button"
            onClick={() => ermApi.incidents.downloadTemplate()}
