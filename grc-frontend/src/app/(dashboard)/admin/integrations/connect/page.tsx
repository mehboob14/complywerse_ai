...
type Platform = 'windows' | 'linux' | 'aws' | 'digitalocean';
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50">
      <div className="max-w-3xl mx-auto px-4 py-8">
        ...
        {tokenData && status?.state !== 'ready' && (picked === 'windows' || picked === 'linux' || picked === 'digitalocean') && (
          <ManualCredsForm
            platform={picked}
...
function ManualCredsForm({
  ...
  const defaultPort = platform === 'windows' ? '5986' : '22';
  const portHint = platform === 'windows'
    ? '5986 (WinRM HTTPS) — usually fixed'
    : '22 (SSH) — change if your server uses a custom port';
              ...
              <div className="grid grid-cols-12 gap-3">
                ...
                <div className="col-span-12 sm:col-span-4">
                  <label className="flex items-center justify-between mb-1.5">
                    ...
                    <span className="text-xs text-slate-400">{platform === 'windows' ? 'WinRM HTTPS' : 'SSH'}</span>
                  </label>
                  <input
                    type="text"
                    value={port}
