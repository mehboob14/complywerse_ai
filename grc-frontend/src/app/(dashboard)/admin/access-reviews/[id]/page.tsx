'use client';
// src/app/(dashboard)/admin/access-reviews/[id]/page.tsx
  ...
  return (
    <div className="mx-auto max-w-[1100px] px-8 py-7 pb-16">
      ...
      {!closed && stage === 4 && !wantReport && (
        <CertifyBlock campaignId={id} items={c.items} sampleSize={c.requested_sample_size} sel={sel} setSel={setSel} onContinue={() => router.push(`/admin/access-reviews/${id}?stage=report`)} />
      )}
      ...
      {(stage >= 5 || closed || (stage === 4 && wantReport)) && (
        <>
          {!closed && stage === 4 && wantReport && (
            <button onClick={() => router.push(`/admin/access-reviews/${id}`)} className="mb-4 inline-flex items-center gap-1.5 text-[12.5px] font-medium text-slate-500"><ChevronLeft size={14} /> Back to certify</button>
          )}
