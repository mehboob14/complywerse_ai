...
export default function EvidenceSuggestionsPage() {
  ...
  const getPriorityBadge = (priority: string) => {
    ...
    return (
      <span className={`inline-flex items-center gap-1 rounded-full ${style.bg} px-2 py-0.5 text-xs font-medium ${style.text}`}>
        {priority === 'critical' && <AlertTriangle size={10} strokeWidth={1.75} />}
  ...
  const getConfidenceBadge = (confidence: number | null) => {
    ...
    const color = pct >= 80 ? 'text-emerald-600' : pct >= 60 ? 'text-amber-600' : 'text-orange-600';
    return (
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Evidence Suggestions & Reuse</h1>
          <p className="text-slate-500">AI-recommended evidence types and reuse metrics</p>
        </div>
        {canCreate && <button
          onClick={() => {
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="stat-card">
          <div className="flex items-start justify-between">
            <div className="rounded-xl bg-primary-50 p-3">
              <Repeat className="h-6 w-6 text-primary-600" strokeWidth={1.75} />
            </div>
          </div>
        ...
        <div className="stat-card">
          <div className="flex items-start justify-between">
            <div className="rounded-xl bg-emerald-50 p-3">
              <TrendingUp className="h-6 w-6 text-emerald-600" strokeWidth={1.75} />
            </div>
          </div>
                  ...
                  <div key={priority}>
                    <div className="mb-1 flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-medium capitalize text-slate-900">{priority}</span>
                      </div>
        ...
        <div className="card">
          ...
          {!evidenceTypes?.length ? (
            <div className="flex flex-col items-center justify-center py-8 text-center">
              <FileText className="mb-3 h-10 w-10 text-slate-400" strokeWidth={1.75} />
              <p className="text-sm text-slate-500">No evidence types yet</p>
            </div>
          ) : (
      ...
      <div className="card">
        ...
        {reuseLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-8 text-center">
            <Repeat className="mb-3 h-10 w-10 text-slate-400" strokeWidth={1.75} />
            <p className="text-sm text-slate-500">No reused evidence found</p>
          </div>
        ) : (
          <div className="space-y-4">
            <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
              <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
                <div className="flex items-center gap-3">
                  <FileCheck className="h-5 w-5 text-primary-600 flex-shrink-0" strokeWidth={1.75} />
                  <div>
                    <p className="text-lg font-semibold text-slate-900">{reuseStats.total_evidence}</p>
              ...
              <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
                <div className="flex items-center gap-3">
                  <Repeat className="h-5 w-5 text-primary-600 flex-shrink-0" strokeWidth={1.75} />
                  <div>
                    <p className="text-lg font-semibold text-slate-900">{reuseStats.multi_framework_evidence}</p>
                    <p className="text-xs text-slate-500">Multi-Framework</p>
                  </div>
                </div>
              </div>
              <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
                <div className="flex items-center gap-3">
                  <BarChart3 className="h-5 w-5 text-emerald-600 flex-shrink-0" strokeWidth={1.75} />
                  <div>
                    <p className="text-lg font-semibold text-slate-900">{reuseStats.average_controls_per_evidence}</p>
                    <p className="text-xs text-slate-500">Avg Controls/Evidence</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-slate-50">
                  <tr>
                    <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-slate-500">Evidence Name</th>
                    <th className="px-4 py-3 text-center text-xs font-medium uppercase tracking-wider text-slate-500">Controls Linked</th>
                    <th className="px-4 py-3 text-center text-xs font-medium uppercase tracking-wider text-slate-500">Frameworks Covered</th>
                  </tr>
                </thead>
                  ...
                  {reuseStats.top_reused_evidence.slice(0, 5).map((item) => (
                    <tr key={item.evidence_id} className="hover:bg-slate-50">
                      <td className="px-4 py-3">
                        <p className="text-sm font-medium text-slate-900 truncate max-w-xs">{item.evidence_name}</p>
                      </td>
                      <td className="px-4 py-3 text-center">
                        <span className="inline-flex items-center gap-1 rounded-full bg-primary-50 px-2 py-0.5 text-sm text-primary-700">
                          <Link2 size={12} strokeWidth={1.75} />
                          {item.controls_linked}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-center">
                        <span className="text-sm text-slate-700">{item.frameworks_covered}</span>
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            {auditSavings && (
              <div className="mt-4 rounded-lg border border-emerald-200 bg-emerald-50 p-4">
                <div className="flex items-center gap-3">
                  <Percent className="h-5 w-5 text-emerald-600 flex-shrink-0" strokeWidth={1.75} />
      ...
      <div className="card">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="relative">
              ...
              <input
                ...
                className="w-full rounded-lg border border-slate-300 bg-white py-2 pl-10 pr-4 text-sm text-slate-900 placeholder-slate-400 focus:border-primary-500 focus:outline-none sm:w-48"
              />
            </div>
            <select
            ...
            {typeFilter && (
              <button
                ...
                className="flex items-center gap-1 rounded-lg border border-primary-300 bg-primary-50 px-3 py-2 text-sm text-primary-700"
              >
                {typeFilter}
        ...
        {recsLoading ? (
          ...
          <div className="flex h-64 flex-col items-center justify-center text-red-600">
            ...
            <button onClick={() => refetchRecs()} className="mt-2 text-sm text-primary-700 hover:underline">
              Try again
            </button>
          </div>
        ) : !filteredRecommendations?.length ? (
          <div className="flex flex-col items-center justify-center py-12 text-center">
            <Sparkles className="mb-4 h-12 w-12 text-slate-400" strokeWidth={1.75} />
            <h3 className="text-lg font-medium text-slate-900">No recommendations yet</h3>
            <p className="mt-1 text-slate-500">Use Bulk Generate to create AI recommendations for your controls</p>
          </div>
        ) : (
          <>
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-slate-50">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-slate-500">Control</th>
                    <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-slate-500">Evidence Type</th>
                    <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-slate-500">Priority</th>
                    <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-slate-500">Confidence</th>
                  </tr>
                </thead>
                        ...
                        <td className="px-4 py-3">
                          <div>
                            <p className="text-sm font-medium text-slate-900">
                              {rec.control_code && (
                                <span className="mr-2 font-mono text-primary-700">{rec.control_code}</span>
                              )}
                              {rec.control_name || rec.group_name || 'Unknown Control'}
                            </p>
                            {rec.framework_name && (
                              <p className="text-xs text-slate-500">{rec.framework_name}</p>
                            )}
                          </div>
                        </td>
                        <td className="px-4 py-3">
                          <span className="inline-flex items-center gap-1 rounded-lg bg-slate-100 px-2 py-1 text-sm text-slate-900">
                            <FileCheck size={14} className="text-slate-500" strokeWidth={1.75} />
                            {rec.evidence_type}
                          </span>
                        </td>
                        <td className="px-4 py-3">
                        ...
                        <tr className="bg-slate-50/60">
                          <td colSpan={5} className="px-4 py-4">
                            <div className="ml-8 space-y-3">
                              {rec.ai_reasoning && (
                                <div>
                                  <p className="mb-1 text-xs font-medium uppercase text-slate-400">AI Reasoning</p>
                                  <p className="text-sm text-slate-700">{rec.ai_reasoning}</p>
                                </div>
                              )}
                              {rec.evidence_description && (
                                <div>
                                  <p className="mb-1 text-xs font-medium uppercase text-slate-400">Description</p>
                                  <p className="text-sm text-slate-700">{rec.evidence_description}</p>
                                </div>
                              )}
                              {rec.sample_evidence_names && rec.sample_evidence_names.length > 0 && (
                                <div>
                                  <p className="mb-1 text-xs font-medium uppercase text-slate-400">Sample Names</p>
                                  <div className="flex flex-wrap gap-2">
              ...
              <div className="flex items-center justify-between border-t border-slate-200 px-4 py-3">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    disabled={page === 0}
                    className="rounded-lg border border-slate-300 bg-white px-3 py-1 text-sm text-slate-900 hover:bg-slate-50 disabled:opacity-50"
                  >
                    Previous
                  </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          <div className="w-full max-w-lg rounded-xl border border-slate-200 bg-white shadow-xl">
            <div className="flex items-center justify-between border-b border-slate-200 p-4">
              <h3 className="text-lg font-semibold text-slate-900">Bulk Generate Recommendations</h3>
              <button
                  ...
                  <div className="flex items-center justify-center">
                    {bulkProgress.status === 'processing' ? (
                      ...
                      <div className="flex flex-col items-center gap-3">
                        ...
                        <p className="text-slate-900 font-medium">Generation Complete!</p>
                      ...
                      <div className="flex flex-col items-center gap-3">
                        <AlertCircle className="h-12 w-12 text-red-600" strokeWidth={1.75} />
                  ...
                  <div>
                    ...
                    <div className="max-h-48 overflow-y-auto rounded-lg border border-slate-300 bg-white">
                      {controlGroups?.map((group) => (
                        <label
                          key={group.id}
                          className="flex items-center gap-3 border-b border-slate-100 px-4 py-2 last:border-0 hover:bg-slate-50"
                        >
                          <input
                            type="checkbox"
                          ...
                          />
                          <div className="flex-1">
                            <p className="text-sm font-medium text-slate-900">{group.code}</p>
                            <p className="text-xs text-slate-500 truncate">{group.name}</p>
                          </div>
                        </label>
                      ))}
                      {(!controlGroups || controlGroups.length === 0) && (
                        <p className="px-4 py-3 text-sm text-slate-400">No control groups available</p>
                      )}
                    </div>
                  </div>
