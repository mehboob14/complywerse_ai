...
const DOCUMENT_TYPE_CONFIG: Record<string, { icon: React.ElementType; color: string; bgColor: string; label: string }> = {
  policy: { icon: BookOpen, color: 'text-purple-700', bgColor: 'bg-purple-50', label: 'Policy' },
  standard: { icon: FileCheck, color: 'text-teal-700', bgColor: 'bg-teal-50', label: 'Standard' },
  procedure: { icon: ClipboardList, color: 'text-emerald-700', bgColor: 'bg-emerald-50', label: 'Procedure' },
  guideline: { icon: Lightbulb, color: 'text-amber-700', bgColor: 'bg-amber-50', label: 'Guideline' },
  charter: { icon: Shield, color: 'text-primary-700', bgColor: 'bg-primary-50', label: 'Charter' },
  framework: { icon: Layers, color: 'text-orange-700', bgColor: 'bg-orange-50', label: 'Framework' },
};

const getTypeConfig = (type: string) => {
  return DOCUMENT_TYPE_CONFIG[type] || { icon: FileText, color: 'text-slate-700', bgColor: 'bg-slate-100', label: type };
};
...
export default function ReviewCalendarPage() {
  ...
  const getDaysUntilDisplay = (days: number | null, isOverdue: boolean) => {
    ...
    if (days <= 7) {
      return { text: `${days} day${days !== 1 ? 's' : ''} left`, className: 'text-amber-700' };
    }

    return { text: `${days} days left`, className: 'text-emerald-700' };
  };
    ...
    <div className="space-y-4 sm:space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <h1 className="text-lg sm:text-xl font-semibold text-slate-900">Review Calendar</h1>
          <p className="text-xs sm:text-sm text-slate-500">Visual overview of upcoming document reviews</p>
        </div>
      </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-red-700">
                {overdueLoading ? <Loader2 className="h-5 w-5 animate-spin" strokeWidth={1.75} /> : overdueDocuments.length}
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-emerald-700">
                {upcomingLoading ? <Loader2 className="h-5 w-5 animate-spin" strokeWidth={1.75} /> : upcomingDocuments.length}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between border-b border-slate-200 p-4">
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              <button
                ...
                className="rounded-lg bg-slate-100 p-2 text-slate-700 hover:bg-slate-200 hover:text-slate-900 transition-colors"
              >
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              ...
              className="rounded-lg bg-slate-100 px-3 py-1.5 text-sm text-slate-700 hover:bg-slate-200 hover:text-slate-900 transition-colors"
            >
              Today
            </button>
          </div>
                ...
                return (
                  <div
                    ...
                    className={`
                      ...
                      ${hasDocuments ? 'cursor-pointer hover:bg-slate-100' : ''}
                      ...
                      <div className="space-y-1">
                        {day.documents.slice(0, 2).map((doc, docIndex) => {
                          ...
                          return (
                            <div
                              key={docIndex}
                              className={`flex items-center gap-1 rounded px-1.5 py-0.5 text-xs truncate border ${getStatusBorderColor(status)} bg-slate-50`}
                            >
            ...
            {selectedDateDocuments.map(doc => {
              ...
              return (
                <div
                  ...
                  className={`flex items-center justify-between rounded-lg border p-3 cursor-pointer transition-colors hover:bg-slate-100 ${getStatusBorderColor(status)} bg-slate-50`}
                >
                  <div className="flex items-center gap-3">
                    <div className={`rounded-lg p-2 ${typeConfig.bgColor}`}>
                      <TypeIcon className={`h-4 w-4 ${typeConfig.color}`} strokeWidth={1.75} />
                    </div>
                    <div>
                  ...
                  <ExternalLink className="h-4 w-4 text-slate-500" strokeWidth={1.75} />
                </div>
              );
            })}
          </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <div className="rounded-xl border border-slate-200 bg-white overflow-hidden">
          <div className="flex items-center gap-3 border-b border-slate-200 p-4">
            <AlertTriangle className="h-5 w-5 text-red-600" strokeWidth={1.75} />
            <h3 className="text-lg font-semibold text-slate-900">Overdue Reviews</h3>
          ...
          <div className="p-4">
            {overdueLoading ? (
              <div className="flex h-32 items-center justify-center">
                <Loader2 className="h-6 w-6 animate-spin text-slate-400" strokeWidth={1.75} />
              </div>
            ) : overdueDocuments.length === 0 ? (
              <div className="flex h-32 flex-col items-center justify-center text-slate-500">
                <CheckCircle className="h-8 w-8 mb-2" strokeWidth={1.75} />
                ...
                {overdueDocuments.slice(0, 10).map(doc => {
                  ...
                  return (
                    <div
                      key={doc.id}
                      onClick={() => handleDocumentClick(doc.id)}
                      className="flex items-center justify-between rounded-lg border border-red-200 bg-slate-50 p-3 cursor-pointer transition-colors hover:bg-slate-100"
                    >
        ...
        <div className="rounded-xl border border-slate-200 bg-white overflow-hidden">
          <div className="flex items-center gap-3 border-b border-slate-200 p-4">
            <CalendarIcon className="h-5 w-5 text-emerald-600" strokeWidth={1.75} />
            <h3 className="text-lg font-semibold text-slate-900">Upcoming Reviews</h3>
          ...
          <div className="p-4">
            {upcomingLoading ? (
              ...
              <div className="flex h-32 flex-col items-center justify-center text-slate-500">
                <CalendarIcon className="h-8 w-8 mb-2" strokeWidth={1.75} />
                ...
                {upcomingDocuments.slice(0, 10).map(doc => {
                  ...
                  return (
                    <div
                      key={doc.id}
                      onClick={() => handleDocumentClick(doc.id)}
                      className={`flex items-center justify-between rounded-lg border p-3 cursor-pointer transition-colors hover:bg-slate-100 ${
                        isDueSoon ? 'border-amber-200' : 'border-emerald-200'
                      } bg-slate-50`}
                    >
                      <div className="flex items-center gap-3 min-w-0">
                        <div className={`rounded-lg p-2 ${typeConfig.bgColor} flex-shrink-0`}>
