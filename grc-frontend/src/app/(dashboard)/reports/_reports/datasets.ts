...
export const DATASETS: ReportDataset[] = [
  ...
  // ── Controls Automation ── the tenant's in-scope common controls, their tests and collectors
  {
    key: 'common_controls', permissions: ['controls:control_library:*', 'compliance:frameworks:*'], module: 'Controls Automation', label: 'Common Controls',
    description: 'In-scope common controls with frameworks, owner, automation status, assurance status, maturity and latest test results.',
    fetch: async () => asRows((await automationApi.listCommonControls({ scope: 'in_scope' })).data?.controls),
    columns: [
      { key: 'control_id', label: 'Control', type: 'text', width: 110, href: (r) => `/automation/soc2-controls/${encodeURIComponent(String(r.control_id))}` },
      { key: 'title', label: 'Name', type: 'text', width: 300, href: (r) => `/automation/soc2-controls/${encodeURIComponent(String(r.control_id))}` },
      { key: 'category', label: 'Domain', type: 'text', width: 200 },
      { key: 'frameworks', label: 'In-scope frameworks', type: 'text', width: 200, accessor: (r) => (Array.isArray(r.frameworks) ? (r.frameworks as string[]).map(titleCase).join(', ') : '') },
      { key: 'requirement_count', label: 'Requirements', type: 'number', width: 120, align: 'right', agg: 'sum' },
      { key: 'sub_type', label: 'Type', type: 'badge', width: 110, badgeTone: () => TONE.slate },
      { key: 'overall_status', label: 'Automation status', type: 'badge', width: 150, badgeTone: statusTone, format: titleCase },
      { key: 'checks_count', label: 'Checks', type: 'number', width: 90, align: 'right', agg: 'sum' },
      { key: 'owner_name', label: 'Owner', type: 'text', width: 150 },
      { key: 'ownership_status', label: 'Ownership', type: 'badge', width: 120, badgeTone: statusTone, format: titleCase },
      { key: 'designation', label: 'Assurance status', type: 'badge', width: 150, badgeTone: statusTone, format: titleCase },
      { key: 'design_effectiveness', label: 'Design effectiveness', type: 'badge', width: 160, badgeTone: statusTone, format: titleCase },
      { key: 'operating_effectiveness', label: 'Operating effectiveness', type: 'badge', width: 170, badgeTone: statusTone, format: titleCase },
      { key: 'cmm_actual', label: 'Maturity (CMM)', type: 'number', width: 120, align: 'right', agg: 'avg' },
      { key: 'cmm_target', label: 'Target (CMM)', type: 'number', width: 110, align: 'right', agg: 'avg' },
      { key: 'last_tested_at', label: 'Last tested', type: 'date', width: 120 },
      { key: 'next_test_date', label: 'Next test due', type: 'date', width: 120 },
      { key: 'custom', label: 'Custom?', type: 'badge', width: 90, format: boolFmt, badgeTone: (v) => (boolTrue(v) ? TONE.teal : TONE.slate) },
      // Register fields — only a tenant-authored control carries these.
      { key: 'control_type', label: 'Control type', type: 'badge', width: 120, badgeTone: () => TONE.slate, format: titleCase },
      { key: 'sub_category', label: 'Sub-category', type: 'text', width: 160 },
      { key: 'operating_frequency', label: 'Operates', type: 'text', width: 120, format: titleCase },
      { key: 'lifecycle_status', label: 'Register status', type: 'badge', width: 140, badgeTone: statusTone, format: titleCase },
      { key: 'priority', label: 'Priority', type: 'badge', width: 100, badgeTone: statusTone, format: titleCase },
      { key: 'is_key_control', label: 'Key control?', type: 'badge', width: 110, format: boolFmt, badgeTone: (v) => (boolTrue(v) ? TONE.amber : TONE.slate) },
      { key: 'regulatory_source', label: 'Regulatory source', type: 'text', width: 200 },
      { key: 'review_date', label: 'Next review', type: 'date', width: 120 },
    ],
  },
  {
    key: 'control_tests', permissions: ['controls:control_library:*', 'compliance:frameworks:*'], module: 'Controls Automation', label: 'Control Tests',
    description: 'Every design and operating effectiveness test on the common controls: result, sample, exceptions and sign-off.',
    fetch: async () => asRows((await automationApi.listControlTests()).data?.tests),
    columns: [
      { key: 'scf_id', label: 'Control', type: 'text', width: 110, href: (r) => `/automation/soc2-controls/${encodeURIComponent(String(r.scf_id))}?tab=assurance` },
      { key: 'control', label: 'Name', type: 'text', width: 280 },
      { key: 'domain', label: 'Domain', type: 'text', width: 180 },
      { key: 'test_type', label: 'Test', type: 'badge', width: 110, badgeTone: () => TONE.teal, format: titleCase },
      { key: 'result', label: 'Result', type: 'badge', width: 150, badgeTone: statusTone, format: titleCase },
      { key: 'status', label: 'Status', type: 'badge', width: 120, badgeTone: statusTone, format: titleCase },
      { key: 'test_date', label: 'Tested', type: 'date', width: 120 },
      { key: 'tester', label: 'Tester', type: 'text', width: 150 },
      { key: 'reviewer', label: 'Signed off by', type: 'text', width: 150 },
      { key: 'independent_review', label: 'Independent?', type: 'badge', width: 110, format: boolFmt, badgeTone: (v) => (boolTrue(v) ? TONE.green : TONE.slate) },
      { key: 'sample_size', label: 'Sample', type: 'number', width: 90, align: 'right', agg: 'sum' },
      { key: 'exceptions_found', label: 'Exceptions', type: 'number', width: 100, align: 'right', agg: 'sum' },
      { key: 'is_key_control', label: 'Key control?', type: 'badge', width: 110, format: boolFmt, badgeTone: (v) => (boolTrue(v) ? TONE.amber : TONE.slate) },
      { key: 'findings', label: 'Findings', type: 'text', width: 300 },
    ],
  },
  {
    key: 'evidence_collectors', permissions: ['controls:control_library:*', 'compliance:scan:execute'], module: 'Controls Automation', label: 'Evidence Collectors',
    description: 'Every connector that collects automated evidence: whether it is connected and how its last collection went.',
    fetch: async () => asRows((await automationApi.listCollectors()).data?.collectors),
    columns: [
      { key: 'label', label: 'Connector', type: 'text', width: 220, href: (r) => `/admin/evidence-collectors?connector=${r.provider}` },
      { key: 'category', label: 'Category', type: 'badge', width: 140, badgeTone: () => TONE.slate, format: titleCase },
      { key: 'connected', label: 'Connected?', type: 'badge', width: 110, format: boolFmt, badgeTone: (v) => (boolTrue(v) ? TONE.green : TONE.slate) },
      { key: 'last_run_status', label: 'Last collection', type: 'badge', width: 140, badgeTone: statusTone, format: titleCase, accessor: (r) => (r.last_run as { status?: string } | null)?.status ?? '' },
      { key: 'last_run_at', label: 'Last collected', type: 'date', width: 140, accessor: (r) => (r.last_run as { started_at?: string } | null)?.started_at ?? null },
      { key: 'controls', label: 'SOC 2 criteria', type: 'text', width: 180, accessor: (r) => (Array.isArray(r.controls) ? (r.controls as string[]).join(', ') : '') },
    ],
  },
  {
