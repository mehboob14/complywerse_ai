    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="w-full max-w-md rounded-xl border border-slate-200 bg-white p-6">
        <div className="mb-4 flex items-center justify-between">
          <h3 className="text-lg font-semibold text-black">{titles[type]}</h3>
        ...
        <div className="mb-4">
          <label className="mb-1 block text-sm font-medium text-slate-600">
            Comments {(type === 'reject' || type === 'return') && <span className="text-red-600">*</span>}
          </label>
          <textarea
            value={comments}
        ...
        <div className="flex justify-end gap-3">
          <button
            onClick={onClose}
            disabled={isLoading}
            className="rounded-lg border border-slate-300 px-4 py-2 font-medium text-slate-600 hover:bg-slate-200"
          >
            Cancel
          </button>
          <button
            ...
            className={`flex items-center gap-2 rounded-lg px-4 py-2 font-medium text-black ${buttonColors[type]}`}
          >
...
export default function ApprovalReviewPage() {
  ...
  const getResponseDisplay = (question: Question) => {
    ...
    switch (question.question_type) {
      case 'risk_rating':
        return (
          <div className="flex items-center gap-4">
            <span className="text-slate-600">Likelihood: <span className="text-black font-medium">{response.likelihood}</span></span>
            <span className="text-slate-600">Impact: <span className="text-black font-medium">{response.impact}</span></span>
            <span className={`px-2 py-0.5 rounded text-xs font-medium ${
              (response.likelihood || 0) * (response.impact || 0) >= 15 ? 'bg-red-50 text-red-700' :
              (response.likelihood || 0) * (response.impact || 0) >= 8 ? 'bg-yellow-50 text-yellow-700' :
      ...
      case 'control_rating':
        return <span className="text-black">{EFFECTIVENESS_LABELS[response.effectiveness || ''] || response.effectiveness}</span>;
      case 'yes_no':
        return (
          <span className={response.yes_no_value ? 'text-green-600' : 'text-red-600'}>
            {response.yes_no_value ? 'Yes' : 'No'}
          </span>
        );
      case 'text':
        return <p className="text-slate-600">{response.text_value}</p>;
      default:
        return null;
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="card p-4">
          ...
          <p className="text-xl font-semibold text-black">{assessment.current_tier} of {assessment.total_tiers}</p>
        </div>
        <div className="card p-4">
        ...
        <div className="card p-4">
          ...
          <p className={`text-xl font-semibold ${
            (assessment.ai_quality_score || 0) >= 80 ? 'text-green-600' : 
            (assessment.ai_quality_score || 0) >= 60 ? 'text-yellow-600' : 'text-red-600'
          }`}>{assessment.ai_quality_score || '-'}%</p>
        </div>
        <div className="card p-4">
          <p className="text-sm text-slate-600">Questions Answered</p>
          <p className="text-xl font-semibold text-black">{assessment.responses.length} / {assessment.questions.length}</p>
        </div>
      </div>

      <div className="card p-4">
        <h3 className="text-lg font-medium text-black mb-4">Approval History</h3>
          ...
          {assessment.approval_history.map((action) => (
            <div key={action.id} className="flex items-center gap-4 p-3 rounded-lg bg-white/50">
              <div className={`p-2 rounded-lg ${
                action.action === 'approved' ? 'bg-green-50' :
                action.action === 'rejected' ? 'bg-red-50' :
              ...
              <div className="flex-1">
                <p className="text-black font-medium capitalize">{action.action}</p>
                <p className="text-sm text-slate-600">by {action.actor_name} • {formatDate(action.created_at)}</p>
              </div>
              {action.comments && (
                <p className="text-sm text-slate-600 italic">"{action.comments}"</p>
              )}
            </div>
          ))}
        </div>
      ...
      <div className="space-y-4">
        <h3 className="text-lg font-medium text-black">Assessment Responses</h3>
        {Object.entries(groupedQuestions).map(([section, questions]) => (
          <div key={section} className="card overflow-hidden">
            ...
            >
              <h4 className="text-black font-medium">{section}</h4>
              ...
              <div className="divide-y divide-slate-700">
                {questions.map((question) => (
                  <div key={question.id} className="p-4">
                    <p className="text-slate-600 mb-2">{question.question_text}</p>
