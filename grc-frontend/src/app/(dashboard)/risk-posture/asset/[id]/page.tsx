...
export default function RiskPostureAssetPage() {
  ...
  // Guard against bad URL shapes (/risk-posture/asset/abc, /risk-posture/asset/0)
  // — bounce back to the dashboard instead of firing a doomed API request.
  useEffect(() => {
    if (params && (!params.id || !Number.isFinite(assetId) || assetId <= 0)) {
      router.replace('/risk-posture');
    }
  ...
  // Backend 404 means the asset doesn't belong to this tenant (or was
  // deleted). Redirect to the list rather than leaving the user on a dead
  // detail page with no obvious next step.
  useEffect(() => {
    const status = (q.error as { response?: { status?: number } } | null)?.response?.status;
    if (q.isError && status === 404) {
      router.replace('/risk-posture?missing=' + encodeURIComponent(String(assetId)));
    }
  }, [q.isError, q.error, router, assetId]);
