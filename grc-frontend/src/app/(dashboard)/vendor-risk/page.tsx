...
import {
  ...
  AlertCircle, ArrowUpRight, Radio, ArrowRight,
} from 'lucide-react';
...
const POSTURE_HEX: Record<string, string> = {
  open: '#dc2626', in_remediation: '#f97316', accepted: '#9D7BFF', closed: '#22c55e',
};
...
function KpiCard({
  ...
  return (
    <div className="relative overflow-hidden rounded-xl border border-gray-200 bg-white p-4">
      ...
      {foot && <div className="mt-1.5 text-[11px] text-gray-500">{foot}</div>}
        ...
        <div className="absolute right-3 top-3 h-8 w-16 opacity-80">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={spark.map((v, i) => ({ i, v }))} margin={{ top: 2, bottom: 2, left: 0, right: 0 }}>
              <Area type="monotone" dataKey="v" stroke="#4C8DFF" fill="#4C8DFF" fillOpacity={0.15} strokeWidth={1.5} dot={false} isAnimationActive={false} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      )}
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500">Third-party risk posture across the portfolio — live from assessments, findings and monitoring.</p>
        </div>
        <div className="flex items-center gap-2">
          <div className="inline-flex rounded-lg border border-gray-300 bg-white p-0.5 text-xs font-medium" role="tablist" aria-label="Dashboard scope">
            {(['portfolio', 'mine'] as const).map((s) => (
              <button key={s} role="tab" aria-selected={scope === s} onClick={() => setScope(s)}
                className={`rounded-md px-2.5 py-1 ${scope === s ? 'bg-primary-600 text-white' : 'text-gray-600 hover:bg-gray-50'}`}>
                {s === 'portfolio' ? 'Portfolio' : 'My queue'}
              </button>
            ))}
          </div>
          <Link href="/vendor-risk/vendors" className="inline-flex items-center gap-1.5 rounded-lg bg-primary-600 px-3 py-1.5 text-xs font-medium text-white hover:bg-primary-700">
        ...
        <Panel title="Inherent vs residual by tier" sub="The gap is the risk removed by controls">
          <ResponsiveContainer width="100%" height={170}>
            <BarChart data={inhRes} margin={{ top: 6, right: 6, left: -18, bottom: 0 }}>
              ...
              <Bar dataKey="residual" name="Residual" fill="#4C8DFF" radius={[3, 3, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </Panel>
        ...
        <Panel title="Portfolio residual" sub="Average residual across the portfolio">
          <div className="relative">
            ...
            <div className="pointer-events-none absolute inset-x-0 bottom-8 text-center">
              ...
              <div className="text-[11px] text-gray-500">/ 100 residual · inherent {k.portfolio_inherent}</div>
            </div>
          </div>
        </Panel>
      </div>
      ...
      <Panel title="Portfolio risk trend" sub="Inherent vs residual over time, against the risk-appetite line">
        {trendSeries.length === 0 ? (
          <p className="py-8 text-center text-xs text-gray-400">No snapshot history yet — the trend builds as assessments are scored.</p>
        ) : (
            ...
            <ComposedChart data={trendSeries} margin={{ top: 6, right: 10, left: -18, bottom: 0 }}>
              <defs>
                <linearGradient id="resfill" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stopColor="#4C8DFF" stopOpacity={0.25} />
              ...
              <Area type="monotone" dataKey="residual" name="Residual" stroke="#4C8DFF" fill="url(#resfill)" strokeWidth={2.4} />
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        ...
        <Panel title="Open findings by risk domain" sub="Where the open gaps concentrate">
          {domainData.length === 0 ? (
            <p className="py-10 text-center text-xs text-gray-400">No open findings.</p>
          ) : (
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <Panel title="Live monitoring feed" sub="Latest outside-in signals — click through to the vendor">
          {data.monitoring_feed.length === 0 ? (
            <p className="py-8 text-center text-xs text-gray-400">No monitoring signals.</p>
          ) : (
              ...
              {data.monitoring_feed.slice(0, 8).map((s) => (
                <button key={s.id} onClick={() => router.push(`/vendor-risk/vendors/${s.vendor_id}`)}
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-[11px] text-gray-500">{s.vendor_name} · {titleCase(s.signal_type)} · {fmtDate(s.occurred_at)}</p>
                  </div>
                  {!s.acknowledged && <span className="mt-0.5 rounded-full bg-blue-50 px-1.5 py-0.5 text-[10px] font-medium text-blue-600">new</span>}
                </button>
              ))}
        ...
        <Panel title="Highest-residual vendors" sub="Ranked by residual risk — click through to the profile">
          {data.top_residual_vendors.length === 0 ? (
            <p className="py-8 text-center text-xs text-gray-400">No scored vendors yet.</p>
          ) : (
