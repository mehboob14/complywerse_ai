...
export default function AssetsPage() {
  ...
  // Inventory ⇄ CIS Benchmark view toggle (CIS merged into IT Asset Inventory)
  const [activeView, setActiveView] = useState<'inventory' | 'cis'>('inventory');
  ...
  return (
    <div className="assets-light space-y-4 sm:space-y-5 px-3 sm:px-4 pt-3">
      {/* Dashboard zone — the warm asset-suite theme is scoped to this wrapper
          ONLY, so the reverted inventory table below keeps its original styling. */}
      <div className="asset-suite space-y-3.5">
        <div className="as-fadeup" style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 10, flexWrap: 'wrap' }}>
          <div className="as-seg">
            <button type="button" className={activeView === 'inventory' ? 'is-active' : ''} onClick={() => setActiveView('inventory')}>Inventory</button>
            <button type="button" className={activeView === 'cis' ? 'is-active' : ''} onClick={() => setActiveView('cis')}>CIS Benchmark</button>
          </div>
        </div>
