...
const DOMAIN_RULES: Array<[RegExp, string]> = [
  // Complyverse — cyber threat-intelligence sources (attack-surface product). These
  // resolve to each org's real logo via favicon/Clearbit instead of initials.
  [/shodan/i, 'shodan.io'],
  [/censys/i, 'censys.io'],
  [/crt\.sh|certificate transparency/i, 'crt.sh'],
  [/securitytrails|security trails/i, 'securitytrails.com'],
  [/mitre|att&ck/i, 'attack.mitre.org'],
  [/\bcwe\b|common weakness/i, 'cwe.mitre.org'],
  [/\bcve\b|common vulnerabilit/i, 'cve.org'],
  [/cisa|kev|known exploited/i, 'cisa.gov'],
  [/epss|exploit prediction/i, 'first.org'],
  [/\bnvd\b|national vulnerability/i, 'nvd.nist.gov'],
  [/owasp/i, 'owasp.org'],
...
export function FrameworkLogo({
  ...
  // Complyverse: start at the Google favicon service (1) — it reliably returns each
  // org's real logo. 2 = Clearbit, 3 = initials badge. (Bundled /public assets
  // only exist for a handful of frameworks, so we skip that stage.)
