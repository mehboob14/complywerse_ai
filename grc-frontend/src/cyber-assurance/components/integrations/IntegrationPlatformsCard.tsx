...
import { cloudConnectorsApi, integrationsApi } from '@/cyber-assurance/lib/api';
import {
...
export function IntegrationPlatformsCard() {
  ...
  const onClick = (p: PlatformDef) => {
    if (p.kind === 'cloud_connector') {
      ...
      router.push(`/cyber-assurance/admin?tab=cloud-connectors&provider=${p.provider}`);
    } else {
      router.push('/cyber-assurance/integrations/connections');
    }
  };
