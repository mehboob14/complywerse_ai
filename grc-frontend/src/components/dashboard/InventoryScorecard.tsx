...
export default function InventoryScorecard() {
  ...
  if (!data || data.performance.score == null) return null;

  const perf = data.performance;
  const band = scoreBand(perf.score);
  const sections = ORDER.map((k) => data.sections[k]).filter((s): s is OverviewSection => Boolean(s));
  const attn = ATTENTION.map((a) => ({ ...a, count: data.attention_queue?.[a.key] ?? 0 }));
  const attnTotal = data.attention_queue?.total ?? 0;
