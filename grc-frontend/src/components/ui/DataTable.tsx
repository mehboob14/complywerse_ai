...
export function DataTable<T extends { id: string | number }>({
  ...
  if (loading) {
    return (
      <div className={clsx('rounded-xl border border-slate-700 bg-surface-800 overflow-hidden', className)}>
        <div className="p-4 border-b border-slate-700">
          <div className="flex items-center gap-4">
            <div className="h-10 w-64 rounded-lg bg-slate-700 animate-pulse" />
            <div className="h-10 w-32 rounded-lg bg-slate-700 animate-pulse" />
          </div>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-700 bg-surface-900">
                ...
                <tr key={i} className="border-b border-slate-700/50">
                  {initialColumns.slice(0, 5).map((_, j) => (
                    <td key={j} className="px-4 py-3">
                      <div className="h-4 w-full max-w-32 rounded bg-slate-700 animate-pulse" />
                    </td>
                  ))}
                </tr>
              ))}
  ...
  return (
    <div className={clsx('rounded-xl border border-slate-700 bg-surface-800 overflow-hidden', className)}>
            ...
            {searchable && (
              <div className="relative flex-1 min-w-64">
                ...
                <input
                  ...
                  className="w-full rounded-lg border border-slate-600 bg-surface-900 py-2 pl-10 pr-4 text-sm text-white placeholder-slate-400 focus:border-primary-500 focus:outline-none focus:ring-1 focus:ring-primary-500 transition-colors"
            ...
            <div className="flex items-center gap-2 ml-auto">
              <div className="relative">
                <button
                  ...
                  className="flex items-center gap-2 rounded-lg border border-slate-600 bg-surface-900 px-3 py-2 text-sm text-slate-300 hover:text-white hover:border-slate-500 transition-colors"
                ...
                {showColumnMenu && (
                  <>
                    ...
                    />
                    <div className="absolute right-0 top-full mt-2 z-50 w-48 rounded-lg border border-slate-600 bg-surface-800 py-1 shadow-elevated">
                      {initialColumns.map((col) => (
                        <button
                          key={col.id}
                          onClick={() => toggleColumnVisibility(col.id)}
                          className="flex w-full items-center gap-2 px-3 py-2 text-sm text-slate-300 hover:bg-surface-700 hover:text-white transition-colors"
                        >
                          <div className={clsx(
      ...
      {selectable && selectedRows.size > 0 && (
        <div className="flex items-center gap-4 px-4 py-3 bg-primary-500/10 border-b border-primary-500/30">
              ...
              {selectable && (
                <th className="w-12 px-4 py-3">
                  <input
                    ...
                    className="h-4 w-4 rounded border-slate-500 bg-surface-800 text-primary-500 focus:ring-primary-500 focus:ring-offset-0"
      ...
      {sortedData.length > 0 && (
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 px-4 py-3 border-t border-slate-700">
          <div className="flex items-center gap-2 text-sm text-slate-400">
            ...
            <select
              ...
              className="rounded border border-slate-600 bg-surface-900 px-2 py-1 text-sm text-white focus:border-primary-500 focus:outline-none"
            >
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-1 rounded-lg border border-slate-600 bg-surface-900 px-3 py-1.5 text-sm text-slate-300 hover:text-white hover:border-slate-500 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
