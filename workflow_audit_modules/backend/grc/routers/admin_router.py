...
from .auth_router import decode_token, require_auth, get_current_user, get_user_primary_tenant

router = APIRouter(prefix="/admin", tags=["Administration"])


# ─── Tenant-wide overview (admin dashboard) ─────────────────────────────
# Per Hassan: admin needs one screen that shows the WHOLE tenant — total
# assets, scan coverage, pass rate, agents/connections health, recent
# failures. Today these numbers were scattered across compliance-overview,
# per-user-summary, agents page, integrations page. This endpoint
# aggregates them so the frontend can render one dashboard.
@router.get("/overview")
def get_admin_overview(
    db: Session = Depends(get_db),
    current_user: GRCUser = Depends(require_auth),
):
    """Single tenant-wide aggregation for the admin dashboard.

    Every count is constrained to the caller's primary tenant. No global
    cross-tenant data leaks even if a different tenant has more activity.
    """
    tenant_id = get_user_primary_tenant(current_user, db)
    if not tenant_id:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="User has no primary tenant",
        )
    tenant = db.query(Tenant).filter(Tenant.id == tenant_id).first()

    def scalar(sql: str, **params):
        return db.execute(text(sql), {"tid": tenant_id, **params}).scalar() or 0

    def fetchall(sql: str, **params):
        return [dict(r._mapping) for r in
                db.execute(text(sql), {"tid": tenant_id, **params}).fetchall()]

    # ─── Asset counts ────────────────────────────────────────────────
    asset_total = scalar("SELECT count(*) FROM grc_it_assets WHERE tenant_id=:tid")
    asset_with_os = scalar(
        "SELECT count(*) FROM grc_it_assets WHERE tenant_id=:tid AND os_normalized IS NOT NULL AND os_normalized != ''"
    )
    asset_with_benchmark = scalar(
        """SELECT count(DISTINCT a.id) FROM grc_it_assets a
           JOIN grc_benchmark_os_mappings m ON
             m.is_active = true
             AND (m.tenant_id = a.tenant_id OR m.tenant_id IS NULL)
             AND (lower(a.os_normalized) = lower(m.os_pattern)
                  OR lower(a.os_normalized) LIKE lower(m.os_pattern) || '-%')
           WHERE a.tenant_id=:tid AND a.os_normalized IS NOT NULL"""
    )
    by_os_family = fetchall(
        """SELECT COALESCE(os_family, 'unknown') AS key, count(*) AS n
           FROM grc_it_assets WHERE tenant_id=:tid
           GROUP BY os_family ORDER BY n DESC"""
    )
    by_criticality = fetchall(
        """SELECT criticality AS key, count(*) AS n FROM grc_it_assets
           WHERE tenant_id=:tid GROUP BY criticality ORDER BY n DESC"""
    )

    # ─── Scan KPIs ───────────────────────────────────────────────────
    runs_total = scalar("SELECT count(*) FROM grc_compliance_plugin_runs WHERE tenant_id=:tid")
    runs_24h = scalar(
        "SELECT count(*) FROM grc_compliance_plugin_runs WHERE tenant_id=:tid AND started_at > now() - interval '24 hours'"
    )
    runs_7d = scalar(
        "SELECT count(*) FROM grc_compliance_plugin_runs WHERE tenant_id=:tid AND started_at > now() - interval '7 days'"
    )
    passed = scalar(
        "SELECT count(*) FROM grc_compliance_plugin_runs WHERE tenant_id=:tid AND status='passed'"
    )
    pass_rate = (passed / runs_total) if runs_total else None
    by_status = fetchall(
        """SELECT status AS key, count(*) AS n FROM grc_compliance_plugin_runs
           WHERE tenant_id=:tid GROUP BY status ORDER BY n DESC"""
    )
    by_trigger = fetchall(
        """SELECT COALESCE(triggered_by, 'unknown') AS key, count(*) AS n FROM grc_compliance_plugin_runs
           WHERE tenant_id=:tid GROUP BY triggered_by ORDER BY n DESC"""
    )

    # ─── Per-benchmark coverage ─────────────────────────────────────
    benchmarks = fetchall(
        """SELECT m.benchmark_name AS benchmark,
                  count(DISTINCT a.id) AS assets_covered,
                  (SELECT count(*) FROM grc_compliance_plugins p
                   WHERE p.benchmark = m.benchmark_name
                     AND p.enabled=true
                     AND p.review_status IN ('approved','auto_approved'))
                    AS applicable_rules
           FROM grc_benchmark_os_mappings m
           LEFT JOIN grc_it_assets a ON
             a.tenant_id=:tid
             AND (lower(a.os_normalized) = lower(m.os_pattern)
                  OR lower(a.os_normalized) LIKE lower(m.os_pattern) || '-%')
           WHERE m.is_active=true AND (m.tenant_id=:tid OR m.tenant_id IS NULL)
           GROUP BY m.benchmark_name
    ...
    conn_total = scalar(
        "SELECT count(*) FROM grc_integration_connections WHERE tenant_id=:tid"
    )
    conn_connected = scalar(
        "SELECT count(*) FROM grc_integration_connections WHERE tenant_id=:tid AND status='connected'"
    )
    agent_total = scalar(
        "SELECT count(*) FROM grc_compliance_agents WHERE tenant_id=:tid"
    )
    agent_active_fresh = scalar(
        """SELECT count(*) FROM grc_compliance_agents
           WHERE tenant_id=:tid AND status='active'
             AND last_heartbeat_at > now() - interval '1 hour'"""
    )
    agent_stale = scalar(
        """SELECT count(*) FROM grc_compliance_agents
           WHERE tenant_id=:tid AND last_heartbeat_at IS NOT NULL
             AND last_heartbeat_at <= now() - interval '1 hour'"""
    )
    agent_pending = scalar(
        "SELECT count(*) FROM grc_compliance_agents WHERE tenant_id=:tid AND status='pending'"
    )

    # ─── Recent failures (last 24h) ─────────────────────────────────
    failures = fetchall(
        """SELECT r.id AS run_id, r.started_at, r.error_message,
                  a.id AS asset_id, a.name AS asset_name,
                  p.rule_id, p.title AS rule_title, p.benchmark
           FROM grc_compliance_plugin_runs r
           LEFT JOIN grc_it_assets a ON a.id = r.asset_id
           LEFT JOIN grc_compliance_plugins p ON p.id = r.plugin_id
           WHERE r.tenant_id=:tid AND r.status IN ('failed','error')
    ...
    return {
        "tenant": {
            ...
            "slug": getattr(tenant, "subdomain", None) if tenant else None,
        },
        "assets": {
            "total": asset_total,
            "with_os_classified": asset_with_os,
            "with_benchmark_mapped": asset_with_benchmark,
        ...
        "agents": {
            "total": agent_total,
            "active_fresh": agent_active_fresh,
            "stale": agent_stale,
...
def create_user(
    ...
    for role_id in data.role_ids:
        ...
        if role:
            # `tenant_id` on `user_roles` is NOT NULL in both Postgres and
            # SQLite. Setting it to None on Postgres (as the previous code
            # did) breaks the INSERT — fixed by always populating from the
            # tenant_schema we resolved at the top of this handler.
            user_role = UserRole(
                tenant_id=tenant_schema,
                user_id=new_user.id,
                role_id=role_id,
...
def generate_audit_log_ai_summary(
    ...
    row_for_ai = {
        ...
        # Workflow-execution rows store step snapshots (each carrying node_meta)
        # under changes.after.steps. Pass `after` through so the AI prompt
        # can render a per-node #### Steps section explaining what each node
        # did, which module it inherited from, and which endpoint it called.
        "after": changes.get("after"),
        "before": changes.get("before"),
    }
