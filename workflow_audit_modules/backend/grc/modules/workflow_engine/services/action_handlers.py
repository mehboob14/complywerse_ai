...
class WorkflowActionHandlers:
    ...
    def _execute_platform_capability_action(db, instance, definition, action_name: str, payload: Dict[str, Any]) -> Dict[str, Any]:
        # Format: platform_action.<verb>.<module>.<submodule>.<functionality>
        parts = (action_name or "").split(".")
        verb = parts[1] if len(parts) > 1 else "trigger"
        ...
        # Generic fallback: log and return result
        db.add(WorkflowAuditLog(
            tenant_id=instance.tenant_id,
            workflow_definition_id=definition.id,
            workflow_instance_id=instance.id,
            event_type="action.platform_capability",
            message=f"Platform action executed: {verb} {module_key}/{submodule_key}/{functionality_key}",
            payload={"action_name": action_name, "input_payload": payload},
        ))
        return {"action": action_name, "result": "executed", "module": module_key, "functionality": functionality_key}
    ...
    def _send_notification_email(db, instance, definition, payload: Dict[str, Any]) -> Dict[str, Any]:
        to = payload.get("to") or payload.get("to_email") or payload.get("email")
        # Pattern-B seeded workflows use a single `recipients` list of user_id
        # strings; legacy workflows split it into `recipient_user_ids`.
        # Accept both shapes.
        ...
        if not recipients:
            logger.warning(
                ...
                "to_raw=%r user_ids=%s role_ids=%s tenant_id=%s — "
                "configure a 'to' email address, user IDs, or role IDs in the notification node",
                instance.id, definition.id, to, user_ids, role_ids, instance.tenant_id,
            )
            db.add(WorkflowAuditLog(
                tenant_id=instance.tenant_id,
                workflow_definition_id=definition.id,
                ...
                message="Email skipped: no recipients resolved — add a 'to' email, user IDs, or role IDs to the notification node config",
    ...
    def _send_in_app_alert(db, instance, definition, payload: Dict[str, Any]) -> Dict[str, Any]:
        # Pattern-B seeded workflows use a single `recipients` list of user_id
        # strings; legacy workflows split it into `recipient_user_ids`. Accept both.
        user_ids = _normalize_ids(
            payload.get("recipient_user_ids")
            or payload.get("user_ids")
            or payload.get("recipients")
    ...
    # ------------------------------------------------------------------
    # Missing catalog action stubs (evidence, compliance, governance, audit, control)
    # ------------------------------------------------------------------
    @staticmethod
    ...
    def _close_audit_finding(db, instance, definition, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        notes = payload.get("notes") or "Closed by automated workflow."
        if finding_id:
            try:
                finding = db.query(AuditFinding).filter(
                    AuditFinding.id == int(finding_id),
                    AuditFinding.tenant_id == instance.tenant_id,
                ).first()
                if finding:
        ...
        db.add(WorkflowAuditLog(
            ...
            message=f"Audit finding #{finding_id} closed: {notes[:100]}",
            payload={"finding_id": finding_id, "notes": notes},
        ))
        return {"action": "close_audit_finding", "finding_id": finding_id, "status": "closed"}
