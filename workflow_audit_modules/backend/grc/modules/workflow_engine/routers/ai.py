...
def _ensure_trigger_node(nodes: list[dict], edges: list[dict], trigger_event: str) -> None:
    """Ensure the graph has a proper trigger node right after Start.

    The save validation (_validate_workflow_graph) looks at the *first node
    after Start* — the target of Start's outgoing edge.  That node must be
    either (a) a dedicated trigger (is_start=True + config.trigger_type) or
    (b) a platform-function action that infers a trigger event.

    GPT often generates Start → Condition/Approval/Action(send_email) which
    fails both checks.  This function inserts a dedicated trigger node
    between Start and whatever GPT placed first.
    """
    start_node = None
    for n in nodes:
        if (n.get("node_key") or "").lower() == "start":
            start_node = n
            break
    if not start_node:
        return

    start_key = start_node.get("node_key")
    start_edges = [e for e in edges if e.get("source_node_key") == start_key]
    if not start_edges:
        return

    first_target_key = start_edges[0].get("target_node_key")
    first_node = None
    for n in nodes:
        if n.get("node_key") == first_target_key:
            first_node = n
            break

    if first_node:
        fcfg = first_node.get("config") or {}
        if first_node.get("is_start") and fcfg.get("trigger_type"):
            return
        if (first_node.get("node_type") or "").lower() == "start" and fcfg.get("trigger_type"):
            return

    trigger_labels = {t["key"]: t["label"] for t in TRIGGER_NODE_TYPES}
    label = trigger_labels.get(trigger_event, trigger_event.replace("_", " ").title())

    trigger_node = {
        "node_key": "trigger_event",
        "node_type": "start",
        "name": label,
        "label": label,
        "is_start": True,
        "config": {"trigger_type": trigger_event},
        "x": 350,
        "y": (start_node.get("y", 30) + 130),
    }

    for n in nodes:
        if n.get("node_key") != start_key:
            n["y"] = n.get("y", 0) + 130

    nodes.insert(1, trigger_node)

    for e in start_edges:
        e["source_node_key"] = "trigger_event"

    edges.append({
        "source_node_key": start_key,
        "target_node_key": "trigger_event",
        "edge_label": "", "priority": 1, "condition": {},
    })


def _fuzzy_match_key(candidate: str, valid_keys: set[str]) -> str | None:
    """Find the closest matching key from a set of valid keys."""
    c = candidate.lower().replace("-", "_").replace(" ", "_")
    if c in valid_keys:
        return c
    for vk in valid_keys:
        if c in vk or vk in c:
            return vk
    parts = c.split("_")
    best, best_score = None, 0
    for vk in valid_keys:
        vk_parts = set(vk.split("_"))
        overlap = sum(1 for p in parts if p in vk_parts)
...
def ai_natural_language_to_workflow(
    ...
    try:
        ...
        for i, node in enumerate(data["nodes"]):
            ...
            if "y" not in node:
                node["y"] = 30 + i * 130
            nk = node.get("node_key", f"step_{i}")
            node["node_key"] = nk
            node_keys.add(nk)
            nt = (node.get("node_type") or "").lower()
            if nt == "end" or node.get("is_terminal"):
                has_end = True
            last_node_key = nk

        # Auto-add End node if GPT omitted it
        if not has_end:
            max_y = max((n.get("y", 0) for n in data["nodes"]), default=30)
            end_node = {
                "node_key": "end", "node_type": "end",
                "name": "End", "label": "End",
                "is_terminal": True, "config": {},
                "x": 350, "y": max_y + 130,
            }
            data["nodes"].append(end_node)
            if last_node_key and last_node_key != "end":
                already_sourced = {e.get("source_node_key") for e in data["edges"]}
                terminal_keys = [
                    n.get("node_key") for n in data["nodes"]
                    if n.get("node_key") != "end"
                    and n.get("node_key") not in already_sourced
                    and n.get("node_type") != "start"
                ]
                if not terminal_keys:
                    terminal_keys = [last_node_key]
        ...
        # Validate trigger_event against known trigger keys
        valid_triggers = {t["key"] for t in TRIGGER_NODE_TYPES}
        raw_trigger = data.get("trigger_event", "")
        if raw_trigger and raw_trigger not in valid_triggers:
            best = _fuzzy_match_key(raw_trigger, valid_triggers)
            if best:
                raw_trigger = best
            else:
                raw_trigger = payload.target_trigger_event or "manual_trigger"
        if not raw_trigger:
            raw_trigger = payload.target_trigger_event or "manual_trigger"

        # Validate action_names in action nodes
        valid_actions = {a["key"] for a in ACTION_NODE_TYPES}
        for node in data["nodes"]:
            cfg = node.get("config") or {}
            aname = cfg.get("action_name", "")
            if aname and aname not in valid_actions:
                best = _fuzzy_match_key(aname, valid_actions)
                if best:
                    cfg["action_name"] = best

        # Ensure Start node has trigger_type set so save validation passes.
        # The save endpoint requires the first node after Start to be a
        # dedicated trigger node (is_start + config.trigger_type) OR an action
        # that infers a trigger. GPT often omits trigger_type from Start config
        # or places a condition first, which fails validation.
        _ensure_trigger_node(data["nodes"], data["edges"], raw_trigger)

        return {
            "name": data.get("name", f"AI: {payload.prompt[:60]}"),
            "description": data.get("description", ""),
            "trigger_event": raw_trigger,
            "trigger_conditions": {},
...
def ai_workflow_suggestions(
    ...
    try:
        ...
        # Only return early if GPT produced real suggestions
        if suggestions:
    ...
    # --- Rule-based fallback (runs when GPT fails OR returns empty) ---
    suggestions = []
    ...
    if not suggestions:
        ...
        _default = [
            {
                ...
                "description": "Auto-escalate risks that breach a critical score threshold to management with a 24-hour SLA timer.",
            ...
            {
                "title": "Evidence Expiry Reminder & Re-collection",
                "description": "Notify control owners 30 days before evidence expires and automatically re-open collection if not renewed.",
                "framework_ref": "General",
                "trigger_event": "evidence_expires",
                "category": "evidence_management",
                "complexity": "medium",
            },
            {
                ...
                "description": "Automatically assign owners, notify management, and escalate unresolved incidents when an incident is reported.",
                "framework_ref": "General",
            ...
            {
                "title": "Vulnerability SLA Breach Escalation",
                "description": "Escalate critical vulnerabilities not remediated within SLA to CISO and create a risk entry automatically.",
                "framework_ref": "General",
                "trigger_event": "vulnerability_sla_breached",
                "category": "vulnerability_management",
                "complexity": "medium",
            },
            {
                "title": "Policy Review & Renewal Cycle",
                "description": "Trigger annual policy review approvals, notify owners 60 days before expiry, and publish after sign-off.",
                "framework_ref": "General",
                "trigger_event": "governance_document_expires",
                "category": "governance",
                "complexity": "medium",
            },
            {
                "title": "Compliance Evidence Review & Gap Closure",
                "description": "Batch-assess uploaded evidence, calculate compliance scores, and generate gap-closure tasks for failed controls.",
                "framework_ref": "General",
        ...
        suggestions = [
            {**s, "already_exists": s["title"].lower() in existing_set}
            for s in _default
        ]
