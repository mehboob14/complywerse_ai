...
const NodePalette: React.FC<NodePaletteProps> = ({ onNodeDragStart }) => {
  ...
  return (
    <div className="w-80 bg-white border-r border-gray-200 h-full overflow-y-auto">
      ...
      {/* Action Modules */}
      {Object.entries(filteredCatalog.actions).map(([moduleName, nodes]: [string, NodeDefinition[]]) => (
...
export const WorkflowBuilder: React.FC<WorkflowBuilderProps> = ({ workflowId, onSave }) => {
  ...
  const handleSave = async () => {
    ...
    const workflowNodes: WorkflowNode[] = nodes.map((node) => ({
      node_key: node.id,
      node_id: (node.data.nodeDefinition as { node_id: string })?.node_id,
      display_name: node.data.label as string,
