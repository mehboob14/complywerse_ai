...
| A6 Reasoned, re-runnable control mapping (P1–P5) | LANDED (18 Aug, ponytail form) | Plan of record `docs/CTEM_VALIDATE_REASONING_PLAN.md` § Status has the layer-by-layer shipped-vs-skipped table. Shipped: corpus retrieval over Unified Library + ALL uploaded frameworks (prompt-local ids, no id-space collision); proposals target either control kind (additive columns, nullable UCL ref); REASON ONCE/APPLY MANY — a human's accept on a (CWE, control) pair is applied to later findings of that CWE as `provenance=reused` (original approver carried, no model call), a reject is never re-proposed for that CWE; post-scanner-sync hook reasons over findings with no proposal (non-fatal, stats on the run row); crosswalk rows carry `basis` (rule/ai/reused/manual) + reason, UI chip + hover; 12-uncovered-CWE battery (caught 2 retrieval holes, fixed); prompt v1.5 after a live semantic pass found CWE-289→MFA (fixed, re-judged ✓). Skipped with named triggers: index/pgvector, published priors (⇒ nothing auto-accepts), weekly sweep/drift sample, why-drawer, no-CVE class keys. Coverage: 18/18 tenant CWEs reach the model (was 6/18). |
