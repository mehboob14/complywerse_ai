...
export default function ConnectWizardPage() {
  ...
  useEffect(() => {
    ...
    const valid = ['windows', 'linux', 'aws', 'digitalocean', 'cisco', 'oracle',
                   'mssql', 'postgres', 'mysql', 'ad', 'azure', 'k8s'] as const;
...
function ManualCredsForm({
  ...
  platform:
    ...
    | 'mssql' | 'postgres' | 'mysql' | 'ad';
  token: string;
  onCancel: () => void;
}) {
  // Per-platform default ports: WinRM 5986, SSH 22, Oracle TNS 1521,
  // SQL Server 1433, Postgres 5432, MySQL 3306, LDAP 389 (LDAPS 636).
  const defaultPort =
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      const r = await apiClient.post('/connect-wizard/handshake', {
        ...
        database_name: (platform === 'mssql' || platform === 'postgres' || platform === 'mysql') ? (cleanExtra || null) : null,
        db_port: (platform === 'mssql' || platform === 'postgres' || platform === 'mysql')
...
function ConnectedSuccessWithScope({
  ...
  const saveAndGo = async () => {
    if (!status.connection_id) { navigate('/dashboard'); return; }
    setSaving(true);
    try {
      ...
      setSaved(true);
      setTimeout(() => navigate('/admin/connections'), 800);
    } catch {
