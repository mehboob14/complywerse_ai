...
from ....models import (
    AuditLog, Vulnerability, VulnerabilityReport, VulnerabilitySLAConfig,
...
def create_vulnerability(
    ...
    write_rich_audit_log(
        ...
        resource_id=vuln.id,
        resource_name=vuln.title,
        summary=f"Created Vulnerability '{vuln.title}' [{vuln.vuln_id}] (severity: {vuln.severity})",
        snapshot=model_to_dict(vuln),
        resource_url=f"/vulnerabilities/{vuln.id}",
    )
    db.commit()
...
def update_vulnerability(
    ...
    _before = {k: getattr(vuln, k, None) for k in update_data}
    _full_before = model_to_dict(vuln)
    ...
    write_rich_audit_log(
        ...
        resource_id=vuln.id,
        resource_name=vuln.title,
        summary=f"Updated Vulnerability '{vuln.title}': changed {', '.join(_changed_fields) or 'fields'}",
        before=_full_before,
        after=model_to_dict(vuln),
        resource_url=f"/vulnerabilities/{vuln.id}",
    )
    db.commit()
...
def delete_vulnerability(
    ...
    _saved_id = vuln.id
    _saved_title = vuln.title
    _saved_tenant = vuln.tenant_id
    _saved_snapshot = model_to_dict(vuln)
    ...
    write_rich_audit_log(
        ...
        summary=f"Deleted Vulnerability '{_saved_title}'",
        snapshot=_saved_snapshot,
        resource_url=f"/vulnerabilities/{_saved_id}",
    )
    db.commit()
